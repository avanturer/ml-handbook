...
**Время на проработку:** ~7 часов (чтение + вывод руками + реализация на numpy)
...
а логиты отдают прямо в softmax внутри функции потерь — так численно устойчивее, см. §11).
...
Теперь то, ради чего глава. Выведем градиенты MLP из §2 полностью, без пропусков, и проверим
...
исключено. Но как инструмент **отладки** на маленькой сети — незаменимо (см. §7).
...
Теперь то же самое, но на матрицах и с настоящим MLP. Это код из §5.5, буквально.
...
Требования к активации: нелинейность (иначе §2.1), дешёвое вычисление, и — критично —
...
2. **Максимум производной 0,25 < 1.** Даже в лучшей точке слой ослабляет градиент вчетверо.
   На 20 слоях это множитель $0{,}25^{20} \approx 9 \cdot 10^{-13}$ — см. §9.
...
Разверните рекуррентное соотношение §5.3 от последнего слоя к первому:
...
[главе про бустинг](../02-classic-ml/07-gradient-boosting.md).
...
потребителями суммирует», §4). Без `optimizer.zero_grad()` градиенты складываются по всем шагам.
...
нормализация перед активацией. Отдельно стоит понимать, что **разреженность сама по себе
не беда**: ~50% нулей на объекте — норма и даже полезна; проблема именно в нейронах, нулевых
на всех объектах.
...
от приведённых в §1.2. Затем обучите такую же сеть градиентным спуском из случайной
...
Расширьте `Value` из §7 операциями `__pow__`, `exp`, `relu`, `__truediv__` и обратными
...
- [Nielsen M. «Neural Networks and Deep Learning»](http://neuralnetworksanddeeplearning.com) —
  ...
  $\delta^{(l)}$, что и здесь. Первый выбор, если вывод в §5 показался тяжёлым.
...
- [Karpathy A. «micrograd»](https://github.com/karpathy/micrograd) — скалярный autograd
  на ~150 строк, из которого вырос `Value` в §7. Сравните со своей реализацией из задачи 3.
...
- Rumelhart D., Hinton G., Williams R. «Learning representations by back-propagating errors»
  ...
  математика там ровно та же, что в §5.

---

⬅️ [Поиск аномалий](../02-classic-ml/18-anomaly-detection.md) | 🏠 [Оглавление](../index.md) | ➡️ [Как на самом деле обучается сеть](02-training-dynamics.md)
