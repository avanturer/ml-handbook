...
> память обучения модели на 7B на салфетке, оценить срок обучения через правило 6ND и объяснить,
> что именно шардит ZeRO-3 и чем за это платит.
...
- [1. Из чего складывается память обучения](#1-из-чего-складывается-память-обучения)
- [2. Считаем память для модели на 7B](#2-считаем-память-для-модели-на-7b)
- [3. Смешанная точность: fp16, bf16, tf32](#3-смешанная-точность-fp16-bf16-tf32)
- [4. Накопление градиента](#4-накопление-градиента)
- [5. Пересчёт активаций (gradient checkpointing)](#5-пересчёт-активаций-gradient-checkpointing)
- [6. DDP: как работает all-reduce](#6-ddp-как-работает-all-reduce)
- [7. ZeRO и FSDP: что шардится на каждой стадии](#7-zero-и-fsdp-что-шардится-на-каждой-стадии)
- [8. Тензорный и пайплайн параллелизм](#8-тензорный-и-пайплайн-параллелизм)
- [9. Как выбрать схему: дерево решений](#9-как-выбрать-схему-дерево-решений)
- [10. Бюджет обучения: правило 6ND](#10-бюджет-обучения-правило-6nd)
...
## 1. Из чего складывается память обучения
...
## 2. Считаем память для модели на 7B
...
## 3. Смешанная точность: fp16, bf16, tf32

### Проблема

Обучение в fp32 использует вчетверо больше памяти на активации и не задействует тензорные ядра,
которые на современных GPU дают в разы больше FLOPs на половинной точности. Хочется считать
в 16 битах. Мешает диапазон.
...
## 5. Пересчёт активаций (gradient checkpointing)

**Проблема.** Обратный проход требует активаций прямого прохода. Для трансформера
на 32 слоя это десятки гигабайт ([§2](#2-считаем-память-для-модели-на-7b)). Активации — самая большая статья расхода, и она растёт
...
Цена по времени считается точно. Из разбора правила 6ND ([§10](#10-бюджет-обучения-правило-6nd)): прямой проход стоит $2ND$,
...
- **С FlashAttention** большая часть выигрыша от пересчёта attention уже получена: он не
  материализует матрицу $(B,h,n,n)$ (см.
...
## 7. ZeRO и FSDP: что шардится на каждой стадии
...
## 8. Тензорный и пайплайн параллелизм
...
## 9. Как выбрать схему: дерево решений
...
И общая метрика, по которой оценивают качество настройки, — **MFU** (Model FLOPs Utilization):
отношение полезных FLOPs к пиковым возможностям железа за то же время. Хорошие значения
для обучения LLM — 35–50 %; ниже 20 % означает, что где-то есть простой (коммуникация,
данные, мелкие ядра), и это надо чинить до покупки новых GPU.

---

## 10. Бюджет обучения: правило 6ND
...
в bf16 — 312 TFLOP/s. Реальная = пиковая × MFU. При MFU 40 %:
...
Что делать: закладывать состояние оптимизатора в расчёт заранее (см. [§1](#1-из-чего-складывается-память-обучения)); проверять память
...
**Задача 1. Калькулятор памяти (45 минут).**
...
**Задача 2. Смешанная точность (45 минут).**
...
**Задача 3. Накопление и checkpointing (60 минут).**
...
**Задача 4. DDP на практике (90 минут).**
...
**Задача 5. Бюджет обучения (30 минут).**
...
- [Micikevicius et al. «Mixed Precision Training» (2017)](https://arxiv.org/abs/1710.03740) —
  ...
  точности. Раздел 3 — ровно материал [§3](#3-смешанная-точность-fp16-bf16-tf32).
...
- [Korthikanti et al. «Reducing Activation Recomputation in Large Transformer Models» (2022)](https://arxiv.org/abs/2205.05198) —
  точные формулы памяти активаций (использованы в [§2](#2-считаем-память-для-модели-на-7b)) и идея селективного пересчёта.
...
- [Rajbhandari et al. «ZeRO: Memory Optimizations Toward Training Trillion Parameter Models» (2019)](https://arxiv.org/abs/1910.02054) —
  первоисточник трёх стадий с полным разбором памяти и коммуникации. Таблица из [§7](#7-zero-и-fsdp-что-шардится-на-каждой-стадии) — оттуда.
