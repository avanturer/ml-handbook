...
> 🌱 **База.** Несущих вещей здесь три. Первая — условия применимости t-теста
> ([§2](#2-t-тест-что-на-самом-деле-требуется)–[§3](#3-тест-уэлча-и-почему-он-лучше-по-умолчанию)):
> он требует нормальности **выборочного среднего**, а не данных, поэтому вопрос не «нормальна ли
> выручка», а «хватает ли $n$ при такой асимметрии» (правило $n > 355\gamma^2$), и по умолчанию
> берут Уэлча, а не Стьюдента. Вторая — что Манн — Уитни
> ([§4](#4-манн--уитни-что-он-проверяет-на-самом-деле)) отвечает на другой вопрос, «кто чаще
> выигрывает», и не является робастной заменой t-теста; контрпример из этого раздела стоит
> запомнить целиком. Третья — ratio-метрики
> ([§6](#6-ratio-метрики-почему-обычный-t-тест-не-работает)–[§8](#8-линеаризация)): почему единица
> рандомизации не совпадает с единицей, по которой считается метрика, и что вместо наивного теста
> делают дельта-метод или линеаризацию. Добавьте к этому формат отчёта из
> [§11](#11-p-value-и-доверительный-интервал-как-докладывать): решение принимается по
> доверительному интервалу эффекта, а не по p-value. Бутстрап ([§9](#9-бутстрап-и-его-стоимость))
> и тяжёлые хвосты ([§10](#10-тяжёлые-хвосты)) при первом чтении пролистайте: они понадобятся,
> когда вам встретится квантильная метрика или маленькая скошенная выборка, а не раньше;
> байесовский [§12](#12-байесовский-ab-те-же-данные-другой-вопрос) откладывается целиком, если
> унести из него один вывод — никакое правило остановки с фиксированным порогом не даёт иммунитета
> к подглядыванию. Идти дальше можно, когда вы своими словами объясните, почему нельзя считать CTR
> z-тестом по показам и почему «данные ненормальные — беру непараметрику» — это подмена вопроса.
