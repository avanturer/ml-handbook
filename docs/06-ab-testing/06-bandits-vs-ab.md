...
**Предварительно нужно:** [дизайн эксперимента](01-experiment-design.md), [статистические критерии](02-statistical-criteria.md), [ловушки A/B-тестов](04-pitfalls.md), [эксплорация и бандиты](../10-recsys/13-exploration-and-bandits.md)
...
Полный разбор с выводами — в главе [эксплорация и бандиты](../10-recsys/13-exploration-and-bandits.md);
...
что данные пригодны для последующей честной переоценки ([§7](#7-как-всё-таки-получить-честную-оценку)). Детерминированные алгоритмы
...
### Вторая половина счёта: ошибка II рода

Расчёт выше учёл только явные потери — трафик, отданный худшему варианту во время теста.
Но в формуле регрета ETC из [§2](#2-формализация-бандит-и-регрет) было второе слагаемое, и именно оно обычно доминирует:
с вероятностью $\beta$ тест не заметит эффект, команда оставит худший вариант — и будет
платить за это весь оставшийся горизонт. При стандартной мощности 0.8 это $\beta = 0.2$.

Посчитаем обе стратегии в конверсиях для тех же чисел ($p_A = 0.10$, $p_B = 0.11$, $n = 14\,752$
на группу). Для бандита возьмём асимптотическую скорость из границы Лая–Роббинса:
$R(T) \approx \frac{\Delta}{\mathrm{KL}(0.10\,\|\,0.11)} \ln T$, где $\mathrm{KL} \approx 0.000525$,
то есть примерно $19 \ln T$.

| Горизонт $T$ | Всего конверсий | ETC (A/B): тест + риск | Thompson sampling |
|---|---|---|---|
| $10^5$ | 11 000 | 348 = 148 + 200 | ≈ 220 |
| $10^6$ | 110 000 | 2 148 = 148 + 2 000 | ≈ 260 |
| $10^7$ | 1 100 000 | 20 148 = 148 + 20 000 | ≈ 310 |

Читать таблицу нужно с тремя оговорками, иначе она врёт в пользу бандита. Первая: столбец
Thompson sampling — асимптотическая скорость, на конечном горизонте реальный регрет выше.
Вторая: слагаемое «риск» посчитано в худшем для мощности случае, когда истинный эффект ровно
равен MDE; если эффект вдвое больше заявленного, $\beta$ падает почти до нуля и весь риск
исчезает. Третья: в жизни неубедительный тест не означает «тему закрыли навсегда» — гипотезу
переформулируют и перезапускают, так что линейный член на практике меньше.

Но качественный вывод остаётся, и он полезен: **популярный аргумент «бандиты экономят трафик»
слабый** — экономия составляет доли процента и легко считается. Сильный аргумент другой:
бандит не фиксирует ошибку навсегда, а explore-then-commit фиксирует. И отсюда практический
вывод уже про A/B: недомощный тест стоит гораздо дороже, чем кажется, потому что цена ошибки
II рода растёт линейно по горизонту жизни продукта. Мощность — не формальность из чек-листа.
...
посчитанная по его логам цифра будет неверной ([§6](#6-почему-нельзя-просто-сравнить-средние)). Если вы всё же строите на бандите систему
...
### 5.4. Метрика одна, а решение многомерное

Награда бандита — одно скалярное число, доступное быстро. Продуктовое решение таким почти
никогда не бывает: есть ключевая метрика, есть guardrail-метрики (латентность, жалобы,
отписки, доля ошибок), есть контрметрики. Бандит по построению не умеет сказать «этот вариант
выиграл по CTR, но у него выросла доля отписок — не катим». Он просто уведёт туда трафик,
и узнаете вы об этом из другого дашборда через две недели.

Три способа это ограничить. **Составная награда** со штрафами за нарушение guardrail —
работает, но веса вы задаёте руками, и это продуктовое решение, замаскированное под технический
гиперпараметр. **Жёсткие ограничения поверх бандита**: рука принудительно отключается, если
guardrail пробит, независимо от её награды. **Сужение полномочий**: отдать бандиту только те
решения, где расхождение между быстрым прокси и бизнес-целью заведомо мало.

Практическое правило: чем дальше награда бандита от бизнес-цели, тем опаснее давать ему власть.
Бандит на выборе картинки в баннере безопасен — вреда от «неправильной» картинки почти нет.
Бандит на выборе агрессивности пуш-уведомлений опасен: он найдёт максимум по кликам и по дороге
выжжет базу подписчиков, потому что отписка не входит в его награду.

> 💬 **На собеседовании.** Спросят: «Бандит поднял CTR блока на 8%, катим?» Хороший ответ:
> нет, пока не отвечу на два вопроса. Первый: откуда взялись эти 8% — если это разница средних
> по рукам в логах бандита, число смещено проклятием победителя и меняющимися долями трафика,
> его надо перемерить либо контрольным слоем, либо отдельным A/B. Второй: что произошло
> с метриками, которых нет в награде — конверсией дальше по воронке, отписками, жалобами,
> разнообразием выдачи. Бандит оптимизировал ровно одно число и про остальные ничего не знает.
> Плохой ответ: «CTR вырос значимо, катим» — здесь сразу две ошибки, смещённая оценка
> и отсутствие guardrail-метрик.
...
[эксплорацию и бандиты](../10-recsys/13-exploration-and-bandits.md).
...
**Бандит с постоянным контрольным слоем.** Схема из [§7](#7-как-всё-таки-получить-честную-оценку): бандит работает на 90% трафика,
...
по таблице из [§4](#4-сколько-стоит-эксперимент-считаем-регрет-ab), а не по новизне метода.
...
Возьмите код из [§6.1](#61-проклятие-победителя) и постройте зависимость фиктивного «прироста победителя» от числа рук
...
A/B, бандит или комбинацию, и обоснуйте по таблице из [§4](#4-сколько-стоит-эксперимент-считаем-регрет-ab). Для каждой опишите, как будет
измеряться эффект.
...
- **Lattimore, Szepesvári. «Bandit Algorithms» (Cambridge University Press, 2020)** —
  ...
  Главы про регрет, нижние границы и UCB закрывают [§2](#2-формализация-бандит-и-регрет) и [§3](#3-три-алгоритма-за-три-абзаца) на уровне сильно глубже
  собеседования.
...
- **Chapelle, Li. «An Empirical Evaluation of Thompson Sampling» (NIPS 2011)** — работа,
  ...
  что прямо относится к [§5.2](#52-отложенная-конверсия).
...
- [Эксплорация и бандиты](../10-recsys/13-exploration-and-bandits.md) — глава этого хендбука
  с выводом Thompson sampling для Бернулли, LinUCB и off-policy оценкой (IPS, doubly robust).
- [Холодный старт и смещения](../10-recsys/12-cold-start-and-bias.md) — про то, зачем
  эксплорация нужна не только для регрета, но и для того, чтобы система не выродилась
  в петле обратной связи.
- Дальше по разделу — [платформа экспериментов](07-experiment-platform.md): как устроена
  инфраструктура, в которой фиксированные доли из этой главы раздаются двум сотням
  экспериментов одновременно, — слои, соли, контрфактическое логирование и ёмкость.

---

⬅️ [Сложные схемы](05-complex-designs.md) | 🏠 [Оглавление](../index.md) | ➡️ [Платформа экспериментов](07-experiment-platform.md)
