...
**Предварительно нужно:** [метрики качества](04-metrics.md), [логистическая регрессия](03-logistic-regression.md), [градиентный бустинг](08-gradient-boosting.md)
**Как проработать:** чтение + эксперимент + реализация калибровки
...
выводится из матрицы стоимостей (полный вывод — в [главе о метриках](04-metrics.md), [§5](#5-рычаг-четвёртый-focal-loss)):
...
о котором предупреждает [глава о метриках, §3.3](04-metrics.md#33-связь-со-статистикой-маннауитни), и в задачах с ограниченным
...
- **строго монотонна** — ROC-AUC не меняется вообще (см. таблицу [§6](#6-сводный-эксперимент));
...
- **выход кусочно-постоянный** — соседние скоры склеиваются в блоки, появляются связки.
  Отсюда просадка PR-AUC в [§6](#6-сводный-эксперимент): в голове списка разрешение теряется;
...
(или нормировку голосов), систематически сжимают предсказания к 0.5. Проверим это руками:
...
rng = np.random.default_rng(0)
y = np.where(rng.random(len(y)) < 0.15, 1 - y, y)          # 15% шума в метках
Xtr, Xte, ytr, yte = train_test_split(X, y, test_size=0.3, random_state=0, stratify=y)

p = AdaBoostClassifier(n_estimators=300, random_state=0).fit(Xtr, ytr).predict_proba(Xte)[:, 1]
print(f"диапазон предсказаний: [{p.min():.3f}, {p.max():.3f}]")

obs, pred = calibration_curve(yte, p, n_bins=5, strategy="quantile")
for pp, pt in zip(pred, obs):
    flag = "  <- переуверен" if pp - pt > 0.05 else ("  <- недоуверен" if pt - pp > 0.05 else "")
    print(f"предсказано {pp:.3f}   наблюдается {pt:.3f}{flag}")
```

```
диапазон предсказаний: [0.313, 0.724]
предсказано 0.364   наблюдается 0.147  <- переуверен
предсказано 0.389   наблюдается 0.157  <- переуверен
предсказано 0.407   наблюдается 0.170  <- переуверен
предсказано 0.433   наблюдается 0.196  <- переуверен
предсказано 0.525   наблюдается 0.610  <- недоуверен
```

Диапазон предсказаний — от 0.31 до 0.72: модель физически не способна сказать «0.95»,
как бы очевиден ни был объект. В четырёх нижних корзинах она переуверена (обещает 0.36 —
случается 0.15), в верхней недоуверена (обещает 0.53 — случается 0.61).
...
даёт приличную калибровку из коробки (в таблице [§6](#6-сводный-эксперимент) базовая модель имеет ECE = 0.0037 —
...
2. **Взвешивание классов и ресэмплинг** — механизм разобран в [§3.1](#31-что-происходит-с-функцией-потерь) и [§6](#6-сводный-эксперимент).
...
воспроизведите таблицу [§6](#6-сводный-эксперимент) на своих данных: базовая модель, веса классов, андерсэмплинг,
...
(скелет — в [§7.2](#72-reliability-diagram)). Проверьте на трёх контролируемых случаях: (а) идеально калиброванные
...
(см. код [§5.2](#52-насколько-это-меняет-вклады)). Сравните с `scale_pos_weight` на одном и том же датасете по PR-AUC и по ECE.
...
⬅️ [Снижение размерности](12-dimensionality-reduction.md) | 🏠 [Оглавление](../index.md) | ➡️ [Работа с признаками](14-feature-engineering.md)
