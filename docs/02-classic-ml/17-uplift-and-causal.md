...
**Время на проработку:** ~5 часов (чтение + расчёт Qini руками + реализация метаобучателей)
...
uplift-модели оценивают не тип, а его ожидание: $\tau(x) = P(\text{persuadable} \mid x) -
P(\text{sleeping dog} \mid x)$.
...
Отдельный простой критерий — $\Delta\Delta p$: максимизировать $|(p_T(A_L) - p_C(A_L)) -
(p_T(A_R) - p_C(A_R))|$, то есть буквально разницу uplift между детьми.
...
**Короткий ответ.** DiD оценивает эффект как разность приростов: $(\bar Y^{post}_T - \bar Y^{pre}_T) -
(\bar Y^{post}_C - \bar Y^{pre}_C)$. Допущение — параллельные тренды: без воздействия обе группы
