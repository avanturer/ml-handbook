...
**Предварительно нужно:** [постановка задачи обучения](01-learning-theory.md), [метрики качества](04-metrics.md), [валидация и утечки](13-validation-and-leakage.md), [интерпретируемость](15-interpretability.md)
...
цифру, а половина из них — это две конкретные пары. Разница между способами не в трудолюбии,
а в том, что первый оптимизирует вслепую, а второй сначала измеряет, где именно теряется качество.
...
Обе нужны, и путать их дорого. Сегмент с lift 5 и долей в выборке 0.4% даёт потолок улучшения
в 2% ошибок — им можно заниматься месяц и не сдвинуть метрику. Сегмент с lift 1.3, но с половиной
выборки, даёт втрое больше. Приоритет ставится по $\Delta_c$, а $\text{lift}_c$ подсказывает, где
у проблемы есть общая причина, которую вообще можно устранить.
...
Читаем. Цифра «1» занимает 10% выборки и даёт 32.7% всех ошибок — lift 3.2. Вторая строка списка
пар говорит, где именно теряется: 8→1 и 1→8 вместе дают 12 ошибок из 49, то есть четверть всего.
...
> В хвостовом сегменте на 30 объектов lift 4 получается из 5 ошибок вместо 1.2 ожидаемых —
> это шум, а не сигнал, и потолок улучшения там 0.7% ошибок. Сортируйте по $\Delta_c$
...
| уверенность модели | $\max_k \hat p_k$ | уверенная ошибка и неуверенная — разные болезни |
...
Колонка с уверенностью нужна не для красоты: уверенная и неуверенная ошибки — разные болезни.
...
> дисциплина, что в [главе про валидацию](13-validation-and-leakage.md).
...
> ошибки от размера выборки в логарифмических осях часто близка к прямой, $e(n) \approx \alpha n^{-\beta} + e_\infty$,
> и по трём-четырём точкам можно оценить, во сколько раз надо увеличить выборку ради нужного
> прироста. Обычно ответ вроде «в восемь раз ради полутора пунктов» и закрывает вопрос сам собой.
...
см. [валидацию](13-validation-and-leakage.md)), берём вероятность **приписанной** метки и сортируем
...
$$
A \;=\; p_{\text{loc}} \cdot p_{\text{seg}} \cdot \Big( p^{6} \;+\; r \cdot 6\, p^{5}\,(1 - p) \Big)
$$

> 📐 **Разбор формулы.** $p_{\text{loc}}$ — вероятность верно найти поле индекса на скане,
> $p_{\text{seg}}$ — вероятность верно разрезать поле на шесть цифр, $p$ — точность классификатора
> на одной цифре, $r$ — доля случаев с одной ошибочной цифрой, которые чинит справочник индексов.
> Первое слагаемое в скобках, $p^{6}$, — все шесть цифр распознаны верно (цифры считаем
> независимыми). Второе — ровно одна цифра ошибочна: множитель $6$ это число способов выбрать,
> какая именно, $p^{5}$ — остальные пять верны, $(1-p)$ — эта одна неверна; из таких случаев
> справочник спасает долю $r$. Случаи с двумя и более ошибками справочник не чинит — их слишком
> много кандидатов. Множители $p_{\text{loc}}$ и $p_{\text{seg}}$ стоят снаружи, потому что при
> провале любого из этих этапов конверт уходит в ручную обработку независимо от классификатора.
...
> 💬 **На собеседовании.** Спросят: «Что такое ceiling analysis и зачем он нужен?»
> Хороший ответ: способ распределить усилия в многоэтапном пайплайне. По очереди подменяем
> каждый этап идеальным (берём для него ground truth) и меряем сквозную метрику; прирост
> показывает потолок улучшения этого этапа. Смысл в том, что этап может иметь ужасное локальное
> качество и при этом почти не влиять на итог, и наоборот. Обратная процедура, ablative analysis,
> выключает компоненты по одному и показывает, что реально держит систему — часто это оказываются
> эвристики и постобработка, а не модель. Обе процедуры считаются за часы и экономят месяцы.
> Плохой ответ: путать с ablation study из статей, где выключают части архитектуры нейросети —
> это родственная, но другая процедура, про модель, а не про конвейер.
...
(см. [валидацию и утечки](13-validation-and-leakage.md)), потом всё остальное.
...
- [Chung Y., Kraska T. et al. «Slice Finder: Automated Data Slicing for Model Validation» (ICDE, 2019)](https://arxiv.org/abs/1807.06068) —
  как искать проблемные срезы с поправкой на множественную проверку гипотез; продолжение
  раздела 4 для тех, кому дерева по мета-признакам мало.
- [Eyuboglu S. et al. «Domino: Discovering Systematic Errors with Cross-Modal Embeddings» (ICLR, 2022)](https://arxiv.org/abs/2203.14960) —
  slice discovery там, где мета-признаков нет: срезы ищутся в пространстве эмбеддингов
  и описываются текстом. Рабочий инструмент для картинок и текста.
