...
**Предварительно нужно:** [решающие деревья](06-decision-trees.md), [бэггинг](07-bagging-random-forest.md), [оптимизация](../01-math/04-optimization.md), [логистическая регрессия](03-logistic-regression.md)
...
Введём агрегаты листа — дальше они будут держать на себе весь вывод, поэтому закрепим за ними
цвет: $\textcolor{#348FEA}{G_j}$ синий, $\textcolor{#E06A27}{H_j}$ оранжевый. Тот же цвет вы увидите
и в формулах, и в тексте вокруг них.

$$
\textcolor{#348FEA}{G_j} = \sum_{i \in I_j} g_i, \qquad \textcolor{#E06A27}{H_j} = \sum_{i \in I_j} h_i
$$

Тогда для каждого листа получаем независимую квадратичную функцию одной переменной
$\varphi(w_j) = \textcolor{#348FEA}{G_j} w_j + \tfrac12 (\textcolor{#E06A27}{H_j} + \lambda) w_j^2$.
Приравниваем производную к нулю:

$$
\varphi'(w_j) = \textcolor{#348FEA}{G_j} + (\textcolor{#E06A27}{H_j} + \lambda) w_j = 0
\qquad\Longrightarrow\qquad
\boxed{\;w_j^{*} = -\frac{\textcolor{#348FEA}{G_j}}{\textcolor{#E06A27}{H_j} + \lambda}\;}
$$

Прочитайте формулу цветами: в числителе — **сумма градиентов** (куда тянуть лист),
в знаменателе — **сумма гессианов** плюс регуляризация (насколько уверенно тянуть).
Чем больше $\textcolor{#E06A27}{H_j}$, тем осторожнее шаг: гессиан здесь работает как мера
надёжности листа.
...
| Нужны хорошо откалиброванные вероятности «из коробки» | logloss-бустинг с ранней остановкой обычно недокалиброван по краям | калибровка поверх (см. [главу о калибровке](13-imbalance-and-calibration.md)) |
...
с его ordered target statistics (см. [следующую главу](09-boosting-in-practice.md)).
...
<details>
<summary><b>🎯 Вопрос.</b> Почему в бустинге деревья мельче, чем в случайном лесе?</summary>

**Короткий ответ.** Потому что методы борются с разными компонентами ошибки. Лес усредняет
глубокие деревья с низким смещением и высокой дисперсией — усреднение гасит дисперсию.
Бустинг складывает мелкие деревья с высоким смещением и низкой дисперсией — последовательное
сложение гасит смещение.

**Развёрнуто.** В лесу каждое дерево должно быть максимально выразительным: чем ниже его
смещение, тем ниже смещение ансамбля (усреднение смещение не уменьшает). Дисперсию за нас
уберёт усреднение по $B$ деревьям, поэтому глубину не ограничивают. В бустинге наоборот:
дисперсия ансамбля накапливается с числом слагаемых, а смещение уменьшается по построению.
Поэтому базовая модель должна быть слабой — типичная глубина 3–8 против неограниченной у леса.
Практическое следствие: увеличение `max_depth` в бустинге почти всегда требует одновременного
уменьшения `learning_rate` или числа итераций, иначе модель переобучится.

**Чего ждёт интервьюер:** связки «лес → дисперсия, бустинг → смещение» с объяснением,
почему из неё следует разная глубина.

**Провал:** «в бустинге деревья мельче, чтобы быстрее считалось».
</details>

<details>
<summary><b>🧠 Вопрос.</b> Что произойдёт, если из обученного случайного леса удалить одно дерево? А из бустинга — первое?</summary>

**Короткий ответ.** Из леса — почти ничего: предсказание изменится на величину порядка $1/B$.
Из бустинга удаление первого дерева ломает модель: все последующие деревья обучались как
поправки именно к нему, и без него сумма перестаёт быть осмысленной.

**Развёрнуто.** Это самый быстрый способ проверить, понимает ли человек разницу между
усреднением и последовательным исправлением. В лесу деревья **обменяемы**: они обучались
независимо на бутстрап-выборках, и порядок не имеет значения. Удалили одно из 500 — предсказание
сдвинулось на 0.2%.

В бустинге деревья образуют **цепочку зависимостей**. Дерево $f_2$ обучалось на псевдоостатках,
посчитанных при текущем предсказании $F_1 = F_0 + \nu f_1$. Уберите $f_1$ — и все $f_2, \dots, f_M$
окажутся поправками к состоянию, которого больше нет. Ошибка при этом не «уменьшится на одну
$M$-ю»: она будет катастрофической, потому что смещено само начало отсчёта.

Отсюда же следует, что удаление **последних** деревьев бустинга безопасно — это ровно то, что
делает ранняя остановка, и поэтому библиотеки умеют предсказывать с `ntree_limit` / `num_iteration`.
А вот выкинуть дерево из середины нельзя.

**Чего ждёт интервьюер:** понимания, что в бустинге модели не обменяемы, а в лесе обменяемы.

**Провал:** «качество немного упадёт в обоих случаях».
</details>
...
⬅️ [Бэггинг и случайный лес](07-bagging-random-forest.md) | 🏠 [Оглавление](../index.md) | ➡️ [XGBoost, LightGBM, CatBoost](09-boosting-in-practice.md)
