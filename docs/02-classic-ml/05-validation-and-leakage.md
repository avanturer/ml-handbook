...
**Как проработать:** чтение + воспроизведение экспериментов с ликами
...
# На этом лёгком датасете (n=569, AUC ~0.996) смещение тонет в шуме и может выйти
# даже слегка отрицательным — потолок достигнут, и подбирать нечего.
# Прогоните тот же код на n=200 с сеткой из 50+ конфигураций: смещение станет
# устойчиво положительным и дойдёт до сотых. Это и есть задача 6 из раздела 13.
```
...
> 📐 **Разбор формулы.** Медленно. Дисперсия суммы — это сумма всех элементов ковариационной
> матрицы: $\operatorname{Var}(\sum_k \hat R_k) = \sum_k\sum_l \operatorname{Cov}(\hat R_k, \hat R_l)$.
> На диагонали стоят $K$ штук $\sigma^2$. Вне диагонали — $K(K-1)$ штук $\rho\sigma^2$
> (по определению корреляции $\operatorname{Cov} = \rho\sigma^2$ при одинаковых дисперсиях).
> Итого $\operatorname{Var}(\sum_k \hat R_k) = K\sigma^2 + K(K-1)\rho\sigma^2 = K\sigma^2(1 + (K-1)\rho)$.
> Делим на $K^2$, потому что среднее — это сумма, делённая на $K$, а дисперсия при делении на
> константу делится на её квадрат. Получаем $\frac{\sigma^2}{K}(1 + (K-1)\rho)$.
> Смысл множителя $(1 + (K-1)\rho)$: он показывает, во сколько раз усреднение по $K$ **зависимым**
> величинам хуже усреднения по $K$ независимым. При $\rho = 1$ (все фолды дают одно и то же)
> множитель равен $K$, дисперсия среднего равна $\sigma^2$, и добавление фолдов не даёт вообще
> ничего — вы $K$ раз померили одно и то же.
...
**Развёрнуто.** $\operatorname{Var}(A - B) = \operatorname{Var}(A) + \operatorname{Var}(B) - 2\operatorname{Cov}(A, B)$. Когда обе модели ошибаются на одних и тех же трудных объектах,
