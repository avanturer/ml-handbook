...
**Время на проработку:** ~5 часов (чтение + вывод EM руками + эксперименты)
...
| average | $\frac{1}{|A||B|}\sum_{a,b}\rho(a,b)$ | компромисс между single и complete |
...
$\log\mathcal{N}(x\mid\mu_k,\Sigma_k) = -\tfrac12(x-\mu_k)^\top\Sigma_k^{-1}(x-\mu_k)
- \tfrac12\log\det\Sigma_k - \tfrac{d}{2}\log 2\pi$. Дифференцируем $Q$ по $\mu_k$
...
всегда, итоговое $\log p(X\mid\theta^{\text{new}}) \ge \mathcal{F}(q,\theta^{\text{new}}) \ge
\mathcal{F}(q,\theta^{\text{old}}) = \log p(X\mid\theta^{\text{old}})$. Правдоподобие не убывает
...
**Короткий ответ.** E-шаг: $\gamma_{ik} = \frac{\pi_k\mathcal{N}(x_i|\mu_k,\Sigma_k)}
{\sum_l \pi_l\mathcal{N}(x_i|\mu_l,\Sigma_l)}$. M-шаг: $N_k = \sum_i\gamma_{ik}$,
...
получаем $\lambda=-n$, откуда $\pi_k = N_k/n$. Монотонность: $\log p(X|\theta) = \mathcal{F}(q,\theta)
+ \mathrm{KL}(q\|p(z|X,\theta))$; E-шаг обнуляет KL, M-шаг не уменьшает ELBO, значит правдоподобие
не убывает.
...
значение при случайной разметке: $\mathrm{ARI}=\frac{\mathrm{RI}-\mathbb{E}[\mathrm{RI}]}
{\max\mathrm{RI}-\mathbb{E}[\mathrm{RI}]}$, откуда $\approx 0$ на случайном разбиении и 1 при
...
Для `make_moons(noise=0.08)` постройте отсортированный k-dist график при `min_samples` $\in
\{3, 5, 10, 20\}$. Определите колено визуально, запустите DBSCAN с этим $\varepsilon$ и посчитайте
