...
**Время на проработку:** ~4 часа (чтение + вывод формул + код)
...
- [2. Формализация](#2-формализация) — что такое дерево как функция
...
бустинга](08-boosting-in-practice.md). Для непрерывного признака кандидатов на порог
...
> дисперсией**, потому что дисперсию убьёт усреднение (см. [следующую главу](06-bagging-random-forest.md)).
> В бустинге, наоборот, деревья намеренно мелкие — там регуляризация зашита в shrinkage
> и в штраф за лист $\gamma$ (см. [главу 7](07-gradient-boosting.md), §5.4). Cost-complexity
...
| Нужны точные вероятности | доли классов в листе грубы: в листе из 5 объектов возможны только значения 0, 0.2, …, 1 | ансамбли + калибровка ([глава 12](12-imbalance-and-calibration.md)) |
...
Разбор всех видов утечек — в [главе 13](13-validation-and-leakage.md).
...
([глава 12](12-imbalance-and-calibration.md)).
...
Это ровно тот эксперимент, который превращает [следующую главу](06-bagging-random-forest.md)
...
⬅️ [Метрики качества](04-metrics.md) | 🏠 [Оглавление](../index.md) | ➡️ [Бэггинг и случайный лес](06-bagging-random-forest.md)
