...
**Время на проработку:** ~5 часов (чтение + вывод двойственной задачи руками + численные эксперименты)
...
flowchart LR
    ...
    A --> D["Naive Bayes<br/>модель p(x|y)<br/>генеративный, параметрический"]
```
...
Второе: $\sum_i \alpha_i y_i w^\top x_i = w^\top \sum_i \alpha_i y_i x_i = w^\top w = \|w\|^2
...
13% от расстояния до ближайшего: точка на расстоянии 12.08 и точка на расстоянии 13.73 практически
...
- **ANN — приближённый поиск** (approximate nearest neighbours): HNSW, IVF-PQ, ScaNN. Отказываемся
  ...
  подробный разбор — в [главе про ANN-поиск](../06-recsys/06-two-tower-and-ann.md).
...
слагаемые с $\xi$ и с $b$ обнуляются, квадратичная часть даёт $-\tfrac12\sum_{i,j}\alpha_i\alpha_j
y_iy_j x_i^\top x_j$, линейная — $\sum_i\alpha_i$. Задача выпуклая с линейными ограничениями,
...
становится нулевой при любых других признаках. Сглаживание $\hat\theta_{kj} = (N_{kj}+\alpha)/
(N_k + \alpha V)$ — это MAP-оценка с сопряжённым априорным распределением Дирихле с параметром
...
Проделайте вывод двойственной задачи из §4 самостоятельно, не подглядывая. Затем ответьте:
...
Повторите эксперимент из §8.2, но для трёх распределений: равномерного в кубе, гауссова и
...
- **Beyer K. et al. «When Is "Nearest Neighbor" Meaningful?», ICDT 1999** — работа, в которой
  доказана теорема о концентрации расстояний из §8. Стоит прочитать хотя бы формулировку теоремы
