...
**Время на проработку:** ~5 часов (чтение + прогон протоколов тюнинга на своих данных)
...
# naive TE:   train AUC заметно выше 0.5, test AUC ~0.5 — модель выучила утечку
# ordered TS: обе величины около 0.5 — признак честно признан бесполезным
```

Это тот случай, когда стоит запустить код руками: разрыв train/test на naive TE — ровно та картина,
которую вы увидите на проде, если посчитаете кодировку не в том месте пайплайна.
