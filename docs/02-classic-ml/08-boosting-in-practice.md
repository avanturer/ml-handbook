...
$m = 81$ — число конфигураций на уровень. Доля от полного разброса $\text{SS}_{\text{tot}}$:
...
| `num_leaves` | 35,3% |
| `lambda_l2` | 4,2% |
| `feature_fraction` | 3,8% |
| `min_data_in_leaf` | 2,0% |
| взаимодействия (остаток) | 17,1% |

Две оси из пяти дают 73% разброса, три оставшиеся — 10% на всех. Это и есть «значимых измерений
...
На нашем стенде она выдала `num_leaves` 0,39, `feature_fraction` 0,33, `min_data_in_leaf` 0,22
и `learning_rate` всего 0,07 — при том, что точное дисперсионное разложение по полной сетке
из [§7.1](#71-сетка-против-случайного-поиска) отдало `learning_rate` 37,6% и назвало
`feature_fraction` практически незначимым. Противоречия здесь нет: TPE уже сузил learning rate
до хорошей зоны, поэтому среди **его** испытаний этот параметр почти не варьируется и объясняет
мало разброса. Такая важность отвечает на вопрос «что ещё различает конфигурации, которые
