...
**Уровень:** 🌱 база → 🧠 middle+
**Предварительно нужно:** [решающие деревья](06-decision-trees.md), [постановка задачи обучения](01-learning-theory.md) (bias-variance), [теория вероятностей](../01-math/02-probability.md)
**Как проработать:** чтение + вывод + эксперименты
...
В [предыдущей главе](06-decision-trees.md) мы разобрали, почему решающее дерево — модель
...
2. **Временная структура.** OOB перемешивает прошлое и будущее. Для задач с дрифтом
   ...
   ([глава 13](05-validation-and-leakage.md)).
...
| Много шумовых признаков при разреженном сигнале | `max_features` слишком часто не даёт узлу ни одного полезного признака (см. врезку в [§5](#5-случайный-лес-декорреляция-через-max_features)) |
...
Причина: полностью выращенные деревья, порядка $2n$ узлов на дерево (см. [§8](#8-гиперпараметры-на-практике)).
...
([глава 12](13-imbalance-and-calibration.md)). Для ранжирования (ROC-AUC) это не нужно —
порядок сохраняется.
...
- [Breiman L. «Random Forests», Machine Learning 45(1), 2001](https://link.springer.com/article/10.1023/A:1010933404324) —
  ...
  формальный аналог формулы из [§4](#4-дисперсия-среднего-коррелированных-величин).
...
⬅️ [Решающие деревья](06-decision-trees.md) | 🏠 [Оглавление](../index.md) | ➡️ [Градиентный бустинг](08-gradient-boosting.md)
