...
Всё содержание главы упирается в один вопрос: **на каких парах $(z_i, y_i)$ обучать $g$?**
...
for tag, models in groups.items():
    Z_oof = np.column_stack([cross_val_predict(m, Xtr, ytr, cv=cv, method="predict_proba")[:, 1]
                             for m in models])
    Z_te = np.column_stack([m.fit(Xtr, ytr).predict_proba(Xte)[:, 1] for m in models])
    err = Z_oof - ytr[:, None]                      # корреляция ошибок, а не предсказаний
    C = np.corrcoef(err.T)
    print(f"\n{tag}")
    print(f"  средняя парная корреляция ошибок: {C[np.triu_indices(3, k=1)].mean():.3f}")
    print(f"  AUC отдельных моделей на тесте:   "
          f"{[round(roc_auc_score(yte, Z_te[:, j]), 4) for j in range(3)]}")
    print(f"  простое усреднение, AUC на тесте: {roc_auc_score(yte, Z_te.mean(axis=1)):.4f}")
    meta = LogisticRegression(max_iter=1000).fit(Z_oof, ytr)
    print(f"  OOF-стекинг, AUC на тесте:        "
          f"{roc_auc_score(yte, meta.predict_proba(Z_te)[:, 1]):.4f}")
```
...
Конструкцию можно повторить: мета-признаки первого уровня подать на второй, его выходы — на
третий. Так делали победители Netflix Prize (BellKor's Pragmatic Chaos, 2009): финальное решение,
...
Первое, что считают неправильно: думают, что модели можно запустить параллельно и получить
$\max_m L_m$ вместо $\sum_m L_m$, где $L_m$ — латентность $m$-й модели. Так бывает не всегда.
Если базовым моделям нужны разные признаки, часть которых считается по цепочке (агрегаты из
...
Если стандартное отклонение дельты сравнимо с самой дельтой — прироста нет.
...
**Провал:** «стекинг — это усреднение нескольких моделей». Усреднение — это блендинг с
фиксированными весами, и мета-модель там не обучается вообще.
</details>
...
**Развёрнуто.** Практических следствия два. Добавить в библиотеку слабый алгоритм почти
...
- **van der Laan M., Polley E., Hubbard A. «Super Learner» (Statistical Applications in Genetics
  and Molecular Biology, 2007)** — единственное строгое обоснование стекинга; читать ради
