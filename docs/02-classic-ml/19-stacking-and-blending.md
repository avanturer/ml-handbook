...
**Предварительно нужно:** [бэггинг и случайный лес](07-bagging-random-forest.md), [градиентный бустинг](08-gradient-boosting.md), [валидация и утечки](05-validation-and-leakage.md), [постановка задачи обучения](01-learning-theory.md)
**Как проработать:** чтение + запуск кода из разделов 3, 5 и 7 + разбор компромиссов
...
задано заранее и ничему не учится**. Лес усредняет $B$ деревьев с одинаковыми весами $1/B$. Бустинг
...
есть жёсткий дефект: оно даёт логрегу с AUC 0.895 такой же вес, как бустингу, который на пять
пунктов сильнее, и тянет результат вниз. В разделе 7 это видно на числах: усреднение тройки
«бустинг + логрег + kNN» даёт 0.9420 против 0.9439 у одного бустинга — ансамбль хуже участника.
Второй заход — подобрать веса руками по
...
где $a_m^{(D)}(x)$ — предсказание $m$-й базовой модели, обученной на выборке $D$, для объекта $x$,
а $M$ — число базовых алгоритмов, оно же длина вектора $z(x)$.

Над этим вектором обучаем **мета-модель** $g:\mathbb{R}^M \to \mathbb{R}$. Итоговое
предсказание — $F(x) = g(z(x))$.
...
Что конкретно кладут в $z$ для классификации: вероятность положительного класса $p$, а лучше —
её логит $\log\frac{p}{1-p}$, потому что линейная мета-модель работает в логит-шкале и не
упирается в насыщение у 0 и 1. Для многоклассовой задачи на каждую модель кладут вероятности
всех классов, кроме одного: полный набор в сумме даёт единицу, то есть линейно зависим, и портит
обусловленность задачи мета-уровня. Отдельный приём: добавить в $z$ несколько
...
попадает в тестовую с точностью до сотых (0.8246 против 0.8319 у 1-NN, 0.9508 против 0.9494
у бустинга) — это и есть честная оценка.
...
и скромный, прирост. Отсюда правило, к которому сводится вся конструкция: мета-признаки строятся
только out-of-fold, а оценку ансамбля берут с внешней выборки, не участвовавшей ни в обучении
базовых моделей, ни в построении OOF.

> 💬 **На собеседовании.** Спросят: «Вы обучили три модели на трейне, сделали `predict` на этом
> же трейне и подали результат логрегу вторым уровнем. Что не так?» Хороший ответ: предсказание
> на объекте, который был в обучении, коррелирует с его собственным ответом — величина этой
> корреляции и есть оптимизм обучающей ошибки. Мета-модель видит признак, у которого в инференсе не будет
...
> чаще держат отдельный тест, который не трогали вообще (см. [валидацию и утечки](05-validation-and-leakage.md)).
...
корреляция обычно 0.8–0.99 (на данных этой главы пять разумных моделей — бустинг, лес,
логрег, kNN-25 и HistGradientBoosting — дают от 0.81 до 0.96). Пространство, в котором работает
мета-модель, — это $M$ почти
...
Что берут конкретно: `LogisticRegression` с L2 и параметром `C` порядка 0.1–1.0 для
классификации, `RidgeCV` для регрессии. Отдельно стоит **неотрицательный МНК**
(NNLS, `scipy.optimize.nnls`): мета-модель ищет предсказание в виде
$F(x) = \sum_{m=1}^{M} \alpha_m z_m(x)$, где $z_m(x)$ — $m$-я компонента вектора мета-признаков
из раздела 2, а $\alpha_m$ — её вес, и на веса накладывается ограничение $\alpha_m \ge 0$.
Брейман в «Stacked Regressions» (1996) показал, что именно это ограничение делает
...
где $f_m(x)$ — предсказание $m$-й модели в точке $x$, $M$ — число моделей в ансамбле,
$\sigma^2 = \mathrm{Var}(f_m(x))$ — дисперсия предсказания одной модели (для всех одинаковая),
$\rho = \mathrm{Corr}(f_m(x), f_{m'}(x))$ при $m \ne m'$ — попарная корреляция предсказаний.
Источник случайности здесь — обучающая выборка и сиды: одна и та же процедура обучения на
разных бутстрап-выборках даёт разные $f_m$.

Эту же формулу мы видели в [главе о бэггинге](07-bagging-random-forest.md). Второе слагаемое
...
где $y$ — истинный ответ в точке $x$, $f_m(x)$ — предсказание $m$-й модели, $\bar f(x)$ — их
среднее. Тождество проверяется раскрытием скобок. Обе суммы справа содержат одно и то же
слагаемое $\frac{1}{M}\sum_m f_m(x)^2$, и при вычитании оно сокращается. Остаётся
$-2y\bar f(x) + y^2 + 2\bar f(x)^2 - \bar f(x)^2$, то есть ровно $(\bar f(x) - y)^2$. Равенство
верно в каждой отдельной точке $x$, а не в среднем по выборке.
...
> 💬 **На собеседовании.** Спросят: «У меня есть пять обученных моделей, все приличные. Какие
> три возьмёте в ансамбль и как выберете?» Хороший ответ: по
...
Теоретическое обоснование стекинга дал при этом не Kaggle и не Netflix, а Super Learner
(van der Laan, Polley, Hubbard, 2007): кросс-валидационный выбор выпуклой комбинации из
библиотеки алгоритмов удовлетворяет oracle-неравенству — риск полученной комбинации не хуже
риска лучшего алгоритма библиотеки с точностью до добавки, которая растёт по числу алгоритмов
лишь логарифмически и убывает с ростом выборки (член порядка $\log M / n$). Отсюда два
практических вывода. Первый: добавление в библиотеку плохой модели почти безвредно, платите вы
только логарифмом от $M$. Второй: гарантия — «не хуже лучшего», а не «сильно лучше». Ожидать от
стекинга скачка качества — неправильное чтение теории, и лидерборды Kaggle это подтверждают:
верхушку таблицы стек двигает на тысячные, а не на сотые.

На Kaggle многоуровневый стекинг тем не менее остался нормой для верхушки: там метрика —
единственная цель, инференс делается один раз офлайн на фиксированном тесте, а разница в
четвёртом знаке решает распределение призов. В проде трёхуровневых стеков практически нет,
и причины ниже.
...
где $T_m$ — время ответа $m$-й модели, $t$ — интересующий нас порог латентности, $M$ — число
моделей в ансамбле; произведение вероятностей правомерно, только если времена ответа независимы
(на общем железе это не так, и хвост будет ещё толще).
...
[learning to rank](../10-recsys/07-learning-to-rank.md)), а не ансамбль равноправных моделей.
...
Ориентир для интуиции: на нашем тесте в 1600 объектов бутстрап по 2000 повторов даёт стандартную
ошибку самой AUC около 0.006 (аналитическая формула Хэнли–Макнейла даёт 0.0057, то же самое) —
это в два с половиной раза больше дельты. Дельта, правда, оценивается точнее, чем каждая AUC по
...
([калибровка](13-imbalance-and-calibration.md)).
...
внутри каждого фолда ([валидация и утечки](05-validation-and-leakage.md)).
...
кросс-валидационный выбор комбинации не хуже лучшего алгоритма библиотеки с точностью до добавки,
логарифмической по числу алгоритмов и убывающей с ростом выборки — порядка $\log M / n$.
...
Запустите код из раздела 3, затем соберите ещё два набора базовых моделей: с
`DecisionTreeClassifier()` без ограничения глубины вместо 1-NN и с
`RandomForestClassifier(random_state=0)` вместо него же. Для каждого набора постройте таблицу
«модель → AUC на своём трейне → OOF → тест → вес в наивной мета-модели» и сравните тест наивного
стекинга с тестом OOF-стекинга.
*Критерий: вы видите, что наивная мета-модель раздаёт вес по разрыву «трейн минус OOF», а не по
тестовому качеству — дерево без ограничения глубины (тест 0.844) получает почти тот же вес, что
и бустинг (тест 0.949), и наивный стекинг проигрывает OOF-версии. И вы можете объяснить, почему
с лесом вместо дерева наивная схема почти не проигрывает: разрыв у леса тоже большой, но сама
модель сильная, поэтому раздутый вес достаётся хорошей модели и вреда почти не приносит.*
...
моделей. *Критерий: на тройке «бустинг + логрег + kNN-25» разница по AUC у вас порядка 0.001
(у меня 0.9495 против 0.9506), и вы можете объяснить, почему на этих данных она мала и когда
была бы большой.*
...
*Критерий: для обеих троек у вас есть пара чисел «средняя корреляция ошибок — тестовый AUC», и
декоррелированная тройка не хуже топ-3 при заметно меньшей корреляции (у меня 0.9489 при
корреляции 0.82 против 0.9486 при 0.94). Отдельно объясните, почему в декоррелированную тройку
попал наивный Байес с OOF AUC 0.837 — модель, которую отбор по метрике выбросил бы первой.*
...
метрики из задачи 3. *Критерий: у вас есть строка формата «+<дельта AUC> ценой ×<во сколько раз
дольше> по времени инференса», посчитанная на ваших замерах, и вы формулируете решение
«везти / не везти» с обоснованием.*
...
- [Документация scikit-learn, раздел про ансамбли](https://scikit-learn.org/stable/modules/ensemble.html) —
  подраздел «Stacked generalization»: как устроены `StackingClassifier`/`StackingRegressor`,
  что именно они делают с `cv`, `passthrough` и переобучением базовых моделей на полном трейне.
...
- **Amatriain X., Basilico J. «Netflix Recommendations: Beyond the 5 Stars» (Netflix Tech Blog,
  2012)** — здесь Netflix объясняет, почему призовой бленд не был внедрён целиком: из призового
  решения в прод уехали матричная факторизация и RBM, остальное не оправдало инженерных затрат.
  Лучший из известных примеров разрыва между офлайн-метрикой и продакшеном.
