...
вероятностью $(1-1/n)^n$. Логарифмируем: $n\ln(1-1/n) = n(-1/n - 1/(2n^2) - \dots)
= -1 - 1/(2n) - \dots \to -1$, значит, предел равен $e^{-1} \approx 0.368$, а доля
уникальных — $0.632$. Сходимость очень быстрая: уже при $n = 100$ это 0.634.
Альтернативный вывод: число попаданий объекта — $\mathrm{Bin}(n, 1/n) \to
\mathrm{Poisson}(1)$, и $\Pr[0] = e^{-1}$. Практическое следствие: оставшиеся 36.8% —
...
**Развёрнуто.** $\mathrm{Var}[\frac1B\sum_b f_b] = \frac{1}{B^2}(\sum_b \mathrm{Var}[f_b]
