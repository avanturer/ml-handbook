...
**Предварительно нужно:** [офлайн-метрики рекомендаций](02-metrics-offline.md), [дизайн эксперимента](../06-ab-testing/01-experiment-design.md), [статистические критерии](../06-ab-testing/02-statistical-criteria.md), [ловушки A/B](../06-ab-testing/04-pitfalls.md), [рекомендации в продакшене](14-recsys-in-production.md)
**Как проработать:** чтение + расчёт MDE для своих метрик + сборка таблицы офлайн↔онлайн
...
> 🌱 **База.** Обязательного здесь четыре вещи. Три места, где рвётся цепочка «офлайн → прод»
> ([§1](#1-почему-в-рекомендациях-мерить-труднее-чем-в-любой-другой-ml-задаче)): данные собраны
> прошлой версией системы, система влияет на данные следующей, единица рандомизации не совпадает
> с единицей предъявления. Таблица MDE ([§2](#2-какие-метрики-двигают)): чувствительность одного
> и того же теста к кликам и к GMV различается втрое, и именно она, а не вкус, решает, по какой
> метрике вы принимаете решение. Четыре причины, по которым врёт CTR
> ([§3](#3-почему-ctr-обманчив)), начиная с управляемого знаменателя. И чек-лист диагностики
> ([§9](#9-диагностика-офлайн-вырос-онлайн-нет)), в котором «офлайн-метрика плохая» — последняя
> гипотеза, а не первая. Разделы 5, 7 и 8 — интерливинг, долгосрочный холдаут, таблица
> офлайн↔онлайн — каждый со своим предусловием: трафика не хватает на A/B; спрашивают про годовой
> эффект команды; накопилось 15–20 экспериментов. Пока предусловия нет, инструмент не нужен;
> при первом чтении достаточно понимать, на какой вопрос каждый из них отвечает. Дальше идите,
> когда сможете своими словами назвать три причины расхождения офлайна с онлайном и сказать,
> какой эффект ваш тест физически способен уловить.
...
холдаутами ([§7](#7-holdout-группы-и-долгосрочный-эффект)), а не обычными A/B.
...
шумом. Подробный вывод — в [статистических критериях](../06-ab-testing/02-statistical-criteria.md).
...
> постоянные holdout-группы ([§7](#7-holdout-группы-и-долгосрочный-эффект)): это единственный способ увидеть накопленный эффект
...
в [ловушках A/B](../06-ab-testing/04-pitfalls.md). Здесь — то, что специфично для рекомендаций.
...
**Эффект новизны** — см. [§4](#4-краткосрочные-и-долгосрочные-метрики).
...
- **Изоляция признаков** — иначе счётчики популярности, порождённые основной группой,
  протекут в холдаут (см. [§6](#6-что-ломает-ab-именно-в-рекомендациях)) и занизят измеренный эффект.
...
5. **Онлайн ломается по инженерным причинам** — это [§9](#9-диагностика-офлайн-вырос-онлайн-нет).
...
ключ кэша не содержит версию модели ([§9 предыдущей главы](14-recsys-in-production.md#9-кэширование)) и часть трафика обслуживается
старыми результатами; фича-флаг не долетел до части подов; конфиг с порогом не тот.

**Шаг 3. Хватало ли мощности.** Сравните MDE вашей метрики (таблица [§2](#2-какие-метрики-двигают)) с эффектом,
который вы ожидали по регрессии офлайн→онлайн ([§8](#8-связь-офлайн-и-онлайн-как-проверить-числом)). Если MDE = 1,6%, а ожидаемый
...
см. [снижение дисперсии](../06-ab-testing/03-variance-reduction.md)), интерливинг ([§5](#5-интерливинг)),
...
калибрована иначе, и множители бизнес-правил $b_r$ из [§8 предыдущей главы](14-recsys-in-production.md#8-фильтры-дедупликация-и-бизнес-правила) теперь дают
...
логирующей политики, случайный сплит вместо временного. Это [§8](#8-связь-офлайн-и-онлайн-как-проверить-числом).
...
и [бандиты против A/B](../06-ab-testing/06-bandits-vs-ab.md).
...
Причина: дисперсия ratio-метрики посчитана по показам, а не по пользователям ([§3](#3-почему-ctr-обманчив)).
...
(см. [сложные схемы](../06-ab-testing/05-complex-designs.md)).
...
отклонение для 5 ключевых метрик и заполните таблицу из [§2](#2-какие-метрики-двигают) для своих объёмов трафика.
...
Реализуйте функции из [§5](#5-интерливинг), прогоните на двух ранжированиях (например, популярное против
...
Возьмите последний эксперимент, где офлайн вырос, а онлайн нет, и пройдите чек-лист [§9](#9-диагностика-офлайн-вырос-онлайн-нет)
...
- **Kohavi, Tang, Xu. «Trustworthy Online Controlled Experiments» (Cambridge University Press, 2020)** —
  ...
  эффекты продолжают [§4](#4-краткосрочные-и-долгосрочные-метрики), [§6](#6-что-ломает-ab-именно-в-рекомендациях) и [§7](#7-holdout-группы-и-долгосрочный-эффект).
...
- **Deng, Xu, Kohavi, Walker. «Improving the Sensitivity of Online Controlled Experiments
  ...
  прямое продолжение [§9](#9-диагностика-офлайн-вырос-онлайн-нет), шаг «не хватило мощности». См. также
