...
> 🌱 **База.** Несущих идей здесь три. Первая — одна причина под всеми симптомами
> ([§1](#1-одна-причина-много-симптомов)): система видит отклик только на то, что показала сама,
> поэтому обучающая выборка описывает предыдущую версию модели, а не мир. Вторая — холодный
> старт бывает трёх видов, и айтемный устроен иначе, чем пользовательский: сам он не рассасывается,
> нужны и признаки, и принудительный трафик ([§2](#2-три-холодных-старта--три-разные-задачи),
> [§4](#4-холодный-старт-айтема)). Третья — петля обратной связи как процесс во времени
> ([§9](#9-петля-обратной-связи-и-вырождение-выдачи)) вместе с выводом из симуляции: при
> вырождении CTR не падает, а растёт, поэтому петлю ловят метриками экспозиции — Джини,
> покрытием каталога, усилением относительно входных данных
> ([§6](#6-popularity-bias-механизм-и-как-его-измерить)). Разделы 3 и 5 — набор приёмов
> для нового пользователя и для запуска с нуля, разделы 7 и 10 — математика непредвзятой оценки
> (propensity, IPS, doubly robust); их читают, когда действительно строят off-policy оценку,
> а при первом чтении достаточно запомнить, что вероятность показа берётся только из интервенции
> и без неё никакая коррекция не считается. Дальше идите, когда сможете своими словами объяснить,
> почему система без эксплорации со временем показывает всё меньшую часть каталога и почему
> дашборд качества этого не покажет.
