...
> Порядок «температура, затем коррекция» соответствует выводу из [§5.2](#52-формально-sampled-softmax), где $s$ — это уже итоговый
> логит.
...
> из [§5](#5-logq-коррекция-вывод) осязаемой.
...
Расширьте код из [§10.3](#103-как-считается-расстояние-по-кодам): добавьте кодирование остатков относительно центров k-means (IVF-PQ)
