...
> 🌱 **База.** Обязательных вещей здесь четыре. Разделение на три контура — офлайн,
> near-real-time, онлайн ([§2](#2-полная-архитектура-сервиса)): что пересчитывается ночью,
> что за секунды, что за миллисекунды; из этого разделения выводится вся остальная
> архитектура. Бюджет латентности по стадиям и правило «дедлайн, а не фиксированный таймаут»
> ([§3](#3-бюджет-латентности-по-стадиям)): бюджет решает, какая модель у вас возможна,
> а не наоборот. Логирование вектора признаков в момент предсказания
> ([§5](#5-feature-store-онлайн-офлайн-и-point-in-time)) — одним приёмом закрываются
> train/serve skew и point-in-time. Лестница деградаций
> ([§10](#10-фолбэки-и-лестница-деградаций)): сервис рекомендаций не отвечает ошибкой,
> и что показать вместо персонализации, решено заранее. Разделы 6–9 и 10a–11 — граница
> предподсчёта, пересборка индекса, фильтры, кэш, GPU, расчёт ёмкости — это арифметика
> под конкретную систему; она понадобится, когда вы будете эту систему строить или защищать
> на System Design, и от переноса на второй проход ничего не ломается. Дальше идите, когда
> сможете своими словами сказать, куда уходят 200 мс и что увидит пользователь, если
> хранилище признаков недоступно.
...
Цена — объём логов (см. [§2](#2-полная-архитектура-сервиса)). На практике это единственный способ, который держится
...
## 10a. Когда рекомендациям нужен GPU (и когда нет)

Отдельный вопрос, который всплывает, как только в ранжирование заезжает нейросеть:
**на чём считать инференс**. Ответ не «GPU быстрее», а расчёт.

Ключевое отличие рекомендаций от, скажем, генерации текста: здесь на один запрос приходится
**батч из сотен кандидатов**, которые надо прогнать через одну и ту же модель. Это идеальная
форма нагрузки для GPU — но только если батч действительно собирается.

**Когда GPU не нужен.** Градиентный бустинг на табличных признаках, линейные модели,
факторизации — всё это на CPU считается за единицы миллисекунд на сотню кандидатов.
Перенос на GPU здесь не даёт ничего, кроме расходов и лишней точки отказа.

**Когда GPU оправдан.** Нейронный ранкер с эмбеддингами и вниманием к истории
(см. [нейронное ранжирование](08-neural-ranking.md)) при сотнях кандидатов на запрос;
двухбашенка, если пользовательская башня считается онлайн; трансформер над историей.
Здесь разница между CPU и GPU — не проценты, а разы.

**Что решает дело — динамический батчинг.** GPU простаивает, если запросы приходят по одному.
Сервер инференса должен собирать запросы из разных пользователей в общий батч с окном
ожидания в единицы миллисекунд. Компромисс прямой: окно 5 мс добавляет 5 мс к латентности
каждого запроса, но может поднять пропускную способность в несколько раз. Считать надо от
бюджета: если на ранжирование выделено 60 мс, отдать 5 из них за кратный рост throughput —
обычно выгодная сделка.

| Что смотрим | CPU | GPU |
|---|---|---|
| Бустинг, 100 кандидатов, 50 признаков | единицы мс | смысла нет |
| Нейроранкер, 500 кандидатов | десятки мс, плохо масштабируется | единицы мс при батчинге |
| Утилизация | растёт линейно с подами | падает без динамического батчинга |
| Стоимость простоя | низкая | высокая — карта дорогая и оплачивается всегда |
| Отказоустойчивость | проще: подов много | нужен фолбэк на CPU-модель |

> 💬 **На собеседовании.** Спросят: «Вы поставили нейросеть в ранжирование — как считаете?»
> Хороший ответ начинается не с «на GPU», а с трёх чисел: сколько кандидатов на запрос,
> какой бюджет латентности у стадии и какой пиковый RPS. Из них следует, собирается ли батч
> достаточного размера, чтобы GPU окупился. Дальше — обязательно про фолбэк: если GPU-сервис
> недоступен, система должна деградировать на CPU-модель попроще, а не отдавать ошибку.
> Провал: «GPU быстрее» без расчёта утилизации.

Подробнее об оптимизации самого инференса — в главе
[оптимизация инференса](../07-mlops/09-inference-optimization.md), про экономику железа —
в [стоимости и ёмкости](../07-mlops/10-cost-and-capacity.md).
...
и минус к p99 из-за fan-out (см. формулу $q^N$ в [§3](#3-бюджет-латентности-по-стадиям)). Практический предел — 4–6 источников;
...
Что делать: квоты по продавцу/бренду в скользящем окне ([§8](#8-фильтры-дедупликация-и-бизнес-правила)), мониторинг концентрации
...
Причина: почти всегда утечка — нарушен point-in-time ([§5](#5-feature-store-онлайн-офлайн-и-point-in-time)) или признак содержит
...
Возьмите систему, которую делаете (или наш стенд), и составьте таблицу из [§3](#3-бюджет-латентности-по-стадиям): стадии,
...
Реализуйте класс `Deadline` из [§3](#3-бюджет-латентности-по-стадиям) и оберните им три стадии учебного сервиса. Проверьте
...
и as-of join'ом из [§5](#5-feature-store-онлайн-офлайн-и-point-in-time). Обучите одну и ту же модель на обоих и сравните офлайн-метрику.
...
Поднимите сервис локально с Redis в docker-compose, запустите k6 из [§11](#11-нагрузочное-тестирование-и-ёмкость) на 300 RPS
...
$N_{\text{pods}}$ по формуле из [§11](#11-нагрузочное-тестирование-и-ёмкость) и проверьте нагрузочным тестом, найдя точку излома.
...
- **Chip Huyen. «Designing Machine Learning Systems» (O'Reilly, 2022)** — главы про feature
  engineering, батч/онлайн-инференс и мониторинг закрывают [§4](#4-признаки-три-горизонта-свежести)–6 этой главы на более общем
...
- [Google. «Rules of Machine Learning»](https://developers.google.com/machine-learning/guides/rules-of-ml) —
  ...
  это ровно [§5](#5-feature-store-онлайн-офлайн-и-point-in-time).
- [Malkov, Yashunin. «Efficient and robust approximate nearest neighbor search using HNSW» (arXiv:1603.09320)](https://arxiv.org/abs/1603.09320) —
  ...
  на память и recall, из которого выведена оценка размера индекса в [§7](#7-обновление-моделей-эмбеддингов-и-ann-индекса).
...
- [Faiss wiki](https://github.com/facebookresearch/faiss/wiki) — практические рецепты выбора
  типа индекса под объём и память; полезно вместе с [§7](#7-обновление-моделей-эмбеддингов-и-ann-индекса) при выборе между HNSW и IVF-PQ.
- [Документация k6](https://k6.io/docs/) — сценарии `ramping-arrival-rate` и пороги,
  на которых построен [§11](#11-нагрузочное-тестирование-и-ёмкость).
