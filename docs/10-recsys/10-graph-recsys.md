...
смежности из [§2](#2-формализация-биграф-useritem) (в оригинале — с добавленными петлями, $\tilde{A} = \hat{D}^{-1/2}(A+I)\hat{D}^{-1/2}$).

Здесь три операции подряд: **распространение** ($\tilde{A}H$), **преобразование признаков**
($\cdot\,W$) и **нелинейность** ($\sigma$). Запомните это разделение — весь сюжет [§7](#7-lightgcn-что-убрали-и-почему-стало-лучше) про то,
...
(тогда $H^{(0)}$ — проекция контента), или индуктивной схемой из [§5](#5-graphsage-и-сэмплирование-соседей).
...
К этому вернёмся в [§7](#7-lightgcn-что-убрали-и-почему-стало-лучше).
...
(расчёт — в [§9](#9-масштабирование-считаем-бюджет)). Нужна mini-batch схема.
...
> для большей глубины берут другие схемы (Cluster-GCN, GraphSAINT) или линейные модели ([§7](#7-lightgcn-что-убрали-и-почему-стало-лучше)).
...
смежность из [§2](#2-формализация-биграф-useritem), $E^{(0)}$ — единственные обучаемые параметры модели.
...
**3. Комбинация слоёв решает over-smoothing ([§8](#8-over-smoothing)) и даёт self-connection.** Слагаемое $k=0$
...
3. **Сэмплирование соседей** (GraphSAGE/PinSage) — когда у вершин есть признаки и нужна
   индуктивность. Помните про взрыв окрестности из [§5](#5-graphsage-и-сэмплирование-соседей).
...
из окрестности сразу, без переобучения ([§5](#5-graphsage-и-сэмплирование-соседей)). Для каталога, где ежедневно появляются десятки
...
> 🧠 **Middle+.** Часть выигрыша графовых моделей в статьях —
...
Что делать: `clamp(min=1)` на степенях (как в коде [§7](#7-lightgcn-что-убрали-и-почему-стало-лучше)) либо выкинуть изолированные вершины
...
плюс over-smoothing ([§8](#8-over-smoothing)).
...
Если новых айтемов много — это аргумент за индуктивную схему ([§5](#5-graphsage-и-сэмплирование-соседей)), а не за более частый пересчёт.
...
в тест), реализуйте код из [§7](#7-lightgcn-что-убрали-и-почему-стало-лучше) и обучите с $K \in \{0,1,2,3,4,6\}$.
...
проигрывает меньше 3% — это ваш ответ на вопрос из [§10](#10-когда-графы-дают-прирост-а-когда-это-оверинжиниринг).*
