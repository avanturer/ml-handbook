...
**Как проработать:** чтение + написать runbook для своего сервиса + учения
...
1. **Подтвердить (30 секунд).** Проблема реальна или сломался мониторинг? Быстрая
   ...
   от сломанного экспортёра — очень частый случай (см. [§13 главы 05](05-observability-stack.md#13-экономика-логирования-и-семплирование)).
...
3. **Остановить ущерб (2–5 минут).** Откат, фолбэк, отключение фичи — из [§4](#4-быстрые-действия-и-порядок-их-применения).
...
- фолбэк на урезанный набор фич стал регулярно проверяемым: раз в неделю
  автоматически включается на 1% трафика на 10 минут (см. [§10](#10-runbook-как-артефакт) про учения).
...
3. **Неправильная валидация** — сплит не учитывает время или группы, и офлайн-оценка
   завышена (см. [валидацию и утечки](../02-classic-ml/05-validation-and-leakage.md)).
...
- обучающая выборка собирается **из логов предсказаний**, а не из витрин: в логе
  записан ровно тот вектор, который ушёл в модель (см. [§11 главы 05](05-observability-stack.md#11-логирование-фич-и-предсказаний)).
...
`DegradedResponsesHigh`) и напишите runbook по шаблону [§10](#10-runbook-как-артефакт) для своего сервиса:
...
переводит все категории в неизвестные. Второй проходит дерево диагностики [§3](#3-дерево-диагностики),
...
Возьмите любой из сценариев [§5](#5-сценарий-1-сломался-апстрим-источник-фич)–[§9](#9-сценарий-5-утечка-в-обучении-обнаруженная-в-проде) и напишите по нему полный постмортем по шаблону [§11](#11-постмортем-без-обвинений):
...
Возьмите последние 5 инцидентов (или сценарии [§5](#5-сценарий-1-сломался-апстрим-источник-фич)–[§9](#9-сценарий-5-утечка-в-обучении-обнаруженная-в-проде)). Для каждого ответьте:
...
- [Google. «Rules of Machine Learning»](https://developers.google.com/machine-learning/guides/rules-of-ml) —
  ...
  напрямую относятся к сценариям [§6](#6-сценарий-2-выкатили-модель-с-другой-версией-препроцессинга) и [§9](#9-сценарий-5-утечка-в-обучении-обнаруженная-в-проде).
...
- [Документация Prometheus по правилам алертов](https://prometheus.io/docs/prometheus/latest/configuration/alerting_rules/)
  ...
  разделы про `for`, silence и inhibit-правила, на которые опираются [§10](#10-runbook-как-артефакт) и [§13](#13-подводные-камни).
