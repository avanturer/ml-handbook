...
**Предварительно нужно:** [что мониторить в ML-системе](01-what-to-monitor.md), [деградация модели](04-model-degradation.md), [дисбаланс и калибровка](../02-classic-ml/13-imbalance-and-calibration.md), [разбор ошибок модели](../02-classic-ml/20-error-analysis.md)
...
[холодный старт и смещения](../10-recsys/12-cold-start-and-bias.md), и лекарство там то же —
...
Недооценённый источник: на собеседованиях о нём спрашивают редко, а правит он больше, чем любая
настройка порога. Настоящая цель редко наблюдаема, поэтому берут прокси — и всё смещение прокси
...
задаче. Масштаб правки виден по тому же исследованию: если тем же алгоритмом предсказывать не
расходы, а число активных хронических состояний, доля чернокожих пациентов среди автоматически
отобранных в программу вырастает с 17.7% до 46.5%. Порогом такого не добиться — там меняется
не граница, а то, что вообще измеряется.
...
оказалась мошеннической). $S \in [0, 1]$ — скор модели. $\hat Y = \mathbf{1}[S \ge t]$ — решение
после порога $t$, где $\mathbf{1}[\cdot]$ — индикатор: единица, если условие в скобках выполнено,
и ноль иначе. $A$ — групповой признак со значениями $a$ и $b$.
...
матрицу ошибок в долях. На $N$ объектах группы положительных $Np$, отрицательных $N(1 - p)$.
Обозначим $TP$ — число положительных, которых система поймала, $FP$ — число отрицательных,
которым она всё-таки поставила $\hat Y = 1$. Тогда
...
доля ложных срабатываний. Подставим в определение точности положительных решений; общий
множитель $N$ сокращается:

$$
\mathrm{PPV} \;=\; \frac{TP}{TP + FP} \;=\; \frac{p\,(1 - \mathrm{FNR})}{p\,(1 - \mathrm{FNR}) + (1 - p)\,\mathrm{FPR}}
$$

Дальше три школьных шага, и их стоит проделать глазами, потому что весь вывод держится на них.
Переворачиваем дробь и делим почленно:

$$
\frac{1}{\mathrm{PPV}} \;=\; \frac{p\,(1 - \mathrm{FNR}) + (1 - p)\,\mathrm{FPR}}{p\,(1 - \mathrm{FNR})}
\;=\; 1 \;+\; \frac{(1 - p)\,\mathrm{FPR}}{p\,(1 - \mathrm{FNR})}
$$

вычитаем единицу из обеих частей:

$$
\frac{1 - \mathrm{PPV}}{\mathrm{PPV}} \;=\; \frac{1}{\mathrm{PPV}} - 1 \;=\; \frac{(1 - p)\,\mathrm{FPR}}{p\,(1 - \mathrm{FNR})}
$$

и выражаем долю ложных срабатываний:
...
Читаем. Скор калиброван в обеих группах: столбцы вида `0.39->0.40` показывают, что среди людей
со скором около 0.39 доля положительных равна 0.40. Расхождение в сотых — выборочный шум, по
построению скор в точности равен вероятности. При этом баланс по KMR нарушен и
...
> ⌨️ **Руками.** Вынесите крутизну (`1.5` в формуле) во второй параметр `make_group` и соберите
> третий сценарий: базовые частоты у групп равны, а качество разделения разное — `1.5` против `0.8`.
> Учтите ловушку: одним и тем же интерсептом равные базовые частоты не получить. При крутизне 0.8
> и `intercept` = −0.9 базовая частота выходит 0.312 против 0.347 у первой группы, потому что
> среднее сигмоиды зависит от разброса её аргумента. Подберите интерсепт (для крутизны 0.8 это
> примерно −0.73, что даёт 0.345) и посмотрите на четыре порога.
> *Что получится: 0.437 по демографическому паритету, 0.389 по равным возможностям, 0.482 по
> равному FPR, 0.627 по предсказательному паритету — то есть они не сблизились почти совсем.
> Равенство базовых частот снимает запрет тождества Chouldechova, но не делает критерии
> эквивалентными: один порог физически не может подогнать сразу две величины, а формы
> распределений скора в группах разные.*
...
правила: $|f(x_1) - f(x_2)| \le L \cdot d(x_1, x_2)$, где $f$ — само решающее правило (модель,
выдающая скор), $x_1$ и $x_2$ — два объекта, $d$ — метрика похожести на пространстве объектов,
$L$ — константа Липшица. Читается это так: насколько два человека различаются по существу,
настолько максимум и разрешено разойтись их скорам.
...
[главе о холодном старте и смещениях](../10-recsys/12-cold-start-and-bias.md), и она куда
...
[главе о дисбалансе и калибровке](../02-classic-ml/13-imbalance-and-calibration.md).
...
- **Obermeyer Z., Powers B., Vogeli C., Mullainathan S. «Dissecting racial bias in an algorithm
  ...
  на реальной системе; алгоритмы этого класса охватывают в США порядка 200 млн человек в год.
  Читать ради того, чтобы приучиться обсуждать таргет до модели.
...
⬅️ [Переобучение моделей](07-retraining.md) | 🏠 [Оглавление](../index.md) | ➡️ [Постановка задачи рекомендаций](../10-recsys/01-recsys-foundations.md)
