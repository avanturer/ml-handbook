...
**Время на проработку:** ~5 часов (чтение + бэктест устаревания на своих данных)
...
не двумя моделями, а весами (см. §6) либо явным признаком «плотность истории».
...
размеченных примеров в сутки даёт $n_{\text{eff}} \approx 2.89 \cdot 100\,000 \cdot 30
\approx 8.7$ млн — то есть примерно как невзвешенное окно в 87 дней. Соответствие
...
или за семплирование при логировании (§13 [главы 05](05-observability-stack.md)),
...
| Стабильность: churn относительно текущей модели | < 20% (см. §12) | гонка за шумом |
...
7. **Продвижение на 100% трафика при значимом изменении поведения.** Если churn
   предсказаний высок (§12), даже технически корректная модель заметно меняет
...
Что делать: собирать обучающую выборку из логов предсказаний (§8); проверять
...
Что делать: считать мощность каждой ступени (§10); для сценариев с рыночными
...
триггеры, а снизу — ограничение из §12: не чаще, чем читается эффект предыдущего цикла.
...
На любом датасете с временной меткой постройте кривую по коду §2: 5–8 точек обучения,
...
Прогоните бэктест по коду §5 для окон {30, 60, 90, 180, 365, всё} на 6–10 точках
...
Возьмите код §9 и добавьте недостающие проверки: churn, доля предсказаний в крайних
...
Напишите конфиг по образцу §4 для своей модели: расписание из задачи 2, триггеры
...
- [Документация LightGBM](https://lightgbm.readthedocs.io/) — разделы про `sample_weight`
  и про продолжение обучения (`init_model`): практическая база для §6.
...
- [Документация Great Expectations](https://docs.greatexpectations.io/) — проверки
  данных как блокирующий шаг конвейера переобучения (§8).
- **Google SRE Workbook, глава «Canarying Releases»** — про выдержку, ступени
  ...
  расчёт мощности из §10.

---

⬅️ [Инциденты](06-incidents-and-runbooks.md) | 🏠 [Оглавление](../index.md) | ➡️ [Дизайн эксперимента](../10-ab-testing/01-experiment-design.md)
