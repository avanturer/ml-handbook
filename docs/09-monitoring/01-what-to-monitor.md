...
**Время на проработку:** ~4 часа (чтение + разбор своего сервиса по чек-листу)
...
[дизайне экспериментов](../10-ab-testing/01-experiment-design.md).
...
1. **Из SLO.** Лучший вариант: порог не выдуман, а выведен из обязательства перед
   пользователем (см. burn rate в §3).
...
> очевидно — откат. Пример из §1 (доход в тысячах вместо рублей) ловится именно ей: PSI
...
статического «error rate > 1%» используем скорость сжигания $\text{BR} =
\text{доля плохих} / (1-\text{SLO})$ на нескольких окнах. BR = 14.4 на окнах 1 час и 5 минут
...
и в минутах простоя за 28 дней. Напишите четыре правила Prometheus по схеме из §3.
...
- [Google SRE Workbook — «Alerting on SLOs»](https://sre.google/workbook/alerting-on-slos/) —
  ...
  Отсюда взяты числа 14.4 / 6 / 1 из §3.
