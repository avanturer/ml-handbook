...
**Время на проработку:** ~5 часов (чтение + реализовать PSI и детектор + прогнать на своих данных)
...
- **Ваша собственная модель.** Начали блокировать заявки со скором > 0.8 — и в обучающие
  ...
  [холодном старте и смещениях](../06-recsys/12-cold-start-and-bias.md).
...
и в [ловушках A/B](../10-ab-testing/04-pitfalls.md).
...
(тот же floor, что в PSI), со всеми оговорками из §6 про произвол.
...
**4. Многомерный тест вместо пятисот одномерных.** Детектор-классификатор (§12) даёт один
...
Всё, что про дрифт как таковой, — это тикет в рабочее время. Логика в §17.
...
- Значение зашумлено сезонностью (см. §15).
...
- Изменилось $P(y)$? Если есть свежие метки — считаем напрямую; если нет — BBSE из §4.
...
| Сдвиг приоров | Перекалибровать (формула из §4 или переобучение калибратора на свежих метках). Полное переобучение обычно не нужно |
...
может дрейфовать сколько угодно. Лечится взвешиванием по важности (§13).
...
из §3, и он работает и на практике: если $P(y|X)$ не изменилось, а модель хорошо
...
Это чистый концепт-дрифт (§5) — $P(X)$ не изменилось, изменилось $P(y|X)$. По входным
...
Реализуйте `fit_psi_bins` и `psi` из §6 без подглядывания. Возьмите любой числовой признак
...
Реализуйте `drift_detector_auc` из §12. Проверьте на двух случайных половинах одного
...
где SD считается по формуле из §12; на сдвинутых данных AUC заметно выше, а важности
...
- [D. Lopez-Paz, M. Oquab. «Revisiting Classifier Two-Sample Tests»
  ...
  из §12: почему точность классификатора является состоятельной статистикой критерия.
...
- [Z. Lipton, Y.-X. Wang, A. Smola. «Detecting and Correcting for Label Shift
  ...
  метод BBSE из §4: оценка нового распределения классов без меток. Один из немногих
...
- [Great Expectations — документация](https://docs.greatexpectations.io/) — проверки
  контрактов данных, тот самый «шаг 0» протокола из §16. Дрифт-мониторинг без проверок
