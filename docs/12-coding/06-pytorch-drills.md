...
Пять строк, в каждой из которых есть чем ошибиться. Держать их лучше не списком, а замкнутым
циклом: на схеме те же шаги плюс то, чего в коде не видно, — где граф вычислений строится,
где освобождается и что мешает его отпустить.

```mermaid
flowchart TB
    B["очередной батч на device"] --> Z["optimizer.zero_grad<br/>обнулять ДО backward, иначе градиенты<br/>сложатся с прошлым шагом"]
    Z --> F["forward: logits = model(x)<br/>здесь строится граф вычислений"]
    F --> L["loss = criterion(logits, y)"]
    L --> BW["loss.backward()<br/>заполняет .grad и освобождает граф"]
    BW --> C["clip_grad_norm_<br/>только между backward и step"]
    C --> ST["optimizer.step, затем scheduler.step<br/>порядок обратный не работает"]
    ST --> LOG["лог: total += loss.item()<br/>без .item накопитель удержит<br/>граф каждого шага в памяти"]
    LOG --> Q{"батчи в эпохе<br/>ещё остались?"}
    Q -->|"да"| B
    Q -->|"нет"| EV["model.eval и torch.no_grad<br/>на валидации граф не строится вовсе"]
    EV --> M["model.train перед следующей эпохой<br/>иначе dropout и BN останутся выключенными"]
    M --> B
```

Вывод, ради которого схема: почти все ошибки обучающего цикла — это ошибки **места** в нём,
а не ошибки формул: обнуление после `backward`, `step` до `backward`, забытый `.item()`,
`no_grad` не надет на валидацию, `train()` не возвращён после неё. Дальше — по задачам.
...
Реализуйте MHA из [§5](#5-задача-4-attention-с-нуля) и напишите четыре теста: сверка с `F.scaled_dot_product_attention`,
...
- [Vaswani et al. «Attention Is All You Need» (NeurIPS 2017)](https://arxiv.org/abs/1706.03762) —
  первоисточник attention; сверяйте свою реализацию с формулами из [задачи 4](#5-задача-4-attention-с-нуля).
