...
**Время на проработку:** ~6 часов (чтение ~2 ч + 15 задач на ускорение)
...
длины $d$. Писать цикл по строкам нельзя — см. §1. Дублировать вектор в матрицу $n \times d$
...
отвечает, а ошибка в перестановке даёт молча неверный результат (см. §3).
...
Дисперсию — можно, накапливая $\sum x$ и $\sum x^2$ (но осторожно с точностью, см. §5).
...
**Шаг 5. Сменить инструмент** — DuckDB, Polars или Spark. Об этом в §17.
...
Причина: цепочечное присваивание, см. §16.
...
**Развёрнуто.** Продвинутая индексация возвращает копию, а не view (см. §4), поэтому
...
*Проблема:* боксинг + диспетчеризация + нет SIMD (§1).
*Решение:* `a.sum()`.
...
*Решение:* через $\|x_i - x_j\|^2 = \|x_i\|^2 + \|x_j\|^2 - 2\langle x_i, x_j\rangle$ (§3).
...
для каждой оптимизации, какой именно из механизмов §1 она задействовала.
...
- [pandas: Copy-on-Write](https://pandas.pydata.org/docs/user_guide/copy_on_write.html) —
  главный документ по §16: что изменилось, как мигрировать, какие паттерны сломаются.
...
- [pandas: Scaling to large datasets](https://pandas.pydata.org/docs/user_guide/scale.html) —
  официальная лестница приёмов для §15, включая честный раздел «когда пора уходить из pandas».
...
- **Wes McKinney. «Python for Data Analysis», 3-е изд.** — доступна бесплатно онлайн
  ...
  reshape и временные ряды — самое подробное изложение §10–§14 на русском рынке
  отсутствует, читайте оригинал.
- [DuckDB](https://duckdb.org/) и [Polars](https://pola.rs/) — если после §17 захотелось
