...
> 🌱 **База.** Обязательных вещей три. Первая — [§1](#1-почему-ml-код-ломается-иначе)
> и [§2](#2-что-тестировать-шесть-слоёв): ML-код почти не падает, он молча возвращает
> не те числа, поэтому проверяют в первую очередь данные и контракты между компонентами,
> а не функции; таблица шести слоёв — карта, к которой потом возвращаются. Вторая —
> [§5](#5-тесты-на-данные) и [§6](#6-тесты-на-препроцессинг-и-признаки): то, чего
> в обычном коде не тестируют вообще, — схема, диапазоны, бюджет пропусков,
> бизнес-инварианты, а из признаков — что агрегат не заглядывает в будущее и что расчёт
> по одной строке совпадает с батчевым. Третья — [§7](#7-тесты-на-модель): у `predict`
> нет правильного ответа, поэтому тест на модель устроен иначе — обучаемость
> («переобучись на 32 примерах»), инварианты и направленные ожидания, гейт по метрике
> на замороженном наборе вместо сравнения с эталонным массивом. Разделы 4, 8, 9 и 12–14
> (справочник по pytest, hypothesis, моки, линтеры, сборка CI) — инструментальные:
> их читают с открытым редактором, когда заводят тесты в своём проекте, а не подряд.
> Дальше идите, когда сможете своими словами объяснить, почему тест «предсказание равно
> 0.7» вредный и чем его заменить.
