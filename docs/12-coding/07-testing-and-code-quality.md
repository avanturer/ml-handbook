...
**Время на проработку:** ~6 часов (чтение ~2 ч + написать тесты для своего проекта)
...
из §2, а не по строкам. Полезная рамка — «ML Test Score» (Breck et al., 2017):
...
из §6. Затем **намеренно** внесите утечку (уберите фильтр по `as_of`) и убедитесь,
что тест падает.
...
Пройдите по чек-листу из §2 и §15 для своего текущего проекта и честно отметьте,
