...
**Как проработать:** чтение + вывод DPO руками + реализация лоссов
...
- [11. Дистилляция рассуждения: трейсы → SFT → RL](#11-дистилляция-рассуждения-трейсы--sft--rl) — как превратить дорогой инференс в дешёвую модель
- [12. RLAIF и Constitutional AI](#12-rlaif-и-constitutional-ai)
- [13. Reward hacking](#13-reward-hacking)
- [14. Что выбрать под ресурсы](#14-что-выбрать-под-ресурсы)
...
|---|---|---|---|
| Ручные демонстрации своих экспертов | 1–20 тыс. | 3–15 \$ за пример | медленно, но данные точно про ваш домен | | Логи реальной поддержки/операторов | 10–500 тыс. | почти бесплатно | PII, шум, стиль оператора ≠ стиль ассистента | | Синтетика от сильной модели (distillation) | 50–500 тыс. | 0,1–2 \$ за 1000 примеров | лицензия провайдера, наследование ошибок учителя |
...
адаптеров (см. [§14](#14-что-выбрать-под-ресурсы)).
...
- **Сложно отлаживать.** Метрика (награда RM) растёт, а качество для человека падает — это норма,
  а не исключение (см. [§13](#13-reward-hacking)).
...
Начнём с задачи RLHF из [§8.1](#81-целевая-функция-и-зачем-нужен-kl-штраф). Заметьте, что она разваливается по промптам: для каждого фиксированного
...
Вспомним из [§7.1](#71-модель-bradleyterry): вероятность предпочтения зависит **только от разности наград**:
...
обозначения — как в [§8.2](#82-ppo-поверх-этого).
...
появляется хакинг проверяющей программы (см. [§13](#13-reward-hacking)). Схема «GRPO + верифицируемая награда» — то, на
...
## 11. Дистилляция рассуждения: трейсы → SFT → RL

Предыдущий раздел закончился на том, что «GRPO + верифицируемая награда» даёт рассуждающую модель.
Теперь неприятная арифметика. Такая модель тратит на запрос не 200 выходных токенов, а 2000–8000,
и половина из них — размышления, которых пользователь не увидит. Если вы обслуживаете миллион
запросов в сутки, это не «дороже на инференсе», это другой порядок счёта. Вычисления на инференсе —
чистый OPEX, растущий вместе с трафиком (подробно про эту ось — в главе
[про промптинг](06-prompting-and-structured-output.md#6-test-time-compute-качество-которое-покупают-на-инференсе)).

**Дистилляция рассуждения** превращает этот OPEX обратно в CAPEX: вы один раз платите за обучение
маленькой модели на трейсах большой и дальше обслуживаете трафик в разы дешевле.

### 11.1 Асимметрия, из-за которой это вообще работает

Ключевое наблюдение опубликовала команда DeepSeek вместе с R1 (2025). Они собрали около 800 тысяч
примеров, сгенерированных R1 (примерно 600 тысяч с рассуждением и 200 тысяч без), и дообучили
на них обычным SFT — **без всякого RL** — открытые Qwen и Llama размеров от 1.5B до 70B. Результат
оказался сильнее, чем масштабный RL, запущенный на той же базовой модели напрямую.

Это несимметрия, и она объясняет весь дальнейший конвейер. RL с проверяемой наградой на маленькой
модели работает плохо: сигнал разрежен (одна скалярная награда на всю цепочку из тысяч токенов),
а разведка почти никогда не набредает на длинную верную цепочку самостоятельно — награда 0/1
не даёт никакого градиента, пока модель не решила задачу хотя бы раз. Дистилляция снимает ровно
эту проблему: **маленькой модели дешевле скопировать рассуждение, чем открыть его самой**.

Механизм видно из формулы. Обычный SFT максимизирует правдоподобие эталонного ответа $a$.
При дистилляции рассуждения эталон — пара «цепочка плюс ответ», и по цепному правилу

$$
\log \pi_\theta(r, a \mid x) \;=\; \log \pi_\theta(r \mid x) \;+\; \log \pi_\theta(a \mid r, x)
$$

где $x$ — задача, $r$ — цепочка рассуждения учителя, $a$ — финальный ответ,
$\pi_\theta$ — политика ученика. Второе слагаемое — это то, что нам нужно; первое — то, что даёт
возможность его получить. Обучаясь на цепочке, ученик получает **пошаговый супервизор**: вместо
одного скаляра на весь ответ — поток из тысяч токенов, каждый со своим градиентом. Именно поэтому
на малом бюджете SFT на трейсах выигрывает у RL: плотность обучающего сигнала отличается
на три порядка.

> 🧠 **Middle+.** Важно понимать, что именно переносится. Мы не сжимаем знание — мы переносим
> **траекторию вычисления**. Учитель на инференсе разворачивает рассуждение на 3000 токенов, ученик
> учится разворачивать сопоставимое на 3000 токенов. Стоимость запроса падает не потому, что цепочка
> стала короче, а потому что в $C \approx 2 P \cdot N_{\text{tok}}$ уменьшилось $P$ — число
> параметров. Наивное ожидание «после дистилляции ответы станут лаконичнее» не сбывается:
> длина обычно сохраняется, дешевеет каждый токен. Если нужна ещё и короткая цепочка, длину
> приходится оптимизировать отдельно — класть целевой бюджет в промпт при обучении или резать
> генерацию на уровне декодера.

### 11.2 Конвейер целиком

Порядок «трейсы → SFT → RL» — не случайная последовательность, а то, как построен сам R1.
Полная схема оригинала: несколько тысяч длинных цепочек как «холодный старт» для SFT → RL с GRPO
на проверяемых задачах → **отбор** трейсов у получившейся модели → большой SFT на этом наборе →
финальный RL. Для продуктовой команды из этого выжимается четыре шага.

**Шаг 1. Сбор трейсов.** Сэмплируем у учителя по $k = 4\ldots16$ ответов на задачу при
температуре 0.6–1.0. Одна генерация не годится: нам нужна оценка того, насколько задача трудна
для учителя, а она берётся из доли верных среди $k$.

**Шаг 2. Rejection sampling — центральный шаг, а не гигиена.** Оставляем только трейсы,
у которых финальный ответ прошёл детерминированную проверку (сверка с эталоном, юнит-тесты,
валидация схемы). Приём старый: так устроен STaR (Зеликман с соавторами, 2022), и он же лежит
в основе всех современных конвейеров.

Именно этот шаг определяет, где дистилляция рассуждения работает, а где нет. Математика, код,
извлечение структуры, SQL — работает: есть чем отфильтровать. «Объясни, почему упали продажи» —
не работает: фильтра нет, и вы дистиллируете ошибки наравне с верными решениями. Причём ошибочные
трейсы выглядят так же уверенно, как верные, поэтому ученик выучивает уверенность отдельно
от правильности.

**Шаг 3. Отбор по сложности.** Дальше делают то, что пропускают чаще всего. Если оставить все
верные трейсы, набор перекосится в сторону лёгких задач: на них учитель прав всегда, и они дают
больше всего трейсов. Полезны задачи с промежуточной долей верных ответов — примерно от 0.15
до 0.9. Слишком лёгкие несут мало информации на токен; на слишком трудных верный ответ чаще
получен случайно, а рассуждение к нему не ведёт — такой трейс учит связывать бессвязное.

```python
import numpy as np
from collections import defaultdict

def build_distillation_set(samples, keep_per_problem=2,
                           min_rate=0.15, max_rate=0.90):
    """Отбор трейсов учителя в обучающий набор ученика.

    samples: список (problem_id, trace_id, is_correct, n_tokens).
    Три фильтра, и каждый закрывает свою дыру:
      1. неверные трейсы выкидываем целиком — иначе дистиллируем уверенные ошибки;
      2. задачи, где учитель прав почти всегда, дают мало информации на токен;
      3. задачи, где учитель почти никогда не прав, дают трейсы,
         в которых верный ответ получен случайно, а рассуждение к нему не ведёт.
    """
    by_problem = defaultdict(list)
    for pid, tid, ok, ntok in samples:
        by_problem[pid].append((tid, ok, ntok))

    kept, stats = [], {"слишком лёгкие": 0, "неподъёмные": 0, "взято задач": 0}
    for pid, rows in by_problem.items():
        rate = np.mean([ok for _, ok, _ in rows])
        if rate > max_rate:
            stats["слишком лёгкие"] += 1
            continue
        if rate < min_rate:
            stats["неподъёмные"] += 1
            continue
        correct = [r for r in rows if r[1]]
        # из верных берём самые короткие: при равной правильности короткий трейс
        # содержит меньше блужданий, и ученик копирует меньше лишней формы
        correct.sort(key=lambda r: r[2])
        kept.extend((pid, tid) for tid, _, _ in correct[:keep_per_problem])
        stats["взято задач"] += 1
    return kept, stats
...
n_problems, k = 2000, 8
# сложность задачи: вероятность верного ответа учителя на одном сэмпле
p_correct = rng.beta(1.4, 1.0, size=n_problems)
samples = []
for pid in range(n_problems):
    for tid in range(k):
        ok = bool(rng.random() < p_correct[pid])
        # неверные трейсы систематически длиннее: модель дольше блуждает
        ntok = int(rng.lognormal(mean=7.0 + (0.0 if ok else 0.35), sigma=0.45))
        samples.append((pid, tid, ok, ntok))

kept, stats = build_distillation_set(samples)
print(f"задач всего:          {n_problems}")
for key, value in stats.items():
    print(f"{key:<22}{value}")
print(f"трейсов на входе:     {len(samples)}")
print(f"трейсов в наборе:     {len(kept)}  ({100 * len(kept) / len(samples):.1f}% от собранного)")
naive = sum(1 for _, _, ok, _ in samples if ok)
print(f"для сравнения, «взять все верные»: {naive} трейсов")
```
...
неподъёмные           266
взято задач           1466
трейсов на входе:     16000
трейсов в наборе:     2932  (18.3% от собранного)
для сравнения, «взять все верные»: 9333 трейсов
```

Обратите внимание на пропорцию: из 16 тысяч сгенерированных трейсов в набор попали 2932 —
меньше пятой части. Наивное «взять всё верное» дало бы 9333 трейса, втрое больше объёма и заметно
хуже состав. Планируя бюджет на генерацию у учителя, закладывайте этот коэффициент: чтобы получить
50 тысяч обучающих трейсов, сгенерировать придётся порядка 250–300 тысяч.

**Шаг 4. SFT.** Всё как в [§3](#3-чат-шаблоны-и-маскирование-лосса): чат-шаблон до символа, лосс
маскируется на промпте. Специфика ровно одна и она болезненная — **длина**. Трейсы на 4–16 тысяч
токенов означают, что при обычном батчинге вы упрётесь в память на attention и на активациях.
Лечится упаковкой нескольких коротких примеров в одну последовательность с корректной маской
внимания, градиентным чекпоинтингом и, при совсем длинных трейсах, параллелизмом по
последовательности.

**Шаг 5 (опциональный). RL поверх.** SFT имитирует распределение учителя, и его потолок — учитель.
RL на проверяемой награде обучается на **собственных** сэмплах ученика, снимает экспозиционное
смещение из [§5](#5-почему-sft-упирается-в-потолок) и на проверяемых задачах позволяет ученика
за учителя вывести. Порядок именно такой, а не наоборот: RL после SFT стартует из политики,
которая уже иногда решает задачу, то есть награда не нулевая и градиент существует.

> ⚠️ **Типичная ошибка.** Собрать трейсы одной генерацией на задачу и без проверки ответа
> («учитель же сильный») → в набор попадают уверенные неверные рассуждения, доля которых равна
> доле ошибок учителя, то есть 10–40% на нетривиальных задачах → ученик выучивает форму уверенного
> рассуждения вместе с ошибками и на валидации даёт длинные красивые неверные ответы. Правильно:
> $k$ сэмплов на задачу, жёсткий фильтр по правильности финального ответа, отбор по доле верных.
> Если детерминированной проверки нет вообще — дистиллировать рассуждение, скорее всего, не нужно:
> берите дистилляцию ответов, а не цепочек.

### 11.3 Где это ломается: форма без содержания

Теперь честная часть, ради которой раздел и написан.

Симптом выглядит так. Дистиллированная модель уверенно воспроизводит **риторику** рассуждения:
«Wait, let me double-check», «Hmm, that doesn't look right», разбиение на пронумерованные шаги,
возврат к условию. Цепочка длинная и на вид грамотная. Но качество ответа — как у базовой модели
до дистилляции, а самокоррекция срабатывает не там, где есть ошибка, а там, где в трейсах учителя
чаще стояло соответствующее слово.

Механизм прямо следует из того, что оптимизирует SFT. Функция потерь — правдоподобие токенов,
и градиент больше там, где ошибка предсказания больше в начале обучения и где токен встречается
часто. Стилистические маркеры рассуждения предсказуемы и стоят в типовых позициях — они выучиваются
первыми и почти бесплатно. Содержательные шаги — самое непредсказуемое в трейсе, они требуют
и знаний, и вычислительной глубины, и выучиваются последними и хуже всего. Если обучение
остановить рано, если данных мало или если ученик слишком мал, вы получаете ровно форму.

Три конкретных режима поломки:

**Разрыв в ёмкости.** Трейс от очень большой модели содержит шаги, которые ученик физически
не может выполнить: они опираются на знание, которого у него нет, или на глубину вычисления,
которой не хватает при его числе слоёв. Воспроизвести он может только то, что может, — форму.
В экспериментах DeepSeek выигрышные точки дистилляции R1 лежали в диапазоне 7B–32B; на 1.5B прирост
есть, но качественно это другая модель. Практический ориентир: чем больше разрыв в размере,
тем большую долю прироста съедает копирование формы.

**Задачи вне покрытия ученика.** Если ученик не решает задачу ни разу из 64 сэмплов, трейс учителя
для него — просто внешний текст, который можно только запомнить. Отсюда практика фильтровать
обучающие задачи ещё и по **ученику**, а не только по учителю: оставлять те, где базовый ученик
хотя бы иногда близок к решению.

**Ранняя остановка на длинных трейсах.** Длинные последовательности плюс маленький батч
(потому что память) плюс мало эпох — типичная конфигурация продуктовой команды с восемью GPU.
В ней модель успевает выучить стиль и не успевает выучить содержание.

Как это диагностировать, а не гадать. Два измерения, оба дешёвые:

1. **Работает ли бюджет.** Возьмите 100 задач и прогоните ученика с принудительно разной длиной
   рассуждения (обрыв цепочки на 512 / 2048 / 8192 токенах). У модели с настоящим рассуждением
   качество растёт по бюджету и выходит на плато. У модели, скопировавшей форму, кривая плоская
   с самого начала: лишние токены не несут вычисления. Это самый чистый тест, который у вас есть.
2. **Следует ли ответ из цепочки.** Возьмите 100 верных ответов и 100 неверных, покажите судье
   (или человеку) только цепочку без финального ответа и попросите продолжить. Если по цепочке
   ответ не восстанавливается, цепочка декоративна.

> 💬 **На собеседовании.** Спросят: «Вы дистиллировали рассуждение большой модели в 7B. Ученик
> пишет длинные цепочки, а качество не выросло. Что произошло и что делаете?»
> Хороший ответ — сначала диагностика. Проверяю, несёт ли цепочка вычисление: прогоняю ученика
> с принудительно разной длиной рассуждения; плоская кривая «качество — бюджет» означает, что
> скопирована форма, а не содержание. Дальше три гипотезы по убыванию вероятности. Первая — данные:
> был ли rejection sampling по правильности финального ответа и отбор по доле верных, или в набор
> попали уверенные ошибки учителя. Вторая — покрытие: решает ли базовый ученик эти задачи хотя бы
> иногда; если ни разу из 64 сэмплов, трейс для него внешний текст, и его можно только запомнить.
> Третья — режим обучения: на трейсах в 8 тысяч токенов при маленьком батче модель успевает выучить
> стилистику и не успевает содержание, лечится длительностью и упаковкой последовательностей.
> И системная мера: добавить поверх SFT этап RL с проверяемой наградой — он обучается
> на собственных сэмплах ученика, поэтому декоративное рассуждение перестаёт приносить награду.
> Плохой ответ: «модель маленькая, ничего не поделаешь».

> 💬 **На собеседовании.** Спросят: «Нужна модель, рассуждающая в вашем домене. Бюджет — восемь GPU
> и месяц. Как построите конвейер?»
> Хороший ответ: не начинать с RL. Порядок такой. (1) Собрать 5–50 тысяч задач домена, где ответ
> проверяется программно — это ограничение определяет, возможен ли проект вообще. (2) Сгенерировать
> у сильной модели по 8 трейсов на задачу; отфильтровать по правильности ответа и по доле верных
> в диапазоне примерно 0.15–0.9; заложить, что в набор дойдёт около пятой части сгенерированного.
> (3) SFT на 7B–32B ученике с маскированием лосса на промпте, длинным контекстом и упаковкой.
> (4) Только после этого — GRPO на проверяемой награде: он стартует из политики, которая уже иногда
> решает задачу, и потому даёт ненулевой градиент. (5) Замерить кривую «качество — бюджет
> рассуждения» и зафиксировать рабочий бюджет. Отдельно: проверить лицензию модели-учителя
> на использование её выходов для обучения — это не формальность, а то, на чём проекты
> действительно останавливаются. Плохой ответ: «запущу GRPO на своей 7B» — на маленькой модели
> без холодного старта награда почти всегда нулевая, и обучение не сдвинется.

---

## 12. RLAIF и Constitutional AI
...
Что делать: тест из [§3.2](#32-маскирование-лосса-на-промпте) — доля обучаемых токенов должна быть в разумном диапазоне (обычно 10–40%).
...
<details>
<summary><b>🧠 Вопрос.</b> Почему дистилляцию рассуждения делают в порядке «трейсы → SFT → RL», а не сразу RL на маленькой модели?</summary>

**Короткий ответ.** Потому что на маленькой модели награда почти всегда нулевая. Проверяемая
награда 0/1 не даёт градиента, пока политика не решила задачу хотя бы иногда, а разведка сама
не набредает на длинную верную цепочку. SFT на трейсах учителя выводит ученика в область,
где награда ненулевая, и только после этого RL начинает работать.

**Развёрнуто.** Разница в плотности сигнала на три порядка: RL даёт один скаляр на цепочку
из тысяч токенов, SFT на трейсе — градиент на каждом токене, потому что мы максимизируем
$\log \pi_\theta(r \mid x) + \log \pi_\theta(a \mid r, x)$, где $r$ — цепочка учителя,
$a$ — ответ. Команда DeepSeek показала на R1 (2025), что простой SFT на 800 тысячах отобранных
трейсов даёт открытым моделям 7B–70B результат сильнее, чем масштабный RL, запущенный на той же
базе напрямую. RL нужен вторым этапом, а не вместо: SFT только имитирует учителя и упирается
в его потолок, тогда как RL обучается на собственных сэмплах ученика, снимает экспозиционное
смещение и на проверяемых задачах позволяет учителя обойти.

**Чего ждёт интервьюер:** понимания, что порядок этапов вытекает из разреженности награды,
а не из традиции.

**Провал:** «RL лучше, значит, надо сразу RL».
</details>

<details>
<summary><b>🎯 Вопрос.</b> Дистиллированная модель пишет длинные рассуждения, а качество не выросло. Что произошло?</summary>

**Короткий ответ.** Скопирована форма рассуждения без содержания. SFT максимизирует правдоподобие
токенов, а стилистические маркеры («Wait, let me check», нумерованные шаги) — самые предсказуемые
токены трейса, они выучиваются первыми и почти бесплатно; содержательные шаги — самые
непредсказуемые, выучиваются последними.

**Развёрнуто.** Диагностика в одно измерение: прогнать ученика с принудительно разной длиной
рассуждения. У модели с настоящим рассуждением качество растёт по бюджету; у скопировавшей форму
кривая плоская — лишние токены не несут вычисления. Причины по убыванию частоты: в набор попали
неверные трейсы (не было rejection sampling по правильности финального ответа); задачи вне
покрытия ученика, где трейс для него просто внешний текст; слишком большой разрыв в размере
между учителем и учеником; недообучение на длинных последовательностях при маленьком батче.
Системная мера поверх исправления данных — этап RL с проверяемой наградой: декоративное
рассуждение не приносит награды, потому что она считается по ответу.

**Чего ждёт интервьюер:** что вы умеете отличить «модель рассуждает» от «модель имитирует
рассуждение» измерением, а не на глаз.

**Провал:** «модель маленькая, ничего не поделаешь».
</details>
...
`build_example` из [§3.2](#32-маскирование-лосса-на-промпте) и напечатайте для трёх примеров: исходную строку через `repr()`,
...
Реализуйте `dpo_loss` (см. [§9](#9-dpo-полный-вывод)) и полный обучающий шаг: два прямых прохода (политика и reference),
...
**Задача 6. Набор для дистилляции рассуждения (90 минут).**
Возьмите 300 задач с проверяемым ответом (GSM8K или своя выборка SQL-запросов с эталоном).
Сгенерируйте у сильной модели по 8 трейсов на задачу, реализуйте `build_distillation_set`
из [§11.2](#112-конвейер-целиком) и постройте распределение задач по доле верных ответов учителя.
*Критерий: у вас есть гистограмма долей верных, число задач, отсеянных как «слишком лёгкие»
и «неподъёмные», и итоговая доля выживших трейсов от сгенерированного. Дополнительно: сравните
среднюю длину верных и неверных трейсов — почти наверняка неверные окажутся длиннее, и вы можете
объяснить почему.*
...
- [Rafailov et al. «Direct Preference Optimization: Your Language Model is Secretly a Reward Model» (2023)](https://arxiv.org/abs/2305.18290) —
  DPO. Раздел 4 и приложение A.1 — тот самый вывод, который разобран в [§9](#9-dpo-полный-вывод); стоит пройти по нему
...
- [DeepSeek-AI «DeepSeek-R1: Incentivizing Reasoning Capability in LLMs via Reinforcement
  Learning» (2025)](https://arxiv.org/abs/2501.12948) — первоисточник раздела 11: RL без SFT
  (R1-Zero), холодный старт, отбор 800 тысяч трейсов и результаты дистилляции в Qwen и Llama.
  Читать разделы про сбор данных и про сравнение «дистилляция против RL напрямую».
- [Zelikman et al. «STaR: Bootstrapping Reasoning With Reasoning» (2022)](https://arxiv.org/abs/2203.14465) —
  откуда вырос rejection sampling по правильности ответа. Короткая работа, читается за вечер
  и объясняет, почему фильтр по финальному ответу заменяет разметку рассуждений.
- [Muennighoff et al. «s1: Simple test-time scaling» (2025)](https://arxiv.org/abs/2501.19393) —
  обратная сторона того же конвейера: тысяча отобранных трейсов плюс контроль длины рассуждения
  на уровне декодера. Полезно как минимальный воспроизводимый рецепт.
