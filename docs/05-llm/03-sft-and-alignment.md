...
адаптеров (см. [§13](#13-что-выбрать-под-ресурсы)).
...
- **Сложно отлаживать.** Метрика (награда RM) растёт, а качество для человека падает — это норма,
  а не исключение (см. [§12](#12-reward-hacking)).
...
появляется хакинг проверяющей программы (см. [§12](#12-reward-hacking)). Схема «GRPO + верифицируемая награда» — то, на
