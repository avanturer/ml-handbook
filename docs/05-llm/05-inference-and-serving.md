...
**Предварительно нужно:** [внимание и трансформер](../03-deep-learning/04-attention-and-transformer.md), [устройство современной LLM](01-llm-architecture.md), [PEFT и квантизация](04-peft-and-quantization.md)
**Как проработать:** чтение + расчёты + прогон бенчмарка
...
- [10. KV-кэш как инфраструктура: оффлоад, маршрутизация, переиспользование](#10-kv-кэш-как-инфраструктура-оффлоад-маршрутизация-переиспользование)
- [11. Сэмплирование: чем крутить и что это делает](#11-сэмплирование-чем-крутить-и-что-это-делает)
- [12. Метрики: TTFT, TPOT, throughput, goodput](#12-метрики-ttft-tpot-throughput-goodput)
- [13. Расчёт железа под нагрузку](#13-расчёт-железа-под-нагрузку)
...
> **Где это в книге.** Глава самодостаточна: всё, что нужно про инференс LLM, разобрано здесь.
> Общая механика сервинга моделей — очереди, автоскейлинг, деградация под нагрузкой, — одинакова
> для LLM и для любой другой модели и разобрана дальше, в главе
> [архитектуры сервинга](../07-mlops/05-serving-architectures.md). Читать её сейчас не нужно.
...
при записи — copy-on-write, как в fork(). Это техническая основа prefix caching (см. [§10](#10-kv-кэш-как-инфраструктура-оффлоад-маршрутизация-переиспользование) и
...
- на H100-80 с весами 16 ГБ доступно ~55 ГБ под кэш: 55 запросов вместо 55/2 — то есть батч
  вырастает вдвое;
- по таблице из [§4](#4-почему-батчинг-разгоняет-throughput-и-почти-не-трогает-latency) удвоение батча в memory-bound режиме даёт прирост throughput примерно
...
## 10. KV-кэш как инфраструктура: оффлоад, маршрутизация, переиспользование

До сих пор мы обращались с кэшем как с временным буфером внутри запроса: посчитали на prefill,
дочитали на decode, освободили. Посмотрите на два обычных продуктовых сценария, где эта картина
разваливается.

**Сценарий первый — диалог.** Промпт первого хода — 6000 токенов: системный блок, инструкции,
история. Модель ответила, пользователь читает ответ и сорок секунд формулирует следующий вопрос.
За эти сорок секунд движок обслужил сотни чужих запросов, блоки нашей сессии вытеснены, и второй
ход платит полный prefill заново: $2\cdot 8\cdot10^{9}\cdot 6000/(445\cdot10^{12}) \approx 216$ мс.
Третий ход — ещё раз, уже на 7500 токенов. Полезной работы в этом пересчёте ровно ноль: получаются
те же самые числа, которые были посчитаны минуту назад и выброшены.

**Сценарий второй — двадцать реплик.** Тот же ассистент, но за балансировщиком стоит 20 подов vLLM,
и у всех честно включён `enable_prefix_caching`. Второй ход той же сессии round-robin отправляет
на случайный под. С вероятностью 19/20 это не тот под, где лежит кэш. Кэш есть, он прогрет,
он просто не там.

В обоих случаях причина одна: **KV-кэш — не буфер внутри запроса, а состояние, которое запрос
переживает**. У такого состояния появляются три вопроса, ответов на которые в конфиге движка нет:
где оно лежит, когда не помещается в HBM; кто им владеет, когда реплик много; и можно ли его
переиспользовать, когда совпадение с новым запросом не префиксное. По порядку.

### 10.1 Рабочее множество кэша больше всей вашей видеопамяти

Посчитаем, сколько кэша нужно сервису, если хранить состояние всех живых сессий, а не только
тех, что считаются прямо сейчас. Ассистент на Llama-3-8B, 10 000 сессий с активностью за последние
пять минут, средняя длина диалога 6000 токенов, кэш в bf16 (128 КиБ на токен из [§3](#3-kv-кэш-зачем-он-и-сколько-весит)):

$$
10\,000 \cdot 6000 \cdot 128\ \text{КиБ} \;=\; 7{,}2\ \text{ТиБ}
$$

Пул из 20 карт H100-80 отдаёт под кэш примерно по 55 ГБ на карту — суммарно 1,1 ТБ, то есть
ровно 1,0 ТиБ. Разрыв в семь раз, и он не лечится покупкой ещё десяти карт: вычислительно активны
десятки последовательностей, остальные — спящее состояние, к которому вернутся через секунды или
минуты. Держать спящее состояние в HBM — самое дорогое хранилище, которое можно придумать.

Отсюда идея многоуровневого хранения, ровно как в любой другой системе с иерархией памяти:
горячее — в HBM, тёплое — в DRAM хоста, холодное — на NVMe, ледяное — в общем хранилище.
Вопрос один: где проходит граница выгоды.

### 10.2 Подкачать или пересчитать: критерий на салфетке

Пусть префикс длины $n$ токенов уже посчитан и лежит на каком-то уровне хранения. Есть два способа
получить его $K$ и $V$ в HBM нужной карты.

Первый — подкачать. Объём равен $n \cdot m_{\text{tok}}$, где $m_{\text{tok}}$ — байт кэша
на токен, а время подкачки

$$
t_{\text{load}} \;=\; \frac{n \cdot m_{\text{tok}}}{\text{BW}}
$$

где $\text{BW}$ — эффективная полоса канала от этого уровня до HBM в байтах в секунду.

Второй — пересчитать через prefill. Прямой проход стоит примерно $2 N_{\text{акт}}$ FLOP на токен,
где $N_{\text{акт}}$ — число активных параметров модели (для плотной модели это просто её размер):

$$
t_{\text{recompute}} \;=\; \frac{2 N_{\text{акт}}\, n}{\text{FLOPS}_{\text{eff}}}
$$

где $\text{FLOPS}_{\text{eff}}$ — достижимая производительность всего пула карт, обслуживающего
модель. Подкачка выгодна, когда $t_{\text{load}} < t_{\text{recompute}}$:

$$
\frac{n\, m_{\text{tok}}}{\text{BW}} \;<\; \frac{2 N_{\text{акт}}\, n}{\text{FLOPS}_{\text{eff}}}
\qquad\Longleftrightarrow\qquad
\text{BW} \;>\; \text{BW}^{*} \;=\; \frac{m_{\text{tok}} \cdot \text{FLOPS}_{\text{eff}}}{2 N_{\text{акт}}}
$$

> 📐 **Разбор формулы.** Слева и справа в исходном неравенстве стоит множитель $n$ — и он
> сокращается. Это главное, что нужно вынести из вывода: **выгодность оффлоада не зависит
> от длины префикса**. Интуиция «длинные префиксы выгоднее подкачивать, короткие дешевле
> пересчитать» неверна: длина одинаково масштабирует обе стороны. Решает одно отношение —
> сколько байт кэша порождает токен ($m_{\text{tok}}$) против того, сколько времени этот токен
> считается ($2N_{\text{акт}}/\text{FLOPS}_{\text{eff}}$). Числитель — свойство архитектуры
> (см. GQA и MLA в [главе про архитектуру](01-llm-architecture.md)), знаменатель — свойство железа.

```python
# Что дешевле: подкачать KV-кэш с уровня хранения или пересчитать префикс заново.
def prefill_sec_per_token(n_active: float, eff_tflops: float) -> float:
    """Prefill стоит ~2N FLOP на токен; делим на достижимую производительность пула карт."""
    return 2 * n_active / (eff_tflops * 1e12)


def breakeven_gbs(kv_bytes_per_token: int, n_active: float, eff_tflops: float) -> float:
    """Полоса, при которой подкачка ровно сравнивается с пересчётом.
    Длина префикса в отношении сокращается — критерий получается потокенным."""
    return kv_bytes_per_token / prefill_sec_per_token(n_active, eff_tflops) / 1e9


MODELS = {  # (байт KV на токен, активных параметров, достижимые TFLOPS всего пула)
    "Llama-3-8B,  bf16-кэш, 1×H100": (2 * 32 * 8 * 128 * 2, 8e9, 445),
    "Llama-3-8B,  fp8-кэш,  1×H100": (2 * 32 * 8 * 128 * 1, 8e9, 445),
    "Llama-3-70B, bf16-кэш, 4×H100": (2 * 80 * 8 * 128 * 2, 70e9, 445 * 4 * 0.85),
}
TIERS = {  # эффективная полоса до HBM целевой карты, ГБ/с
    "DRAM хоста, PCIe 4.0 x16": 20.0,
    "DRAM хоста, NVLink-C2C": 450.0,
    "локальный NVMe Gen4, один диск": 6.0,
    "локальный NVMe Gen5, четыре диска": 25.0,
    "другой узел, сеть 100 GbE": 12.0,
    "объектное хранилище, один поток": 1.0,
}

print("порог окупаемости подкачки:")
for name, (kv_b, n_act, tf) in MODELS.items():
    print(f"    {name}   {breakeven_gbs(kv_b, n_act, tf):5.2f} ГБ/с")

kv_b, n_act, tf = MODELS["Llama-3-8B,  bf16-кэш, 1×H100"]
thr, n_pref = breakeven_gbs(kv_b, n_act, tf), 6000
print(f"\nпрефикс {n_pref} токенов, Llama-3-8B: пересчёт "
      f"{n_pref * prefill_sec_per_token(n_act, tf) * 1e3:.0f} мс")
for tier, bw in TIERS.items():
    ms = n_pref * kv_b / (bw * 1e9) * 1e3
    print(f"    {tier:34s} {bw:6.1f} ГБ/с  {ms:6.1f} мс  "
          f"{'подкачка' if bw > thr else 'пересчёт'} ({bw / thr:.1f}× порога)")
```

```
порог окупаемости подкачки:
    Llama-3-8B,  bf16-кэш, 1×H100    3.65 ГБ/с
    Llama-3-8B,  fp8-кэш,  1×H100    1.82 ГБ/с
    Llama-3-70B, bf16-кэш, 4×H100    3.54 ГБ/с

префикс 6000 токенов, Llama-3-8B: пересчёт 216 мс
    DRAM хоста, PCIe 4.0 x16             20.0 ГБ/с    39.3 мс  подкачка (5.5× порога)
    DRAM хоста, NVLink-C2C              450.0 ГБ/с     1.7 мс  подкачка (123.4× порога)
    локальный NVMe Gen4, один диск        6.0 ГБ/с   131.1 мс  подкачка (1.6× порога)
    локальный NVMe Gen5, четыре диска    25.0 ГБ/с    31.5 мс  подкачка (6.9× порога)
    другой узел, сеть 100 GbE            12.0 ГБ/с    65.5 мс  подкачка (3.3× порога)
    объектное хранилище, один поток       1.0 ГБ/с   786.4 мс  пересчёт (0.3× порога)
```

Три вещи в этих числах стоят внимания.

**Порог удивительно устойчив — около 3,5 ГБ/с для всех трёх конфигураций.** Это не совпадение
и не закон: и байты кэша на токен, и FLOPs на токен растут вместе с моделью (оба примерно
пропорциональны числу слоёв и ширине), поэтому отношение почти не меняется. Практический вывод:
цифру «нужно быстрее 3–4 гигабайт в секунду» можно держать в голове как ориентир, а конкретное
значение пересчитывать под свою модель за минуту.

**DRAM хоста окупается всегда, с пятикратным запасом даже на PCIe 4.0.** Это самый безопасный
уровень оффлоада и то, что стоит включать первым: пул на 512 ГБ хостовой памяти держит 4 млн
токенов кэша Llama-3-8B — в семь раз больше, чем вся видеопамять узла.

**Диск — впритык, объектное хранилище — мимо.** NVMe Gen4 даёт 1,6× порога в идеальном
последовательном режиме; при конкурирующих чтениях запас исчезает. Gen5-массив или fp8-кэш
(порог падает вдвое, до 1,8 ГБ/с) возвращают уровню смысл. А вот подкачка из объектного хранилища
проигрывает пересчёту втрое — и, по доказанному выше, **никакая длина префикса это не исправит**.

> ⚠️ **Типичная ошибка.** Продавать многоуровневый оффлоад «до S3» как способ ускорить TTFT
> и удивляться, что после включения стало хуже. Арифметика выше этого не подтверждает: канал
> медленнее порога — значит, каждый попавший в S3 префикс стоит дороже, чем честный prefill,
> плюс десятки миллисекунд на первый байт. У холодного уровня другая роль, и её надо называть
> честно: (а) **предзагрузка** — пока пользователь сорок секунд печатает следующий вопрос,
> префикс успевает подняться в DRAM, и латентность канала полностью скрыта; (б) **переживание
> рестарта** — после выкатки новая реплика не входит в ротацию холодной и не собирает на себя
> всплеск p99; (в) **общий пул между узлами**, когда сессию нужно перенести. Ни один из трёх
> пунктов не про мгновенную подкачку в момент запроса.

Отдельная поправка, которую редко проговаривают: критерий выше сформулирован по времени стены,
то есть под метрику TTFT одного запроса. Под нагрузкой оптимизируют другое — goodput, а там
дефицитен не канал, а вычислитель. Подкачка не тратит ни одного FLOP, поэтому даже уровень,
чуть проигрывающий по времени стены, может поднять общую пропускную способность: карта в это
время делает decode для остальных. Проверять это надо замером, а не рассуждением — но знать
про расхождение двух критериев обязательно.

> 💬 **На собеседовании.** Спросят (middle+): «Кэш не влезает в память карты. Вы хотите
> выгружать его на CPU и на диск — как поймёте, что от этого станет лучше, а не хуже?»
> Хороший ответ: сравнить время подкачки со временем пересчёта и заметить, что длина префикса
> в этом сравнении сокращается. Остаётся порог по полосе: $\text{BW}^{*} = m_{\text{tok}}\,\text{FLOPS}_{\text{eff}} / (2N_{\text{акт}})$,
> для Llama-3-8B на H100 это около 3,6 ГБ/с. Дальше по уровням: хостовая DRAM через PCIe 4.0 —
> 20 ГБ/с, окупается; NVMe — впритык, но fp8-кэш вдвое снижает порог и делает диск рабочим;
> объектное хранилище не окупается вообще, и удлинение префикса тут не поможет. Сильное
> дополнение — оговорка, что под нагрузкой критерий меняется: подкачка не тратит FLOPs,
> поэтому по goodput граница проходит правее, чем по TTFT.
> Провал: «выгрузим на диск, память же кончилась» без единого числа.

### 10.3 KV-aware маршрутизация вместо round-robin

Теперь второй вопрос: кто владеет состоянием, когда реплик двадцать.

Сначала уточним, что именно ломает round-robin, потому что обычно это формулируют слишком широко.
Общий системный промпт round-robin **не** ломает: он одинаков у всех запросов, поэтому прогревается
на каждой реплике за первые же секунды и дальше попадает в кэш всегда. Ломается ровно то, что
специфично для сессии, документа или пользователя. Отсюда честная формула ожидаемой доли попаданий
для round-robin:

$$
\text{hit} \;=\; \frac{n_{\text{общ}} + n_{\text{сесс}}/R}{n_{\text{общ}} + n_{\text{сесс}}}
$$

где $n_{\text{общ}}$ — длина общей для всех запросов части промпта, $n_{\text{сесс}}$ — длина
части, специфичной для этой сессии, $R$ — число реплик. Для нашего диалога ($n_{\text{общ}}=400$,
$n_{\text{сесс}}=5600$, $R=20$) это 11%. Пересчитывается в среднем 5320 токенов, то есть 191 мс
на каждом ходе, при том что маршрутизация «в ту же реплику» оставила бы 60 токенов нового вопроса —
2 мс. Разница почти в сто раз, и получена она не оптимизацией модели, а выбором номера пода.

**Как маршрутизировать.** Маршрутизатору нужно знать, у какой реплики какие префиксы. Практически
это делается так: промпт режется на выровненные по блокам куски (те самые 16-токенные блоки
из [§6](#6-pagedattention-виртуальная-память-для-kv-кэша)), от каждого префикса считается хеш, реплики публикуют множество своих хешей,
маршрутизатор хранит их в префиксном дереве. Для каждой реплики $r$ получается $c_r$ — сколько
ведущих токенов запроса у неё уже посчитано. Дальше выбираем реплику по оценке TTFT:

$$
\widehat{\text{TTFT}}_r \;=\; \big(Q_r + n - c_r\big)\cdot \tau
$$

где $Q_r$ — число ещё не обработанных токенов prefill в очереди реплики $r$, $n$ — длина промпта,
$c_r$ — сколько его ведущих токенов лежит в кэше этой реплики, $\tau$ — время prefill одного
токена (те самые 36 мкс для 8B на H100). Берём $\arg\min_r \widehat{\text{TTFT}}_r$.

Из этой формулы сразу читается правило, ради которого её и стоит выписать: **попадание в кэш
на $c$ токенов стоит ровно столько же, сколько очередь длиной $c$ токенов**. Идти в загруженную
реплику ради кэша можно ровно до тех пор, пока её очередь короче сэкономленного префикса. Всё
остальное в KV-aware роутерах — реализация этой одной строчки.

```mermaid
flowchart LR
    Q["второй ход диалога:<br/>400 общих + 5600 сессионных + 60 новых"] --> R["маршрутизатор:<br/>префиксное дерево<br/>хешей блоков"]
    R -->|"c = 6000, очередь 200 токенов"| A["реплика 3<br/>TTFT ≈ 9 мс"]
    R -->|"c = 400, очередь 0"| B["реплика 11<br/>TTFT ≈ 200 мс"]
    R -.-> C["round-robin взял бы<br/>любую из 20"]
```

**Что ломается при перекосе.** Как только маршрутизация становится липкой, нагрузка перестаёт быть
равномерной. Популярный документ (инструкция, которую спрашивают в десять раз чаще остальных)
намертво привязан к одной реплике — она копит очередь, а девятнадцать соседних простаивают.
Проверим на симуляции.

```python
# Сколько префиксного кэша съедает round-robin и что с ним делает маршрутизация по префиксу.
import numpy as np

R, N_DOCS, N_REQ = 20, 200, 20_000      # реплик, разных документов-префиксов, запросов
DOC_TOKENS = 4000                       # общий документ в промпте; вопрос дописывается справа
CACHE_DOCS = 6                          # сколько документов помещается в кэш одной реплики
rng = np.random.default_rng(0)

# Трафик перекошен: несколько документов спрашивают в разы чаще остальных (Zipf, s = 1.1).
w = 1.0 / np.arange(1, N_DOCS + 1) ** 1.1
docs = rng.choice(N_DOCS, size=N_REQ, p=w / w.sum())


def run(router) -> tuple[float, float]:
    """Возвращает (доля пересчитанных токенов промпта, перекос нагрузки max/mean)."""
    lru = [[] for _ in range(R)]        # список документов на реплике, свежие в конце
    load = np.zeros(R)
    recomputed = 0
    for doc in docs:
        r = router(doc, load, lru)
        load[r] += 1
        if doc in lru[r]:
            lru[r].remove(doc)          # попадание: пересчитываем только вопрос
        else:
            recomputed += DOC_TOKENS    # промах: весь документ заново через prefill
            if len(lru[r]) == CACHE_DOCS:
                lru[r].pop(0)           # вытесняем самый давний
        lru[r].append(doc)
    return recomputed / (N_REQ * DOC_TOKENS), load.max() / load.mean()


rr_counter = [0]


def round_robin(doc, load, lru):
    rr_counter[0] += 1
    return rr_counter[0] % R


def by_prefix(doc, load, lru):
    return hash(doc) % R                # вся нагрузка по документу липнет к одной реплике


def by_prefix_bounded(doc, load, lru):
    """Липнем к «своей» реплике, пока она не перегружена: иначе — к самой свободной."""
    home = hash(doc) % R
    return home if load[home] <= 1.25 * max(load.mean(), 1) else int(load.argmin())


for name, router in [("round-robin", round_robin),
                     ("по префиксу", by_prefix),
                     ("по префиксу + ограничение нагрузки", by_prefix_bounded)]:
    miss, skew = run(router)
    print(f"{name:36s} пересчитано {miss:5.1%} токенов промпта, перекос {skew:.2f}×")
```

```
round-robin                          пересчитано 70.9% токенов промпта, перекос 1.00×
по префиксу                          пересчитано 10.1% токенов промпта, перекос 4.60×
по префиксу + ограничение нагрузки   пересчитано 22.1% токенов промпта, перекос 1.25×
```

Три строчки — весь компромисс. Round-robin идеально ровный и выбрасывает 71% работы prefill
на ветер. Чистая привязка к префиксу спасает почти всё, но одна реплика получает в 4,6 раза
больше нагрузки, чем средняя, — её очередь, а значит и p99 по TTFT, разваливается. Привязка
с ограничением по нагрузке отдаёт часть выигрыша (22% пересчёта вместо 10%) и возвращает
равномерность.

Единого мнения о том, где ставить границу, нет: открытые роутеры (маршрутизатор vLLM
production-stack, роутер SGLang, NVIDIA Dynamo) сводятся к одной схеме «скор попадания минус
штраф за очередь», а коэффициент подбирают под свой трафик. Формула $\widehat{\text{TTFT}}_r$
выше даёт естественный способ его не подбирать вслепую: обе части измеряются в токенах, и обмен
между ними честный.

> 💬 **На собеседовании.** Спросят (middle+): «У вас 20 реплик vLLM за балансировщиком.
> Как балансируете нагрузку?»
> Хороший ответ начинается с того, что round-robin здесь неверен по существу, и почему именно:
> общий системный промпт прогрет везде и от round-robin не страдает, а сессионная часть теряется
> с вероятностью $(R-1)/R$ — на диалоге 400 + 5600 токенов это 191 мс лишнего prefill на каждом
> ходе. Дальше — механика: маршрутизатор хранит префиксное дерево хешей блоков, знает $c_r$
> для каждой реплики и выбирает по оценке $\widehat{\text{TTFT}}_r = (Q_r + n - c_r)\tau$,
> то есть меняет попадание в кэш на длину очереди один к одному в токенах. И обязательно —
> про обратную сторону: липкая маршрутизация даёт перекос на популярных префиксах, поэтому нужен
> потолок по нагрузке или выбор из двух кандидатов; в замерах это выглядит как «10% пересчёта
> при перекосе 4,6×» против «22% при 1,25×».
> Провал: «round-robin, потому что реплики одинаковые» — реплики одинаковые, а кэши у них разные.

### 10.4 Не-префиксное переиспользование: почему RAG проходит мимо кэша

Префиксный кэш требует **точного совпадения всего, что стоит слева**. В диалоге это выполняется
естественно: история растёт справа. В RAG — нет, и вот почему.

Промпт RAG-сервиса выглядит как `[системный блок][документ A][документ B][вопрос]`. Документы
подбираются ретривером под запрос, и набор каждый раз свой. Пусть документ B встретился второй
раз, но в паре с документом C вместо A. Его $K$ и $V$ считались в контексте, где слева лежал A,
и на позициях, сдвинутых длиной A. Совпадения префикса нет — попадания нет. Практически это значит,
что в RAG префиксный кэш ловит только системный блок, то есть проценты промпта, а основную его
массу пересчитывает всегда.

Первое, что надо сделать, — не техника, а порядок склейки: если один и тот же большой кусок
(инструкция, схема БД, описание продукта, few-shot) стоит в начале, он становится настоящим
префиксом и кэшируется бесплатно во всех движках. Это ноль строк кода и половина выигрыша.
Разбор порядка сборки промпта — в [главе про RAG](07-rag.md#94-порядок-склейки-промпта-и-кэш).

Дальше начинается интересное. Наивная идея «посчитать $K,V$ каждого чанка отдельно и просто
склеить» (её реализуют системы уровня prompt cache) даёт неправильный кэш: в нём ни один чанк
никогда не смотрел на соседей. На вопросах, где ответ собирается из двух фрагментов, качество
падает обвально — а это ровно те вопросы, ради которых RAG и делают.

**CacheBlend** (Yao et al., 2024) чинит это выборочно. Кэш чанков берётся готовым, но
не целиком: на каждом слое считается, насколько $K,V$ токена расходятся с тем, что дал бы честный
полный prefill, отбирается небольшая доля токенов с наибольшим расхождением (в замерах авторов
порядка 10–15%), и **только они** пересчитываются полностью; поправка распространяется на следующий
слой. Загрузка кэша следующего слоя при этом идёт параллельно с вычислением текущего, так что
подкачка прячется за арифметику. Заявленный результат — сокращение TTFT в разы при качестве
в пределах шума относительно полного prefill.

Цена, которую надо назвать вслух, потому что она решает, стоит ли это внедрять:

**Хранилище раздувается на четыре порядка.** Чанк на 500 токенов — это пара килобайт текста
и $500\cdot128\ \text{КиБ} = 62{,}5$ МиБ KV-кэша для Llama-3-8B: отношение порядка 30 000.
Корпус из 200 тысяч чанков займёт 12 ТиБ вместо считанных сотен мегабайт. Это принципиально другой
класс системы хранения, и держать так весь индекс бессмысленно — только горячую голову
распределения запросов.

**Кэш привязан к модели.** Он валиден для конкретных весов, типа данных и схемы тензорного
параллелизма. Обновили модель — всё хранилище стало мусором, и его надо строить заново. Текстовый
индекс переживает смену модели, KV-хранилище — нет.

**Позиции.** Чанк считался на позициях с нуля, а лежит на позициях со сдвигом. С RoPE поворот
пересчитывается на разницу дёшево (кодирование относительное), так что это техническая деталь,
а не препятствие. Препятствие — именно отсутствие перекрёстного внимания, и его лечит выборочный
пересчёт.

Практический итог: не-префиксное переиспользование окупается там, где одни и те же документы
запрашивают снова и снова (база поддержки, репозиторий кода, свод регламентов), порядок склейки
уже исправлен, а TTFT всё равно не укладывается в SLO. Во всех остальных случаях сначала
переупорядочивание промпта, потом префиксный кэш, и только потом это.

> 💬 **На собеседовании.** Спросят (middle+): «В вашем RAG hit rate префиксного кэша около нуля,
> хотя документы повторяются. Почему и что делать?»
> Хороший ответ: префиксный кэш требует совпадения всего левого контекста, а в RAG повторяющийся
> документ стоит в середине промпта и его $K,V$ зависят от того, что подобрал ретривер левее.
> Чинится в три шага. Первый — переупорядочить промпт: всё стабильное (системный блок, инструкции,
> few-shot, схему) в начало, изменчивое в конец; это возвращает попадания на общей части
> и не стоит ничего. Второй — стабилизировать порядок самих документов, чтобы одинаковые наборы
> давали одинаковый префикс. Третий, если этого мало, — не-префиксное переиспользование
> в духе CacheBlend: брать готовые $K,V$ чанков и пересчитывать 10–15% токенов с наибольшим
> расхождением, чтобы восстановить перекрёстное внимание. И сразу назвать цену: KV-хранилище
> в десятки тысяч раз объёмнее текста и умирает при смене модели.
> Провал: «включим prefix caching» — он уже включён, в этом и вопрос.

### 10.5 Выбрасывание токенов: последний рычаг и его цена

Квантизация кэша разобрана в [§9](#9-flashattention-и-квантизация-kv-кэша); она уменьшает $b$ в формуле $M_{KV}$ и стоит
почти ничего. Остаётся последний множитель — $n$, число хранимых токенов. Архитектурно его
ограничивает скользящее окно, но это решение принимается до обучения (см.
[главу про архитектуру](01-llm-architecture.md)). На готовой модели остаётся один ход:
выбрасывать из кэша уже посчитанные токены.

Наблюдение, на котором всё держится: распределение внимания сильно
неравномерно — небольшая доля позиций собирает основную массу весов, остальные почти не влияют
на выход. H2O (Zhang et al., 2023) накапливает суммарный вес внимания на каждую позицию и держит
в кэше «тяжеловесов» плюс скользящее окно последних токенов. SnapKV (Li et al., 2024) делает
то же самое разово на prefill: смотрит, на что смотрят последние токены промпта, и сжимает кэш
до этого подмножества ещё до начала генерации.

Платят за это тремя вещами, и все три систематические.

**Потеря необратима и проявляется не там, где тестировали.** Решение «этот токен неважен»
принимается по тому вниманию, которое есть сейчас, а спросить могут через двадцать ходов диалога
про деталь, которая тогда казалась фоном. На суммаризации падения не видно, на извлечении
конкретного факта из длинного документа — видно сразу.

**Сжатый кэш нельзя переиспользовать.** Это тот пункт, который почти никогда не проговаривают,
а он ломает всю конструкцию предыдущих разделов: какие токены выброшены, зависит от запроса,
поэтому кэш перестаёт быть функцией одного лишь префикса. Делить его между запросами нельзя,
маршрутизировать по префиксу — тоже. Вы меняете переиспользование на объём, а в проде
переиспользование обычно дороже.

**Нужны веса внимания, которых нет.** FlashAttention вообще не материализует матрицу весов
внимания — в этом весь его смысл ([§9](#9-flashattention-и-квантизация-kv-кэша)). Чтобы получить накопленные веса
для отбора, нужен либо отдельный проход, либо ядро, которое их попутно аккумулирует. Это
не смертельно, но объясняет, почему методы этого класса приживаются в движках медленно.

> 🧠 **Middle+.** Разложите рычаги по формуле $M_{KV} = 2Lh_{kv}d_{head}nBb$ и увидите порядок,
> в котором их берут в проде. $b$ — квантизация кэша: fp8 даёт ровно 2× практически бесплатно,
> включается флагом. $h_{kv}$ и $L$ — архитектура: GQA, MLA, разделение кэша между слоями;
> на готовой модели не меняется, но определяет выбор модели ([глава про архитектуру](01-llm-architecture.md)).
> $n$ — скользящее окно (архитектурное, честное) и выбрасывание токенов (эвристическое, ломает
> переиспользование). $B$ — просто ограничение конкурентности, то есть отказ от throughput.
> А иерархия хранения и маршрутизация из этого раздела не трогают формулу вообще: они меняют
> не размер кэша, а то, сколько раз вы за него платите. Именно поэтому в 2025–2026 годах
> выигрыш чаще приходит оттуда, а не от очередного метода сжатия.

---

## 11. Сэмплирование: чем крутить и что это делает
...
## 12. Метрики: TTFT, TPOT, throughput, goodput
...
Берём таблицу из [§4](#4-почему-батчинг-разгоняет-throughput-и-почти-не-трогает-latency). Нужный уровень параллелизма определяется законом Литтла: среднее число
...
Что делать: считать по формуле [§3](#3-kv-кэш-зачем-он-и-сколько-весит); в vLLM ограничить `max_num_seqs` и `max_model_len`,
...
**Prefix caching включён на всех репликах, а hit rate 10%.**
Симптом: на одном поде в бенчмарке кэш попадает почти всегда, в проде за балансировщиком —
почти никогда.
Причина: round-robin. Общая часть промпта прогрета везде и попадает, а сессионная живёт
на одной случайной реплике из $R$ — ожидаемая доля попаданий падает до
$(n_{\text{общ}} + n_{\text{сесс}}/R)/(n_{\text{общ}} + n_{\text{сесс}})$.
Что делать: маршрутизировать по префиксу с потолком по нагрузке ([§10.3](#103-kv-aware-маршрутизация-вместо-round-robin)); мерить hit rate
не на поде, а на балансировщике; не забывать, что липкая маршрутизация приносит с собой перекос
и его надо ограничивать.

**Включили оффлоад кэша, стало медленнее.**
Симптом: после выгрузки KV на диск или в общее хранилище hit rate вырос, а TTFT p95 ухудшился.
Причина: полоса канала ниже порога окупаемости — каждое «попадание» обходится дороже честного
prefill, плюс латентность первого байта.
Что делать: посчитать $\text{BW}^{*} = m_{\text{tok}}\text{FLOPS}_{\text{eff}}/(2N_{\text{акт}})$
([§10.2](#102-подкачать-или-пересчитать-критерий-на-салфетке)); оставить синхронную подкачку только для уровней быстрее порога (обычно это
DRAM хоста), а медленный уровень использовать для предзагрузки между ходами диалога и для
прогрева реплик после выкатки.
...
<details>
<summary><b>🧠 Вопрос.</b> Кэш не влезает в память карты. Когда выгодно выгружать его на CPU
или на диск, а когда дешевле пересчитать префикс?</summary>

**Короткий ответ.** Сравниваем время подкачки со временем пересчёта. Длина префикса в этом
сравнении сокращается, остаётся порог по полосе канала:
$\text{BW}^{*} = m_{\text{tok}}\,\text{FLOPS}_{\text{eff}}/(2N_{\text{акт}})$. Для Llama-3-8B
на H100 это ≈3,6 ГБ/с.

**Развёрнуто.** Подкачка стоит $n\,m_{\text{tok}}/\text{BW}$, пересчёт — $2N_{\text{акт}}n/\text{FLOPS}_{\text{eff}}$;
$n$ сокращается, поэтому «длинный префикс выгоднее подкачивать» — неверная интуиция. По уровням:
хостовая DRAM через PCIe 4.0 даёт 20 ГБ/с, пятикратный запас, включать можно всегда; NVMe Gen4 —
1,6× порога, впритык, но fp8-кэш снижает порог вдвое и делает диск рабочим; объектное хранилище
проигрывает пересчёту втрое, и никакая длина префикса это не изменит. Порог, к слову, почти
одинаков для 8B и 70B (3,6 против 3,5 ГБ/с): байты кэша на токен и FLOPs на токен растут вместе.
Важная поправка: критерий выведен под TTFT одного запроса. Под нагрузкой оптимизируют goodput,
а там подкачка выигрывает дополнительно, потому что не тратит FLOPs. Для медленных уровней
правильный режим — не подкачка в момент запроса, а предзагрузка, пока пользователь печатает,
и прогрев реплик после рестарта.

**Чего ждёт интервьюер:** что вы сведёте вопрос к одному неравенству и заметите сокращение $n$,
а не будете перечислять уровни хранения.

**Провал:** «выгрузим на диск, память же кончилась» без сравнения со стоимостью prefill.
</details>

<details>
<summary><b>🧠 Вопрос.</b> У вас 20 реплик vLLM за балансировщиком, на всех включён prefix caching.
Как балансируете нагрузку?</summary>

**Короткий ответ.** Не round-robin: он теряет сессионную часть кэша с вероятностью $(R-1)/R$.
Нужна маршрутизация по префиксу — маршрутизатор знает, сколько ведущих токенов запроса лежит
на каждой реплике, и выбирает по оценке $\widehat{\text{TTFT}}_r = (Q_r + n - c_r)\tau$.

**Развёрнуто.** Уточнение, которое отличает понимание от заученного: общий системный промпт
round-robin не ломает — он прогрет на всех репликах. Теряется ровно сессионная (или документная)
часть. Для промпта 400 + 5600 токенов при 20 репликах ожидаемая доля попаданий — 11%, лишний
prefill — 5320 токенов, то есть 191 мс на каждом ходе диалога. Механика липкой маршрутизации:
промпт режется на выровненные по блокам куски, от префиксов считаются хеши, реплики публикуют
свои, маршрутизатор держит префиксное дерево. Формула $\widehat{\text{TTFT}}_r$ даёт правило
обмена: попадание на $c$ токенов равноценно очереди в $c$ токенов. Обратная сторона —
перекос: популярный документ привязывается к одной реплике. В симуляции с зипфовским трафиком
чистая привязка даёт 10% пересчёта при перекосе 4,6×, а привязка с потолком по нагрузке —
22% при 1,25×. Что выбрать, зависит от того, что первым пробивает SLO.

**Чего ждёт интервьюер:** что вы отделите общую часть промпта от сессионной и сами назовёте
проблему перекоса, не дожидаясь наводящего вопроса.

**Провал:** «round-robin, реплики же одинаковые» — реплики одинаковые, а их кэши разные.
</details>
...
Возьмите код из [§11](#11-сэмплирование-чем-крутить-и-что-это-делает) и на реальной модели сгенерируйте по 20 продолжений одного промпта при
...
**Задача 7. Порог оффлоада и KV-aware маршрутизация (75 минут).**
Часть А: посчитайте $\text{BW}^{*}$ для своей модели и своего железа и разложите по уровням
хранения, которые у вас реально есть (хостовая память, локальный NVMe, сетевое хранилище).
Часть Б: возьмите симулятор из [§10.3](#103-kv-aware-маршрутизация-вместо-round-robin) и добавьте в него очередь: каждая реплика
обрабатывает токены prefill с постоянной скоростью, запросы приходят пуассоновским потоком,
маршрутизатор выбирает реплику по $\widehat{\text{TTFT}}_r = (Q_r + n - c_r)\tau$. Снимите
p50 и p99 TTFT для round-robin, чистой привязки к префиксу и привязки с потолком по нагрузке.
*Критерий: (а) вы называете, какие уровни хранения у вас окупаются, а какие годятся только
для предзагрузки; (б) на графике видно, что чистая привязка выигрывает по p50 и проигрывает
по p99, и вы можете указать интенсивность потока, при которой это происходит.*
...
- [Qin et al. «Mooncake: A KVCache-centric Disaggregated Architecture for LLM Serving» (2024)](https://arxiv.org/abs/2407.00079) —
  как выглядит продовый сервис, у которого KV-кэш вынесен в отдельный пул DRAM и SSD на весь
  кластер. Читать ради инженерных деталей планировщика и политики вытеснения — это самый
  подробный публичный текст про кэш как инфраструктуру.
- [Yao et al. «CacheBlend: Fast Large Language Model Serving for RAG with Cached Knowledge Fusion» (2024)](https://arxiv.org/abs/2405.16444) —
  не-префиксное переиспользование: как склеить готовые $K,V$ разных чанков и вернуть перекрёстное
  внимание пересчётом малой доли токенов. Ключевая работа, если у вас RAG с повторяющимися документами.
- [Zheng et al. «SGLang: Efficient Execution of Structured Language Model Programs» (2023)](https://arxiv.org/abs/2312.07104) —
  RadixAttention: префиксное дерево поверх блоков кэша. Отсюда же растут KV-aware маршрутизаторы.
- [Zhang et al. «H2O: Heavy-Hitter Oracle for Efficient Generative Inference» (2023)](https://arxiv.org/abs/2306.14048)
  и [Li et al. «SnapKV: LLM Knows What You are Looking for Before Generation» (2024)](https://arxiv.org/abs/2404.14469) —
  два канонических способа выбрасывать токены из кэша. Читать вместе с разделом про их цену:
  обе работы меряют качество, но не переиспользование.
