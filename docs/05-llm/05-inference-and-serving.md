...
при записи — copy-on-write, как в fork(). Это техническая основа prefix caching (см. [§7](#7-chunked-prefill-как-перестать-ронять-tpot-длинными-промптами) и
...
Возьмите код из [§10](#10-сэмплирование-чем-крутить-и-что-это-делает) и на реальной модели сгенерируйте по 20 продолжений одного промпта при
