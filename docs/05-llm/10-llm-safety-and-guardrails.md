...
**Как проработать:** чтение + сборка фильтров + разбор атак на своей системе
...
Если модель физически не может выполнить опасное действие, инъекция бессильна. Об этом — весь [§9](#9-инструменты-sandbox-и-наименьшие-привилегии).
...
В этом коде три инженерных решения, которые легко пропустить при чтении.
...
на разных маршрутах — это прямое следствие формулы из [§8.2](#82-экономика-фильтра-как-выбрать-порог).
...
Что делать: начинать с [§9](#9-инструменты-sandbox-и-наименьшие-привилегии). Привилегии — первый уровень защиты, фильтры — третий.
...
Что делать: двухуровневый аудит ([§11.1](#111-что-записывать)), редакция на входе в лог, TTL, ограничение доступа,
...
Реализуйте функцию из [§4](#4-что-реально-работает-против-инъекций) (уровень 2) полностью: белый список доменов для ссылок, удаление внешних
...
Реализуйте `ToolGuard` из [§9.1](#91-принцип-наименьших-привилегий-конкретно) и подключите его к простому агенту с двумя инструментами (одно
...
Реализуйте `verify_citations` из [§10.2](#102-три-технических-рычага) и проверьте её на трёх случаях: настоящая цитата, цитата с
...
Внедрите двухуровневый аудит из [§11.3](#113-минимальная-реализация). Затем смоделируйте инцидент: проведите успешную инъекцию
...
- [Wei A. et al. «Jailbroken: How Does LLM Safety Training Fail?» (2023)](https://arxiv.org/abs/2307.02483) —
  та самая рамка «конкуренция целей + несовпадение обобщения» из [§5.2](#52-почему-выравнивание-пробивается). Лучшее объяснение, *почему*
