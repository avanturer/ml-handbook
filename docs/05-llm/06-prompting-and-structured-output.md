...
Типичные отказы: обёртка в ` ```json `, преамбула «Конечно, вот результат:», одинарные кавычки,
хвостовая запятая, обрезка по `max_tokens` посреди объекта, поле с выдуманным именем.
