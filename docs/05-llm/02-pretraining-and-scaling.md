...
**Время на проработку:** ~6 часов (чтение + расчёты + реализация дедупликации)
...
4. **Неконтролируемое изменение смеси.** Если один домен дублирован сильнее другого,
   ваши тщательно подобранные проценты в §2.3 на деле не соблюдаются.
...
в прямом проходе ($L$ — слоёв, $n$ — длина контекста, $d$ — размерность модели; множитель учитывает,
что при причинной маске токен в среднем смотрит на $n/2$ предшественников). Относительная добавка:

$$
\frac{2Lnd}{2N} = \frac{L\,n\,d}{N}
$$

Для Llama-3-8B при $n = 8192$: $32\cdot8192\cdot4096 / 8{,}03\cdot10^9 \approx 0{,}13$ — то есть
$6ND$ занижает бюджет на 13%. При контексте 128k занижение будет уже двукратным. Практическое
правило: **$6ND$ точна с ошибкой в единицы процентов, пока $n \ll N/(Ld)$**, и это выполняется
для типовых претрейнов на 4–8k.
...
Именно перплексию (точнее, лосс) минимизируют законы масштабирования из §6, и именно она
...
перплексия улучшится без всякого улучшения модели. См. §3.
...
> 💬 **На собеседовании.** Спросят (middle+): «Что такое continual pretraining и когда
> вы бы его применили?»
> Хороший ответ: это продолжение предобучения с той же целью (next-token prediction) на новом
> корпусе сырого текста. Применяют, когда нужны **знания**: новый язык, новый домен
> с собственной терминологией, расширение контекстного окна. Нужны миллиарды токенов, LR
> в 3–10 раз ниже пикового, повторный warmup, подмешивание 1–5% исходных данных против
> забывания и регрессионный набор для контроля общих способностей. Если задача — формат,
> стиль или следование инструкции, CPT не нужен, нужен SFT на тысячах примеров. Если задача —
> актуальные факты, нужен RAG: класть меняющиеся знания в веса — это переобучать модель
> при каждом изменении.
> Провал: назвать CPT «просто fine-tuning на своих данных» и не назвать объём и риск забывания.
...
десятках миллиардов токенов. Причина понятна из §5: attention квадратичен по длине, и обучать
...
Что делать: пользоваться поправкой $L n d/N$ из §5; при контексте 128k закладывать двукратный
запас.
...
код из §3) MinHash+LSH и измерьте: долю удалённых документов, распределение размеров кластеров
