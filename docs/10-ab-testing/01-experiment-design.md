...
**Время на проработку:** ~4 часа (чтение + расчёты + упражнения)
...
> и незначимый результат не означает отсутствие эффекта.
...
> больше, но независимых единиц столько же; наивно применять t-тест нельзя (см. §4), однако
...
значимость (типичные причины: зависимые наблюдения из §4, неверная формула для ratio-метрик,
...
**Короткий ответ.** Порядка 240 тысяч на группу, около полумиллиона суммарно.

**Развёрнуто.** Абсолютный эффект $\Delta = 0.04 \cdot 0.05 = 0.002$. Дисперсия бинарной
метрики $\sigma^2 = p(1-p) = 0.04 \cdot 0.96 = 0.0384$. По правилу $n \approx 16\sigma^2/\Delta^2
= 16 \cdot 0.0384 / 4\cdot10^{-6} = 0.6144 / 4\cdot10^{-6} \approx 153{,}600$. С точным
...
*Сделано правильно: для доли 5% и относительного лифта 2% при 50/50 функция даёт ~760 тыс.
...
из §9, включая три абзаца «что делаем, если эффект положительный / отрицательный / незначимый».
...
- [Google. «Rules of Machine Learning»](https://developers.google.com/machine-learning/guides/rules-of-ml) —
  правила 1–10 и раздел про метрики хорошо ложатся на §3 этой главы.
