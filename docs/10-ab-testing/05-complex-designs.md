...
и правда тоже около `+0.004`. Оценка завышена больше чем на порядок, и это не шум:
...
принципиально. Для этого нужен holdout ([§8](#8-holdout-группы-и-долгосрочный-эффект)).
...
# Ожидаем ≈ 0.5; при n=4000 и sigma=1 стандартная ошибка коэффициента около 0.06,
# так что значение 0.45-0.55 — это норма, а не ошибка в коде
print(round(did(y, treat, post), 3))
```
...
Возьмите симуляцию из [§1](#1-интуиция-эксперимент-который-врёт-в-15-раз). Постройте график: по оси X — доля трафика в тесте от 1% до 99%,
...
- **Kohavi, Tang, Xu. «Trustworthy Online Controlled Experiments» (Cambridge University Press, 2020)** —
  ...
  эффекты закрывают [§2](#2-формализация-sutva-и-откуда-берётся-смещение), [§3](#3-соцсети-кластерная-рандомизация-и-цена-мощности) и [§8](#8-holdout-группы-и-долгосрочный-эффект) этой главы на уровне, достаточном для собеседования.
...
- [Hernán, Robins. «Causal Inference: What If»](https://miguelhernan.org/whatifbook) —
  ...
  допущений дают язык, на котором обсуждаются [§2](#2-формализация-sutva-и-откуда-берётся-смещение) и [§7](#7-квазиэксперименты-когда-рандомизации-нет-вообще).
