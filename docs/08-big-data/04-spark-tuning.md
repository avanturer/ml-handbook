...
**Как проработать:** чтение + разбор UI своей реальной джобы
...
|---|---|---|
| **Перекос данных** | max задачи ≫ median (в 10–1000 раз) | [§3](#3-перекос-данных-как-его-увидеть)–[§6](#6-лечение-перекоса-3-разделение-горячих-ключей-и-прочие-приёмы) |
| **Неверное число партиций** | задачи по 30 мс или задачи по 20 минут со spill | [§7](#7-число-партиций-арифметика-вместо-гадания)–[§8](#8-repartition-coalesce-repartitionbyrange-rebalance) |
| **Нехватка памяти** | spill, высокий GC time, OOM, `FetchFailed` | [§10](#10-память-экзекьютора-модель-и-расчёт)–[§12](#12-spill-что-это-и-как-убрать) |
| **Лишние shuffle** | много узлов `Exchange`, огромный Shuffle Write | [§14](#14-чек-лист-джоба-тормозит) и глава 03 |
| **Проблема источника** | долгий листинг, тысячи мелких файлов, нет pushdown | [§14](#14-чек-лист-джоба-тормозит) и глава 01 |
...
**Способ 1: Spark UI, отношение Max/Median.** Уже разобран в [§2](#2-чтение-spark-ui-по-шагам). Быстрее всего.
...
1. **Перекос в агрегации, а не в джойне.** `groupBy("user_id").agg(...)` с горячим ключом
   ...
   Лечится двухфазной агрегацией с солью ([§5](#5-лечение-перекоса-2-salting)).
...
если обе большие — используйте точечный вариант из [§6](#6-лечение-перекоса-3-разделение-горячих-ключей-и-прочие-приёмы).
...
| `coalesce(n, shuffle=True)` | да, полный | вверх и вниз | да | это и есть `repartition(n)` |
...
> 💬 **На собеседовании.** Спросят: «Можно ли через `coalesce` увеличить число партиций?»
> Хороший ответ: в форме по умолчанию — нет, `coalesce(n)` только склеивает соседние партиции
> и работает без shuffle. Но у него есть второй аргумент: `coalesce(n, shuffle=True)` добавляет
> полный shuffle и после этого умеет и увеличивать, и выравнивать. Более того, `repartition(n)`
> внутри Spark и реализован как `coalesce(n, shuffle=true)` — это не два разных механизма,
> а один и тот же с разными значениями флага.
> Провал: «нельзя, coalesce только уменьшает» — ответ верен ровно для формы по умолчанию
> и рассыпается от первого уточняющего вопроса.
...
**3. Обработка перекоса при джойне** — разобрана в [§4](#4-лечение-перекоса-1-aqe-skew-join).
...
| Причина | Лечение |
|---|---|
| Партиция слишком большая (перекос или мало партиций) | увеличить `shuffle.partitions`, лечить перекос ([§3](#3-перекос-данных-как-его-увидеть)–[§6](#6-лечение-перекоса-3-разделение-горячих-ключей-и-прочие-приёмы)) |
...
| `--executor-cores 5` | слотов задач на экзекьютор. 4–5 — рабочий диапазон (см. [§10](#10-память-экзекьютора-модель-и-расчёт)) |
...
| `spark.sql.shuffle.partitions=4000` | стартовое (при AQE — максимальное) число партиций после shuffle. Считается по [§7](#7-число-партиций-арифметика-вместо-гадания) |
| `spark.sql.adaptive.*` | блок AQE, разобран в [§9](#9-aqe-целиком) |
...
Отношение > 5 по Duration или Shuffle Read → перекос. Идите в [§3](#3-перекос-данных-как-его-увидеть)–[§6](#6-лечение-перекоса-3-разделение-горячих-ключей-и-прочие-приёмы). Это причина №1,
...
Ненулевой и сравнимый с Shuffle Read → мало памяти на задачу. [§12](#12-spill-что-это-и-как-убрать).

**4. Проверьте GC Time на вкладке Executors.**
Больше 10–15% от Task Time → память на пределе, скоро будет OOM. [§10](#10-память-экзекьютора-модель-и-расчёт)–[§11](#11-oom-пять-видов-симптомы-лечение).
...
как в [§13](#13-spark-submit-разбор-каждого-параметра).
...
чек-лист [§14](#14-чек-лист-джоба-тормозит) целиком, записывая ответ по каждому пункту.
...
- [Официальная документация: SQL Performance Tuning](https://spark.apache.org/docs/latest/sql-performance-tuning.html) —
  раздел про AQE с полным списком параметров и их дефолтов. Первоисточник для всего [§9](#9-aqe-целиком);
...
- [Официальная документация: Spark Configuration](https://spark.apache.org/docs/latest/configuration.html) —
  ...
  из [§13](#13-spark-submit-разбор-каждого-параметра) описаны здесь с актуальными дефолтами.
...
- [Официальная документация: Web UI](https://spark.apache.org/docs/latest/web-ui.html) —
  описание каждой метрики на всех вкладках. Читать параллельно с [§2](#2-чтение-spark-ui-по-шагам), с открытым UI своей джобы.
