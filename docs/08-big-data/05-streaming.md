...
**Как проработать:** чтение + поднять Kafka локально и прогнать практику
...
> способом: класть связанные события в одну партицию через ключ (см. [§3](#3-ключ-сообщения--главное-проектное-решение)).
...
«ключ → партиция» (см. [§3](#3-ключ-сообщения--главное-проектное-решение)).
...
но вы обязаны сделать обработку идемпотентной (см. [§13](#13-идемпотентность-и-дедупликация)). **Это дефолт для 90% ML-пайплайнов.**
...
продавца за час», «число неуспешных оплат по карте за 5 минут». Здесь важен выбор окна ([§9](#9-время-события-окна-и-watermark)).
...
[утечка](../02-classic-ml/05-validation-and-leakage.md). В стриминге лечится логированием:
...
Запустите код из [§14](#14-сквозной-пример-клики--kafka--агрегация--онлайн-feature-store) на локальной Kafka. Добейтесь того, чтобы: (а) джоб переживал рестарт
...
- **Gwen Shapira, Todd Palino, Rajini Sivaram, Krit Petty. «Kafka: The Definitive Guide», 2-е издание
  (O'Reilly, 2021)** — главы про продюсера, консьюмера и exactly-once закрывают [§3](#3-ключ-сообщения--главное-проектное-решение)–[§6](#6-гарантии-доставки-и-цена-каждой) этой главы
