...
> 🌱 **База.** Обязательного здесь три пункта. Первый — зачем оркестратор нужен вообще
> ([§1](#1-интуиция-почему-cron-перестаёт-работать)): cron не знает про зависимости между
> джобами и не умеет сказать скрипту, за какую дату он считает. Второй — модель интервалов
> ([§6](#6-logical_date-и-интервалы-главный-источник-путаницы)): запуск происходит в конце
> интервала данных, `{{ ds }}` — это не «сегодня», а `datetime.now()` внутри задачи — баг,
> который вылезет при первом же перезапуске за прошлую дату. Третий — идемпотентность
> ([§8](#8-идемпотентность-центральное-требование)): Airflow перезапустит задачу и по ретраю,
> и по кнопке Clear, поэтому задача, которая дописывает данные вместо перезаписи своей партиции,
> рано или поздно их удвоит. Разделы 9, 10, 13 и 14 (ретраи, пулы, мониторинг самого Airflow,
> сравнение с Dagster и Prefect) — это эксплуатация конкретной установки; они понадобятся,
> когда инстанс станет вашим, а при первом чтении их можно пролистать. Дальше идите, когда
> сможете своими словами объяснить, почему DAG «за 26 июля» выполняется 27-го и что произойдёт
> с таблицей, если неидемпотентную задачу перезапустить дважды.
