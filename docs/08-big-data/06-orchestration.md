...
**Время на проработку:** ~4 часа (чтение + поднять Airflow локально и прогнать backfill)
...
антипаттерн главы (§11): любой тяжёлый код на верхнем уровне файла выполняется десятки раз
в минуту.
...
времени парсинга (§13). Кнопка «Reparse DAG» в UI как раз принудительно запускает разбор
конкретного файла, не дожидаясь планового цикла, — она нужна, когда вы выкатили правку
и не хотите ждать.

**2. Передача данных через XCom.** Разобрано в §5.
...
**5. Вычисления внутри Airflow.** Разобрано в §2. Airflow — оркестратор.

**6. Неидемпотентные задачи.** Разобрано в §8.

**7. `catchup=True` по недосмотру.** Разобрано в §7.
...
*Что делать:* привести к одному из трёх идемпотентных паттернов (§8); до этого — не запускать
backfill вообще.
...
- [Airflow: Best Practices](https://airflow.apache.org/docs/apache-airflow/stable/best-practices.html) —
  короткий официальный документ, из которого выросла половина §11; там же — про верхний уровень
...
- **Bas Harenslak, Julian de Ruiter. «Data Pipelines with Apache Airflow» (Manning, 2021)** —
  ...
  как встроить обучение модели в пайплайн, ложатся ровно на §7–§12.
- [Документация Dagster: Assets](https://docs.dagster.io/) — стоит прочитать, даже если работаете
  ...
  взгляд на проектирование и напрямую отвечает на вопрос из §16 про свежесть данных.
...
- Дальше — [распределённое обучение](07-distributed-training.md), где разбирается, что происходит
  внутри задачи `train` из §12, и [CI/CD для ML](../07-mlops/07-ci-cd-for-ml.md), где пайплайн
