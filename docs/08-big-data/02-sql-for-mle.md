...
**Время на проработку:** ~8 часов (чтение + 15 задач руками)
...
после соединения — см. §2), и в промежуточном результате будет
...
> читайте §2, §5, §6 и решайте задачи 1–7 из §11. Этого достаточно для скрининга.
> Разделы §3 (физика джойнов), §10 (планы) и задачи 8–15 — уже про грейд middle+.
...
> Для кумулятивной суммы почти всегда нужен второй. Для сессионизации (задача 3 в §11)
...
следствие взрыва строк (§4), и `SUM`, посчитанный до `DISTINCT`, всё равно завышен.
...
испорченным и не анализируется. Подробнее — [ловушки A/B-тестов](../10-ab-testing/04-pitfalls.md).
...
[статистические критерии](../10-ab-testing/02-statistical-criteria.md).
...
**Вопрос вдогонку:** «Как дедуплицировать в стриме?» — см. врезку в §7: окно состояния
...
метрика тихо поедет. Два выхода: `RANGE` с интервалом (как выше) либо календарная опора (§8)
...
Здесь взрыв строк (§4) — не баг, а часть постановки: одна покупка порождает $k$ строк,
...
Причина: взрыв строк (§4) — правая таблица неуникальна по ключу.
...
Что делать: исключать когорты моложе горизонта метрики (§11, задача 4) либо явно
...
Что делать: point-in-time join (§11, задача 11), проверка инвариантом
...
Решите все задачи из §11, закрыв решения. Отмечайте те, где смотрели ответ.
...
- **Markus Winand. «SQL Performance Explained»** и его сайт
  ...
  и почему предикат перестаёт быть sargable. Лучший источник по §9.
...
- [ClickHouse SQL Reference](https://clickhouse.com/docs/en/sql-reference) — если работаете
  ...
  `windowFunnel` закрывают половину задач из §11 одной функцией.
