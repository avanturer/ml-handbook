...
на практике, и именно поэтому §9 в этой главе такой большой.
...
для нужного слоя. Подробности в §8 и в главе про
...
см. §7). Для восьми карт с $b=64$ и $A=1$ это $B_\text{eff}=512$.
...
и корректный LR (§6). Эластичный запуск, который молча меняет $N$ с 32 на 24, изменит
...
*Что делать:* линейное правило + warmup (§6); при использовании Adam попробовать корневое
...
*Что делать:* тест с синтетическим загрузчиком (§9); увеличить `num_workers` и `prefetch_factor`;
...
*Что делать:* полный чекпоинт (§10); тест на восстановление в CI: обучить 100 шагов, сохранить,
...
- [Goyal et al. «Accurate, Large Minibatch SGD: Training ImageNet in 1 Hour» (2017)](https://arxiv.org/abs/1706.02677) —
  первоисточник линейного правила масштабирования LR и warmup. Читать ради §2–§3 статьи:
