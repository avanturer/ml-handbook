...
**Как проработать:** чтение + прогон примеров на локальном Spark
...
Три полных прохода по 420 ГБ вместо одного. Лечится либо `cache()` (см. [§11](#11-cache-и-persist-уровни-хранения-и-когда-кэш-вредит)), либо тем,
...
mapPartitions, coalesce (в форме по умолчанию — слияние соседних партиций)
```
...
>
> Третья: «coalesce умеет только вниз» верно лишь для формы по умолчанию. У него есть второй
> аргумент, и `coalesce(n, shuffle=True)` — это ровно то, чем `repartition(n)` и реализован
> внутри: полный shuffle, умеет и вверх, и выравнивать. Подробнее — в
> [главе про тюнинг](04-spark-tuning.md#8-repartition-coalesce-repartitionbyrange-rebalance).
...
**Фаза 4. Генерация кода.** См. [§10](#10-tungsten-и-whole-stage-codegen).
...
4. **Разрыв whole-stage codegen**: узел `BatchEvalPython` рвёт конвейер, и соседние операторы
   тоже теряют codegen (см. [§10](#10-tungsten-и-whole-stage-codegen)).
...
Прогоните скрипт из [§13](#13-python-udf-почему-медленно-и-что-с-этим-делать) и добавьте четвёртый вариант: pandas UDF в итераторной форме
