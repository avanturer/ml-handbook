...
> необходимость в Delta/Iceberg/Hudi (см. [§11](#11-табличные-форматы-delta-iceberg-hudi)).
...
1. **Колонка без `RLE_DICTIONARY` в encodings, хотя значений мало.** Значит словарь переполнился
   (см. [§4](#4-кодировки-как-колонка-сжимается-ещё-до-компрессора)) и колонка пишется PLAIN. Частая причина раздутых файлов.
...
> из [§3.3](#33-смотрим-метаданные-руками): смотрите поле `encodings`.
...
и заодно включает data skipping ([§5](#5-статистики-predicate-pushdown-и-projection-pushdown)).
...
Отсюда — сортировка при записи и Z-ordering ([§12](#12-data-skipping-и-z-ordering)).
...
**Симптом 3: файлы по 200 КБ.** Партиций много → в каждую попадает мало данных → см. [§10](#10-small-files-problem).

**Практический предел:** держите число партиций **в тысячах, максимум в десятках тысяч**.
Если хочется резать мельче — это не партиционирование, это бакетирование ([§9](#9-бакетирование)) или
Z-ordering ([§12](#12-data-skipping-и-z-ordering)).
...
Вернёмся к свойству из [§3](#3-parquet-изнутри): **Parquet-файл неизменяем**. Из этого растут четыре практические
...
Мы уже установили ([§5](#5-статистики-predicate-pushdown-и-projection-pushdown)): статистики min/max работают тогда, когда данные кластеризованы
...
- 10 ГБ – 10 ТБ → партиционировать по дате, гранулярность считать из [§8](#8-партиционирование).
...
Что делать: скрипт из [§3.3](#33-смотрим-метаданные-руками), смотреть `encodings` по колонкам и `ratio`. Лечится нормализацией
...
Возьмите любую Parquet-таблицу от 10 ГБ и прогоните скрипт из [§3.3](#33-смотрим-метаданные-руками) на трёх случайных файлах.
...
Для трёх таблиц вашего проекта посчитайте по формуле из [§8](#8-партиционирование) оптимальную гранулярность.
...
- [Apache Parquet — официальная документация](https://parquet.apache.org/docs/) — раздел
  «File Format» описывает ровно ту иерархию, что в [§3](#3-parquet-изнутри), а «Encodings» — все кодировки
