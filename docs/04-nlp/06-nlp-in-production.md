...
**Как проработать:** чтение + замеры на своей машине
...
копирование, синхронизация), $C(T)$ — операции на один объект из [§3.1](#31-счёт-операций-в-слое-энкодера), $R$ — реальная
...
На данных из [§4.2](#42-три-стратегии-и-их-цена) (p95 = 162, p99 = 298) `max_length = 192` покрывает 96% текстов целиком
...
Порядок действий важен: сначала бесплатное ([§4](#4-батчинг-переменной-длины) — паддинг и батчинг), потом дешёвое
...
# Реальные эмбеддинги не заполняют пространство равномерно, а лежат вокруг тематических
# сгущений — воспроизводим именно такую структуру, иначе демонстрация ничего не покажет:
# в равномерном шуме высокой размерности все точки одинаково далеки друг от друга.
topics = rng.normal(size=(3, 64))


def sample(n, centers, scale=0.35):
    idx = rng.integers(0, len(centers), n)
    return centers[idx] + scale * rng.normal(size=(n, 64))


ref = sample(2000, topics)
same = sample(1000, topics)                                   # то же распределение
mixed = np.vstack([sample(800, topics),                       # 80% старых тем
                   sample(200, rng.normal(size=(1, 64)))])    # 20% новой темы

print("длины, без дрифта:      ", round(psi(rng.lognormal(3.6, .9, 5000),
                                           rng.lognormal(3.6, .9, 5000)), 3))
print("длины, тексты длиннее:  ", round(psi(rng.lognormal(3.6, .9, 5000),
                                           rng.lognormal(4.3, .9, 5000)), 3))
print("эмбеддинги, без дрифта: ", embedding_drift(ref, same))
print("эмбеддинги, новая тема: ", embedding_drift(ref, mixed))
# длины, без дрифта:       0.003
# длины, тексты длиннее:   0.575
# эмбеддинги, без дрифта:  {'centroid_shift': 0.0006, 'outlier_share': 0.05}
# эмбеддинги, новая тема:  {'centroid_shift': 0.1203, 'outlier_share': 0.237}
```

`outlier_share` при отсутствии дрифта держится около 0,05 по построению (мы взяли 95-й процентиль),
а подмешивание 20% новой темы поднимает его до 0,24 — почти ровно доля новых объектов плюс
фоновые 5%. Сдвиг центроида тоже реагирует, но слабее: новое подмешано к старому, и среднее
сдвигается пропорционально доле. Именно поэтому эти две метрики держат вместе — центроид ловит
общий сдвиг всего потока, доля выбросов ловит появление нового меньшинства.
...
и наивное удаление символов категории `Cf` (как в [§2.2](#22-токенизатор--это-часть-модели-а-не-утилита)) разрывает такие последовательности.
...
регуляркой из [§2.2](#22-токенизатор--это-часть-модели-а-не-утилита). С опечатками системно борются двумя путями: символьные или subword-модели
...
из [§8](#8-мониторинг-качества-без-разметки) и проверьте, какие срабатывают в каком сценарии.
...
Напишите тесты на нормализатор из [§2.2](#22-токенизатор--это-часть-модели-а-не-утилита): идемпотентность, устойчивость к пустому входу,
...
- [Gretton et al. «A Kernel Two-Sample Test» (JMLR, 2012)](https://www.jmlr.org/papers/v13/gretton12a.html) —
  строгая версия того, что в [§8](#8-мониторинг-качества-без-разметки) сделано эвристикой: как корректно проверять, что две выборки
