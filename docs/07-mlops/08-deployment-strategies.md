...
**Предварительно нужно:** [CI/CD для ML](07-ci-cd-for-ml.md), [архитектуры сервинга](05-serving-architectures.md), [Docker и Kubernetes](06-docker-and-k8s.md), [упаковка модели](04-model-packaging.md), [дизайн эксперимента](../06-ab-testing/01-experiment-design.md)
**Как проработать:** чтение + собрать канареечный пайплайн на своём сервисе
...
> 🌱 **База.** Если тема новая, минимальный маршрут: [§2](#2-пять-стратегий-на-одной-оси-риска) (сводная таблица стратегий),
> [§4](#4-canary-критерии-продвижения-в-числах) (канарейка — то, чем пользуются каждый день) и [§10](#10-чек-лист-выката-модели-15-пунктов) (чек-лист). Одна мысль, которую
...
измеряются в A/B (см. [§6](#6-ab-выкатка-как-измерение)), канарейка отвечает только за инженерную стабильность и грубые
прокси качества.
...
детерминированное хеширование `user_id` (код в [§8](#8-feature-flags-для-моделей)).
...
guardrail-метрики. Всё это разбирается в [дизайне эксперимента](../06-ab-testing/01-experiment-design.md)
и [ловушках A/B](../06-ab-testing/04-pitfalls.md).
...
регрессию. Посчитайте по формуле [§4](#4-canary-критерии-продвижения-в-числах) число запросов и длительность для долей 1%, 5% и 25%.
...
Разверните `Rollout` из [§4](#4-canary-критерии-продвижения-в-числах) на любом сервисе (подойдёт заглушка, отдающая случайное число).
...
Возьмите код `ModelRouter` из [§8](#8-feature-flags-для-моделей), напишите тест: 100 000 случайных `user_id`, конфиг 5/95.
...
Возьмите последний выкат модели в вашей команде и пройдите по чек-листу из [§10](#10-чек-лист-выката-модели-15-пунктов).
