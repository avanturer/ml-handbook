...
**Предварительно нужно:** [валидация и утечки](../02-classic-ml/05-validation-and-leakage.md), [метрики](../02-classic-ml/04-metrics.md)
...
- [3. Стадия 0 крупным планом: а нужен ли здесь ML](#3-стадия-0-крупным-планом-а-нужен-ли-здесь-ml) — оракульный расчёт, порог окупаемости, build vs buy
- [4. Роли и зоны ответственности](#4-роли-и-зоны-ответственности)
- [5. Технический долг ML: разбор статьи Google](#5-технический-долг-ml-разбор-статьи-google)
- [6. Стоимость владения в числах](#6-стоимость-владения-в-числах)
- [7. Уровни зрелости MLOps 0 / 1 / 2](#7-уровни-зрелости-mlops-0--1--2)
- [8. Что ломается на каждой стадии](#8-что-ломается-на-каждой-стадии)
- [9. Антипаттерн «ноутбук в проде»](#9-антипаттерн-ноутбук-в-проде)
- [10. Вывод из эксплуатации](#10-вывод-из-эксплуатации)
...
> **Где это в книге.** Здесь мы разбираем жизненный цикл по этапам: что происходит с моделью
> от идеи до вывода из эксплуатации. Собрать эти этапы в связный ответ на проектной секции —
> отдельный навык, и ему посвящён [весь раздел ML System Design](../11-system-design/01-framework.md)
> в конце книги. Сейчас он не нужен: сначала надо знать этапы, потом их складывать.
...
## 3. Стадия 0 крупным планом: а нужен ли здесь ML

В таблице выше стадия 0 занимает одну строку и 3–10 дней. Это единственная строка, где решение
«не делать» стоит дешевле любого другого, и единственная, которую систематически пропускают.

Живой пример того, что происходит при пропуске. Команда из двух человек четыре месяца строила
модель, предсказывающую, какому пользователю показать промо-баннер. Модель получилась
приличная. На A/B она дала +0.4% к конверсии в целевое действие. При обороте продукта эта
дельта — около 700 тысяч рублей в год. Стоимость владения моделью — примерно 0.3 FTE
([§6](#6-стоимость-владения-в-числах)), то есть около 1.8 млн рублей в год, плюс четыре человеко-месяца, уже потраченных.
Модель работает, метрика выросла, эксперимент значим — и проект убыточен с первого дня.
Причём это было **вычислимо до старта**, за половину дня, по историческим данным.

Разберём, как считать, и почему «не делать ML» — валидный, а часто и правильный ответ.

### 3.1. Оракульный расчёт: потолок ценности до первой строки кода

Первый вопрос не «какую модель взять», а «сколько денег принесёт **идеальная** модель».
Идеальная — то есть та, что безошибочно угадывает целевой класс. Это верхняя граница, и если
она не покрывает стоимость владения, дальше можно не считать: никакая архитектура не спасёт.

Возьмём типовую постановку «предсказать и воздействовать»: модель ранжирует объекты, мы
воздействуем на верхние $K$ штук (звоним, шлём предложение, отправляем на ручную проверку),
и часть воздействий срабатывает. Годовая ценность модели:

$$
V_{\text{год}} \;=\; 12 \cdot K \cdot \big(\pi_{\text{ml}} - \pi_{\text{base}}\big) \cdot q \cdot L
$$

где $K$ — число объектов, на которые мы воздействуем за месяц (это бюджет кампании, он обычно
фиксирован); $\pi_{\text{base}}$ — доля попаданий текущего решения, то есть precision@$K$
у действующего правила; $\pi_{\text{ml}}$ — доля попаданий модели; $q$ — вероятность, что
воздействие сработало на попадании; $L$ — денежная ценность одного сработавшего воздействия;
12 — месяцев в году.

> 📐 **Разбор формулы.** Читаем слева направо. $K$ и $q$ и $L$ задаются продуктом и бухгалтерией,
> а не ML: сколько мы можем обзвонить, какая доля откликается, сколько стоит удержанный
> пользователь. Единственная величина, на которую влияет модель, — это разность
> $\pi_{\text{ml}} - \pi_{\text{base}}$, **прирост** точности над тем, что уже работает.
> Не абсолютное качество: если правило уже даёт 12% попаданий, модель с 15% приносит ценность
> трёх пунктов, а не пятнадцати. Ошибка «посчитать ценность от нуля, как будто до нас ничего
> не было» — самая частая в защите ML-проектов и самая заметная для того, кто утверждает бюджет.
> Обратите внимание и на то, что стоимость самой кампании в формулу не входит: при фиксированном
> $K$ мы контактируем одно и то же число людей в обоих вариантах, и эта статья сокращается.

Потолок получается подстановкой $\pi_{\text{ml}} = \pi_{\text{oracle}} = \min(1,\ C/K)$,
где $C$ — сколько объектов целевого класса вообще появляется за месяц. Смысл простой: если
уходящих 28 тысяч, а обзвонить мы можем 40 тысяч, то даже всеведущая модель наберёт максимум
70% попаданий — остальные 30% списка занять просто нечем.

### 3.2. Порог окупаемости: разработка разовая, владение вечное

Теперь вторая половина: во что обходится модель. Здесь важна асимметрия, которую в защитах
проектов почти всегда смазывают. **Разработка — разовый расход, владение — ежегодный.**
Правильный критерий поэтому не «окупится ли когда-нибудь», а «покрывает ли годовая ценность
годовую стоимость владения плюс амортизацию разработки на разумном горизонте». Модель, которая
приносит ровно столько, сколько стоит её поддержка, — это чистый убыток: она вдобавок съедает
внимание команды, которого хватило бы на другой проект.

Приравняв ценность к издержкам, получаем минимальный прирост точности, ради которого стоит
браться:

$$
\pi_{\text{ml}} - \pi_{\text{base}} \;\ge\; \frac{C_{\text{own}} + C_{\text{dev}} / T}{12 \cdot K \cdot q \cdot L}
$$

где $C_{\text{own}}$ — стоимость владения за год (люди плюс инфраструктура),
$C_{\text{dev}}$ — стоимость разработки, разовая, $T$ — горизонт в годах, на котором мы её
амортизируем (обычно 2–3: дальше продукт изменится).

Посчитаем на живых числах. Подписочный сервис, отток 3.5% в месяц, действующее правило
«не заходил 14 дней» даёт precision@$K$ = 12%. Кампания удержания: скидка тем, кто в списке,
срабатывает на четверти реально уходящих, удержанный подписчик приносит ещё 3000 рублей.
Разработка — два инженера на четыре месяца, около 4 млн рублей полной стоимости; владение —
0.3 FTE плюс ночной батчевый скоринг, около 1.9 млн рублей в год.

```python
# Порог окупаемости anti-churn модели: сколько precision@K она обязана добрать,
# чтобы покрыть разработку и владение. Ничего специфичного для ML — арифметика.

def payback(base, churn_rate, contacts, precision_rule,
            conv_retain, ltv_retain, dev_cost, own_cost_year, horizon_years):
    churners = base * churn_rate                      # уходящих в месяц
    oracle_precision = min(1.0, churners / contacts)  # потолок: идеальная модель
    # годовые издержки, приведённые к горизонту принятия решения
    annual_cost = own_cost_year + dev_cost / horizon_years
    # ценность одного пункта precision в год (бюджет контактов фиксирован,
    # поэтому стоимость самой кампании в дельту не входит и сокращается)
    value_per_point = contacts * 0.01 * conv_retain * ltv_retain * 12
    need_points = annual_cost / value_per_point       # в процентных пунктах
    headroom_points = (oracle_precision - precision_rule) * 100
    return {
        "уходящих в месяц": round(churners),
        "precision оракула, %": round(oracle_precision * 100, 1),
        "зазор до оракула, п.п.": round(headroom_points, 1),
        "издержки в год, млн руб": round(annual_cost / 1e6, 2),
        "цена 1 п.п. precision в год, млн руб": round(value_per_point / 1e6, 2),
        "нужно добрать, п.п.": round(need_points, 1),
        "доля зазора, которую надо забрать, %": round(100 * need_points / headroom_points, 1),
    }


common = dict(churn_rate=0.035, precision_rule=0.12, conv_retain=0.25,
              ltv_retain=3000, dev_cost=4_000_000, own_cost_year=1_900_000,
              horizon_years=2)

print("=== Крупный сервис: 800 000 подписчиков, контактируем 5% базы ===")
for k, v in payback(base=800_000, contacts=40_000, **common).items():
    print(f"{k:44s} {v}")

print()
print("=== Тот же продукт, но 80 000 подписчиков, контакты 5% базы ===")
for k, v in payback(base=80_000, contacts=4_000, **common).items():
    print(f"{k:44s} {v}")
```

```text
=== Крупный сервис: 800 000 подписчиков, контактируем 5% базы ===
уходящих в месяц                             28000
precision оракула, %                         70.0
зазор до оракула, п.п.                       58.0
издержки в год, млн руб                      3.9
цена 1 п.п. precision в год, млн руб         3.6
нужно добрать, п.п.                          1.1
доля зазора, которую надо забрать, %         1.9

=== Тот же продукт, но 80 000 подписчиков, контакты 5% базы ===
уходящих в месяц                             2800
precision оракула, %                         70.0
зазор до оракула, п.п.                       58.0
издержки в год, млн руб                      3.9
цена 1 п.п. precision в год, млн руб         0.36
нужно добрать, п.п.                          10.8
доля зазора, которую надо забрать, %         18.7
```

Читается это так. На крупной базе модель обязана поднять precision@40k с 12% до 13.1% —
одна десятая процентного пункта сверху за каждый миллион рублей издержек. Это забирает 1.9%
от зазора до всеведущего оракула, то есть задача решается почти любой честно обученной моделью.
Ответ: делать.

На базе в десять раз меньше **всё остальное не меняется** — ни отток, ни экономика удержания,
ни зарплаты, — но порог вырастает с 1.1 до 10.8 пунктов: нужно забрать почти пятую часть
оракульного зазора только для выхода в ноль. Первая версия модели обычно берёт 20–40%
от зазора, то есть решение балансирует на грани, а любая неточность в оценке $q$ или $L$
переворачивает вывод. Ответ: не начинать четырёхмесячный проект, а за неделю усложнить правило
и измерить, сколько даёт оно.

Вот содержательный итог, который и делает этот расчёт обязательным: **один и тот же проект
с одной и той же моделью окупается на большой базе и не окупается на маленькой**, и определяется
это арифметикой до старта, а не качеством модели после. Стоимость владения не масштабируется
вниз вместе с бизнесом — 0.3 FTE стоят одинаково при 800 тысячах пользователей и при 80 тысячах.

> ⚠️ **Типичная ошибка.** Считать ценность от нуля: «модель ловит 28% оттекающих, это столько-то
> миллионов». Так вы приписываете ML заслуги действующего правила. Считать надо **прирост
> над бейзлайном**, и бейзлайн для этого должен быть измерен, а не оценён на глаз. Второй
> вариант той же ошибки — сравнивать модель с «ничего не делаем», когда на самом деле
> альтернатива не «ничего», а «полдня работы аналитика над правилом».

> 💬 **На собеседовании.** Спросят: «Как вы решаете, браться ли за ML-проект?» Хороший ответ —
> это расчёт вслух, а не список принципов: «Считаю верхнюю границу. Сколько объектов целевого
> класса в месяц, на скольких мы можем воздействовать, какая доля воздействий срабатывает,
> сколько стоит одно срабатывание. Умножаю — получаю ценность идеальной модели. Отдельно —
> ценность действующего правила: разница между ними и есть весь потенциальный выигрыш. Дальше
> сравниваю с издержками, где владение считаю ежегодным, а разработку амортизирую на два года.
> Если нужный прирост точности больше трети зазора до оракула, я предлагаю не делать модель,
> а потратить неделю на эвристику и вернуться к вопросу с измеренным бейзлайном». Плохой ответ:
> «если есть данные и задача интересная».

### 3.3. Heuristics-first: почему эвристика не выбрасывается

Правило «сначала эвристика» звучит как совет для бедных, а на деле у него три независимых
основания, и только первое — про экономию.

**Первое: эвристика даёт бейзлайн, без которого нельзя посчитать ценность модели.** Это ровно
та величина $\pi_{\text{base}}$ из формулы выше. Пока её нет, любая защита ML-проекта опирается
на выдуманное число.

**Второе: эвристика в проде производит данные.** Правило, запущенное на месяц раньше модели, —
это месяц логов воздействий и откликов, то есть готовая обучающая выборка с обратной связью.
Команды, которые начинают с модели, обнаруживают на третьем месяце, что таргета в нужной форме
просто нет, потому что раньше никто не воздействовал и не логировал результат.

**Третье, и самое недооценённое: эвристика становится признаком.** Правило «не заходил 14 дней
и отменил автопродление» — это спрессованное доменное знание. Модель может вывести его сама
из сырых счётчиков, но потратит на это ёмкость и данные, а на редких сегментах не выведет вовсе.
Отдав правило как бинарный признак, вы дарите модели готовый ответ. Google в «Rules of ML»
разбирает четыре способа переиспользовать накопленную эвристику, и все четыре стоит держать
в голове: (1) сделать из неё признак; (2) применить её на препроцессинге, отфильтровав вход;
(3) разобрать её на сырые входы и отдать модели их, если само правило грубовато;
(4) использовать её выход как исходную метку или как начальное приближение. Чего в этом списке
нет — так это «удалить».

**И четвёртое, уже эксплуатационное:** эвристика остаётся фолбэком. Когда модель недоступна,
протухли фичи или сработал circuit break, отвечает правило — и качество деградирует, а не
исчезает (см. [архитектуры сервинга](05-serving-architectures.md)).

> 🧠 **Middle+.** Есть обратная сторона, о которой честно сказать выгоднее, чем умолчать.
> Эвристика-признак с сильным весом создаёт петлю: правило определяет, кого мы трогаем, отклик
> собирается только на этих людях, модель обучается на смещённой выборке и подтверждает правило.
> Через год у вас модель, которая умеет предсказывать поведение вашего правила, а не оттока.
> Лечение то же, что и в рекомендациях: доля случайного воздействия (1–3% бюджета кампании
> уходит на случайных людей вне списка) и логирование того, по какой причине объект попал
> в список. Это стоит доли процента бюджета и спасает следующую версию модели.

### 3.4. Юнит-экономика предсказания

Второй способ зарубить проект на бумаге — сравнить стоимость одного предсказания с приростом
ценности одного решения, который оно даёт. Обе величины считаются легко, и разрыв часто
оказывается в два-три порядка.

Пример. Кто-то предлагает переранжировать выдачу рекламных блоков языковой моделью на каждый
показ. Считаем: eCPM 100 рублей означает 0.1 рубля выручки с показа, маржинальность 30% —
0.03 рубля вклада. Идеальное переранжирование добавит к этому проценты, оптимистично +10%,
то есть 0.003 рубля. Один вызов LLM с контекстом стоит от десятых долей рубля. Отношение —
два порядка не в пользу идеи, и это верно **при любом качестве модели**: спор о том, какая
LLM лучше ранжирует, здесь бессмысленен.

Правило, которое стоит проговаривать вслух: **сравнивать стоимость предсказания надо
с приростом ценности решения, а не с полной ценностью объекта**. Показ стоит 0.1 рубля,
но ваша модель борется не за весь показ, а за его улучшение.

Отсюда же следует, что «нужен ли ML» и «нужен ли онлайн-ML» — два разных вопроса.
Батчевое предсказание стоит на два-три порядка дешевле онлайнового
([стоимость и ёмкость](10-cost-and-capacity.md)), и задача, безнадёжная в онлайне, вполне может
окупаться ночным пересчётом.

### 3.5. Build vs buy

Третья развилка стадии 0: сделать самим или взять готовое. Критерий не «что лучше по метрике»,
а **есть ли у вас данные, которых нет у вендора**.

**Покупать** имеет смысл там, где задача общая и решается на данных, которых у вас всё равно
меньше: распознавание речи, OCR документов, перевод, детекция лиц, базовая модерация
изображений, общий антиспам. Коробочное решение обучено на объёмах, недоступных отдельной
компании, и ваша команда за квартал его не догонит. Ключевой вопрос здесь: даёт ли эта задача
конкурентное преимущество? Если распознавание паспорта у вас будет на 2% точнее, чем
у конкурента, продукт от этого не выиграет.

**Строить** имеет смысл там, где предсказание опирается на ваши данные: ранжирование вашего
каталога, скоринг вашей клиентской базы, антифрод под вашу схему мошенничества, рекомендации
на вашей истории взаимодействий. Никакой вендор этого не сделает лучше, потому что данных
у него нет.

Что не входит в сравнение цен, но переворачивает решение:

| Скрытое в «купить» | Скрытое в «построить» |
|---|---|
| лок-ин: миграция стоит столько же, сколько первая интеграция | время до запуска: месяцы против дней |
| нельзя дообучить под свой домен, а качество на нём обычно ниже заявленного | 0.2–0.5 FTE навсегда, не только на время разработки |
| латентность внешнего вызова 50–300 мс и чужой SLA внутри вашего p99 | необходимость собрать разметку, которой у вендора уже есть миллионы примеров |
| данные уходят наружу — вопрос к юристам и регулятору | риск, что первая версия окажется хуже коробки, и это выяснится через квартал |
| вендор меняет цены и модель под вами, а вы это увидите как дрифт | |

Практическое правило, которое почти всегда верно: **для первой версии берите готовое, даже
если планируете строить своё.** Внешний API даёт измеримый бейзлайн за неделю и снимает главный
риск — что функция вообще не нужна продукту. Переход на своё делается позже, когда объём
вырос настолько, что счёт за API сравнялся со стоимостью владения собственным флотом; эта точка
считается по формулам из [главы про стоимость](10-cost-and-capacity.md).

### 3.6. Признаки задачи, где ML не нужен вовсе

Список, который стоит держать в голове целиком: каждый пункт — готовый аргумент на секции
system design.

- **Правило исчерпывающее и стабильное.** Расчёт налога, применение тарифа, проверка
  на соответствие регламенту. Здесь ML вносит недетерминизм туда, где нужен детерминизм,
  и создаёт ошибки на ровном месте.
- **Данных мало.** Сотни объектов, десятки событий целевого класса. Модель выучит шум,
  а доверительный интервал её метрики будет шире любого эффекта. Экспертное правило честнее.
- **Обратной связи нет.** Истинный ответ не приходит никогда (мы заблокировали транзакцию —
  и не узнаем, была ли она мошеннической) или приходит через годы. Модель некуда переобучать
  и нечем мониторить.
- **Цена ошибки катастрофична, а проверить человеком нельзя.** Тогда либо человек в контуре
  и ML как подсказка, либо не ML.
- **Требуется объяснимость каждого решения по закону.** Отказ в кредите, медицинское решение,
  кадровое решение в ряде юрисдикций. Иногда это снимается интерпретируемой моделью, иногда нет.
- **Потолок ценности ниже стоимости владения.** Оракульный расчёт из [§3.1](#31-оракульный-расчёт-потолок-ценности-до-первой-строки-кода) дал число меньше,
  чем 0.3 FTE в год.
- **Задача решается изменением продукта, а не предсказанием.** Самый частый и самый обидный
  случай. «Давайте предсказывать, кто не найдёт товар в поиске» — почти всегда означает, что
  надо чинить поиск. «Предскажем, кто бросит корзину на шаге оплаты» — сначала посмотрите,
  почему шаг оплаты отваливается на мобильных. ML здесь маскирует продуктовый дефект и делает
  его вечным, потому что теперь на нём построена система.

> 💬 **На собеседовании.** Спросят: «Вам поставили задачу сделать модель, а вы считаете,
> что она не нужна. Что делаете?» Хороший ответ показывает, что вы умеете не спорить,
> а измерять: «Не отказываюсь и не соглашаюсь сразу, а прошу два-три дня на расчёт потенциала
> и приношу три числа: ценность идеальной модели, ценность действующего правила и стоимость
> владения. Дальше решение принимает не моё мнение, а арифметика. Если она в пользу ML —
> я первым и берусь. Если против — предлагаю дешёвую альтернативу с измеримым эффектом:
> правило в проде за неделю, и через месяц у нас есть и результат, и данные, чтобы вернуться
> к вопросу о модели». Плохой ответ: «объяснил бы менеджеру, что ML тут не поможет» — это
> позиция без доказательства, и её не примут.
...
## 5. Технический долг ML: разбор статьи Google
...
## 8. Что ломается на каждой стадии
...
<details>
<summary><b>🧠 Вопрос.</b> Как понять до старта проекта, что ML здесь окупится?</summary>

**Короткий ответ.** Посчитать потолок: сколько принесла бы идеальная модель, вычесть из него
то, что уже приносит действующее правило, и сравнить остаток со стоимостью владения за год
плюс амортизацией разработки. Если требуемый прирост точности составляет заметную долю зазора
до оракула, проект рискованный.

**Развёрнуто.** Годовая ценность считается как
$12 K (\pi_{\text{ml}} - \pi_{\text{base}}) q L$: $K$ — сколько объектов в месяц мы можем
обработать (бюджет кампании), $\pi$ — доли попаданий модели и текущего решения, $q$ — доля
сработавших воздействий, $L$ — денежная ценность одного срабатывания. Потолок получается
подстановкой $\pi_{\text{oracle}} = \min(1, C/K)$, где $C$ — число объектов целевого класса
в месяц. Издержки: владение ежегодно (0.2–0.5 FTE плюс инфраструктура), разработка разово
и амортизируется на 2–3 года. Ключевой практический вывод — стоимость владения не
масштабируется вниз вместе с бизнесом: один и тот же проект окупается на базе в 800 тысяч
пользователей и не окупается на базе в 80 тысяч, потому что 0.3 FTE стоят одинаково.

**Чего ждёт интервьюер:** что вы считаете прирост **над бейзлайном**, а не ценность от нуля,
и что владение для вас ежегодная статья, а не «разработали и забыли».

**Провал:** «посмотрим на AUC и решим» — качество модели не отвечает на вопрос об окупаемости.
</details>

<details>
<summary><b>🎯 Вопрос.</b> Зачем начинать с эвристики, если вы всё равно потом сделаете модель?</summary>

**Короткий ответ.** Эвристика даёт три вещи, которых иначе нет: измеренный бейзлайн, без
которого нельзя посчитать ценность модели; поток логов воздействия и отклика, то есть готовый
таргет; и сильный признак для будущей модели. Плюс она остаётся фолбэком, когда модель
недоступна.

**Развёрнуто.** Эвристику не выбрасывают, а переиспользуют, и способов четыре: сделать из неё
бинарный признак; применить на препроцессинге, отфильтровав вход; разобрать её на сырые
входы и отдать модели их; использовать её выход как исходную разметку. Правило вроде
«не заходил 14 дней и отменил автопродление» — это спрессованное доменное знание, которое
модель по сырым счётчикам восстановит дорого, а на редких сегментах не восстановит вовсе.
Обратная сторона, которую стоит назвать самому: правило определяет, кого мы трогаем, отклик
собирается только на них, и модель начинает предсказывать поведение правила, а не оттока.
Лечится долей случайного воздействия в 1–3% бюджета и логированием причины попадания в список.

**Чего ждёт интервьюер:** что «сначала эвристика» для вас инженерный приём с обоснованием,
а не признак низкой амбиции.

**Провал:** «эвристика — это временное решение до модели».
</details>
...
Возьмите любой свой ML-проект (учебный тоже подойдёт) и заполните таблицу из [§7](#7-уровни-зрелости-mlops-0--1--2) по всем
...
Возьмите ноутбук, обучающий любую модель, и приведите проект к структуре из [§9](#9-антипаттерн-ноутбук-в-проде): пакет `src/`,
...
**Задача 5. Расчёт потенциала до старта (50 минут).**
Возьмите любую ML-задачу, которую вы делали или собираетесь делать, и заполните шесть чисел
из [§3](#3-стадия-0-крупным-планом-а-нужен-ли-здесь-ml): $C$ (объектов целевого класса в месяц), $K$ (сколько можем обработать),
$\pi_{\text{base}}$ (точность действующего решения), $q$ (доля сработавших воздействий),
$L$ (ценность одного срабатывания), стоимость владения за год. Посчитайте порог
$\pi_{\text{ml}} - \pi_{\text{base}}$ и его долю от зазора до оракула. Затем повторите расчёт,
уменьшив масштаб бизнеса в десять раз.
*Сделано правильно: у вас есть конкретное число «модель обязана добрать X процентных пунктов»
и вывод «делать / не делать» с указанием, какое из шести чисел вы знаете хуже всех и что
изменит вывод при его двукратной ошибке.*

**Задача 6. Ассерты в пайплайн (45 минут).**
...
- [Sculley et al. «Hidden Technical Debt in Machine Learning Systems» (NeurIPS 2015)](https://papers.nips.cc/paper/2015/hash/86df7dcfd896fcaf2674f757a2463eba-Abstract.html) —
  первоисточник [§5](#5-технический-долг-ml-разбор-статьи-google). Короткая (9 страниц) и её реально спрашивают. Читать целиком,
  особенно разделы про CACE, glue code и configuration debt.
- [Google Cloud. «MLOps: Continuous delivery and automation pipelines in machine learning»](https://cloud.google.com/architecture/mlops-continuous-delivery-and-automation-pipelines-in-machine-learning) —
  первоисточник классификации уровней 0/1/2 из [§7](#7-уровни-зрелости-mlops-0--1--2), со схемами компонентов каждого уровня.
...
⬅️ [Платформа экспериментов](../06-ab-testing/07-experiment-platform.md) | 🏠 [Оглавление](../index.md) | ➡️ [Воспроизводимость и трекинг](02-reproducibility-and-tracking.md)
