...
Формализуем. Есть модель $f_\theta: \mathbb{R}^d \to \mathbb{R}^K$ с параметрами $\theta$, где
$d$ — размерность входа (число признаков), а $K$ — число классов. На вход подаётся объект
$x \in \mathbb{R}^d$ с истинной меткой $y$, качество ответа меряет функция потерь $L$. Атакующий
ищет добавку $\delta$, которая ломает предсказание, оставаясь незаметной:
...
где $t = 0, 1, 2, \dots$ — номер итерации, $x^{(t)}$ — текущее приближение, $\alpha$ — шаг
(обычно $\varepsilon/4$ или меньше), $\mathcal{B}_\varepsilon(x)$ — шар радиуса $\varepsilon$
вокруг исходного объекта, $\Pi$ — проекция на пересечение шара с допустимой областью,
$x^{(0)}$ — случайная точка внутри шара. PGD считается стандартом оценки устойчивости:
...
Читаем вывод. При $\varepsilon = 0.1$ (каждый пиксель сдвинут максимум на десятую часть
...
Механизм водяных знаков неочевиден, поэтому разберу его отдельно: это **намеренная
...
Отсюда практичный вывод: отдельного арсенала против MIA заводить не нужно, первый рубеж вы уже
построили. Всё, что сокращает разрыв обобщения, сокращает и утечку — регуляризация, ранняя
остановка, dropout, аугментации, отказ дообучать до нулевой ошибки на train. Вы применяете это
ради качества на новых данных, а приватность достаётся побочным эффектом, бесплатно.
...
где $\epsilon \ge 0$ — бюджет приватности (чем меньше, тем строже), а $\delta \in [0,1]$ —
допустимая вероятность того, что гарантия вообще не сработает (берут порядка $1/n$ или меньше,
где $n$ — число записей). Обе буквы здесь заняты второй раз, и это не опечатка, а устоявшаяся
нотация двух разных областей: $\epsilon$ приватности не имеет отношения к радиусу атаки
$\varepsilon$ из раздела 2, а $\delta$ приватности — к возмущению $\delta$ оттуда же. Дальше в
разделе буквы означают только приватность.
...
где $\tilde g$ — зашумлённый градиент, которым и делается шаг оптимизатора,
...
$\mathcal{N}$ — гауссов вектор соответствующей размерности.
...
> 💬 **На собеседовании.** Спросят: «Приходит команда приватности и говорит — обучим эту модель
> с epsilon = 3. Что это вам даёт и что вы у них переспросите?»
...
Строку про «разметить признаки по контролируемости атакующим» разверну: приём работает в любом
сервисе с противником, а делают его редко. Разделите признаки на три группы: полностью подконтрольные атакующему (сумма,
...
- **OWASP «Top 10 for Large Language Model Applications»** — чек-лист по LLM-специфике
  (prompt injection, небезопасная работа с инструментами, утечки через плагины); продолжает
  [§12](#12-модель-угроз-для-антифрода) там, где в сервисе появляется языковая модель. Ищите
  по названию проекта на сайте OWASP: он переезжал между доменами, поэтому прямую ссылку не даю.
