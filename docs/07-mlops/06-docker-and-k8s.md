...
**Как проработать:** чтение + собрать образ + поднять в minikube/kind
...
- **Deployment** — декларация «хочу N подов с таким образом». Он создаёт ReplicaSet и управляет
  выкаткой новой версии ([§14](#14-rolling-update-и-корректное-завершение)).
...
- **Ingress** — правила внешнего HTTP-роутинга: какой хост и путь ведут в какой Service,
  плюс TLS.
- **ConfigMap / Secret** — конфигурация отдельно от образа ([§11](#11-configmap-и-secret)).
...
- В образе процесс должен быть PID 1 и получать сигнал напрямую — отсюда exec-форма `CMD` ([§4](#4-рабочий-dockerfile)).
...
[главы 05](05-serving-architectures.md), не подглядывая в [§10](#10-полный-манифест-сервиса). Значения requests/limits
...
- [Docker build cache (docs.docker.com)](https://docs.docker.com/build/cache/) —
  как именно инвалидируется кэш и как им управлять. Прямо отвечает на [§2](#2-слои-кэш-и-порядок-инструкций).
