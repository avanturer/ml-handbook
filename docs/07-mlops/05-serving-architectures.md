...
**Время на проработку:** ~5 часов (чтение + поднять сервис + нагрузочный тест)
...
`asyncio.to_thread(model.predict, X)` из §5 действительно распараллеливается, а
...
- **GPU-модель**: один процесс на GPU. Несколько процессов на одну карту дерутся за память
  и контекст; параллелизм добывается батчингом внутри процесса или через Triton (§8).
...
- **Держать запас (headroom).** Целевая утилизация 50–60% из §3 — это и есть запас: он
...
под SLA (см. код в §5 и [стек наблюдаемости](../09-monitoring/05-observability-stack.md)).
...
Что делать: считать память по формуле из §6; `joblib.load(mmap_mode="r")` для разделения
...
Возьмите код из §5, обучите любой LightGBM на табличных данных, запустите сервис. Прогоните
...
показать, что она соответствует утилизации около 0.7, и посчитать по формуле §3, сколько
