...
**Время на проработку:** ~6 часов (чтение + расчёт бюджета + прототип дистилляции)
...
осязаемая, самая дешёвая и самая часто внедряемая польза — про неё §3.
...
**Через API.** Возьмём дешёвую модель уровня «mini/flash»: порядка $0.3 за 1 млн входных
и $1.2 за 1 млн выходных токенов (порядок величины; конкретные цены смотрите у провайдера
на день расчёта).
...
> (4) дистиллировать — заплатить один раз за разметку и держать в проде дешёвую модель (§9);
> (5) уменьшить выход — заставить модель отдавать не перестановку, а один токен (§4).
...
Цена бутстрапа — умножение стоимости на $M$. При $M=5$ и расчёте из §2 запрос стоит уже
...
- как **учитель для дистилляции** — про это §9, и это самое частое реальное применение.
...
- **id как токены.** `item_7391` разбивается токенизатором на куски, между которыми нет
  семантики — та же проблема, которую в §5 решают semantic IDs. Обобщение на новые айтемы
  почти отсутствует.
...
предшественника TIGER, но не как архитектуру для внедрения. Наследники — instruction-tuned
модели поверх открытых LLM (линия TALLRec, InstructRec и подобных) — упираются в тот же
барьер стоимости и живут в основном как учителя для дистилляции.
...
против $95 млн/год за онлайн-инференс из §2.
...
3. **Посчитайте стоимость учителя** калькулятором из §2 **до** запуска. Если получилось
...
| Извлечение атрибутов и тегов из описаний | офлайн, батч | тысячи $ разово + центы/сутки | **в проде широко** |
| Понимание поисковых запросов (расширение, исправление, длинный хвост) | офлайн-кэш + онлайн на редких запросах | сотни–тысячи $/мес | **в проде** |
| Дистилляция в ранкер / кросс-энкодер | офлайн разметка, лёгкий ученик онлайн | тысячи $ разово | **в проде, главный паттерн** |
| Semantic IDs как признаки ранжирования | офлайн | стоимость обучения RQ-VAE | **в проде точечно** (крупные компании) |
| Генерация объяснений | офлайн для топ-N пар, кэш | ~$1–2 тыс. на полный прогон | **в проде точечно** |
| Диалоговые рекомендации / ассистент | онлайн, низкий QPS | зависит от объёма чата | **в проде как отдельная поверхность** |
| LLM-ранкер на срезе трафика (<1%, холодные пользователи) | онлайн | сотни тысяч $/год | **пилоты** |
| LLM-ранкер на всём трафике ленты | онлайн | десятки–сотни млн $/год | **не поедет** |
...
Что делать: считать бюджет на полном трафике **до** пилота, калькулятором из §2, и заранее
...
Что делать: шаг 5 рецепта из §9 — ручная валидация учителя на 300–500 примерах перед массовым
...
Расширьте функцию из §2: добавьте расчёт числа GPU для self-hosted варианта (по формуле
...
обучите RQ-VAE из §5 с $L=3$, $V=256$.
...
- [Rajput S. et al. «Recommender Systems with Generative Retrieval» (2023)](https://arxiv.org/abs/2305.05065) —
  TIGER. Главная статья §5. Смотреть раздел про RQ-VAE и про обработку коллизий,
...
- [Sun W. et al. «Is ChatGPT Good at Search? Investigating Large Language Models as Re-Ranking Agents» (2023)](https://arxiv.org/abs/2304.09542) —
  ...
  в существенно меньшую специализированную модель. Ключевая работа для §9.
...
- [Hou Y. et al. «Towards Universal Sequence Representation Learning for Recommender Systems» (2022)](https://arxiv.org/abs/2206.05941) —
  ...
  рекомендательной модели адаптером. Прямая опора для §3.
- **Singh A. et al. «Better Generalization with Semantic IDs: A Case Study in Ranking for Recommendations», RecSys 2024** —
  ...
  Лучший аргумент в пользу «скромной» версии идеи из §5.
...
- [Глава про инференс LLM](../05-llm/05-inference-and-serving.md) — если хотите считать
  латентность и железо точнее, чем в §2: prefill vs decode, KV-cache, continuous batching.
