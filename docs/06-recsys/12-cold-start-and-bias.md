...
**Время на проработку:** ~5 часов (чтение + симуляция петли + расчёт бюджета эксплорации)
...
Это ровно та же идея, что и контентная башня для айтемов из §4, только со стороны пользователя.
...
5. **Петля обратной связи.** Показали популярное → собрали ещё взаимодействий по популярному →
   усилили. Механику разбираем в §9.
...
в **рекомендациях** с концентрацией во **входных данных**. $\Delta G = G_{\text{показы}} -
G_{\text{исторические взаимодействия}}$. Если $\Delta G > 0$ — система не просто отражает
...
2. **Взвешивание по propensity** (§10) — восстанавливаем несмещённость оценки, если умеем оценить
   вероятность показа.
...
Понять петлю лучше всего, воспроизведя её. Здесь синтетический мир: истинные предпочтения
фиксированы, модель — просто эмпирический CTR с добавленной эксплорацией.

```python
import numpy as np

rng = np.random.default_rng(0)
M, U, K, DAYS = 2000, 5000, 10, 60          # айтемов, пользователей в день, слотов, дней
true_ctr = rng.beta(1.2, 30, size=M)         # истинная привлекательность айтемов
...
def gini(x):
    x = np.sort(x.astype(float)); m = x.size; t = x.sum()
    return 0.0 if t <= 0 else float(((2 * np.arange(1, m + 1) - m - 1) * x).sum() / (m * t))

for day in range(DAYS):
    # оценка модели: сглаженный эмпирический CTR (у непоказанных остаётся априор)
    score = (clicks + 1.0) / (shows + 50.0)
    order = np.argsort(-score)
    daily = np.zeros(M)
    for _ in range(U):
        eps = 0.0                            # << поменяйте на 0.05 и сравните графики
        n_explore = int(K * eps)
        slate = np.concatenate([order[:K - n_explore], rng.choice(M, n_explore, replace=False)])
        daily[slate] += 1
        clicked = rng.random(K) < true_ctr[slate]
        clicks[slate] += clicked
    shows += daily
    exposure_log.append((day, gini(daily), int((daily > 0).sum())))

for row in exposure_log[::15]:
    print(f"день {row[0]:>2}: Gini={row[1]:.3f}  уникальных айтемов={row[2]}")
```

При `eps = 0.0` число уникальных показанных айтемов схлопывается до нескольких десятков, а Джини
уходит к 0,9+ — при том что «качество» (средний CTR) выглядит отлично, ведь мы показываем то,
что действительно кликают. При `eps = 0.05` система продолжает открывать хорошие айтемы, которым
не повезло на старте. Это лучшая иллюстрация к тому, зачем нужна следующая глава.
...
**Развёрнуто.** $\hat R_{DR} = \frac{1}{|\mathcal{U}||\mathcal{I}|}\sum_{u,i}[\hat\delta_{u,i} +
O_{u,i}(\delta_{u,i} - \hat\delta_{u,i})/P_{u,i}]$. Если $P$ верны, ожидание второго слагаемого
...
Возьмите код из §9 и доведите его до эксперимента: постройте графики Джини по дням и числа
уникальных показанных айтемов для `eps ∈ {0, 0.01, 0.05, 0.2}`. Добавьте измерение «регрета» —
разницы между средним CTR показанного и средним CTR десяти лучших айтемов мира.
*Критерий: при eps = 0 число уникальных айтемов схлопывается более чем в 10 раз за 60 дней;
вы можете назвать значение eps, при котором регрет минимален, и объяснить, почему он растёт
и при слишком малом, и при слишком большом eps.*
...
Для своей системы (или для чисел из §1) посчитайте: сколько показов в сутки уйдёт на квоту новым
...
- [Schnabel T. et al. «Recommendations as Treatments: Debiasing Learning and Evaluation» (ICML 2016)](https://arxiv.org/abs/1602.05352) —
  ...
  датасет Coat. Читать вместе с §8 и §10.
- [Joachims T., Swaminathan A., Schnabel T. «Unbiased Learning-to-Rank with Biased Feedback» (WSDM 2017)](https://arxiv.org/abs/1608.04468) —
  ...
  оценки propensity. Продолжение §7.
...
- **Jiang R. et al. «Degenerate Feedback Loops in Recommender Systems» (DeepMind, AIES 2019)** —
  формальная модель вырождения выдачи и роль эксплорации в его замедлении. Теоретическая опора §9.
...
- **Abdollahpouri H. et al. — серия работ про popularity bias в рекомендациях (2019–2021)** —
  ...
  Полезны как источник аккуратных определений метрик из §6.
...
- **Wang X. et al. «Position Bias Estimation for Unbiased Learning to Rank in Personal Search»
  (WSDM 2018, Google)** — regression-EM для оценки propensity без интервенций; способ 4 из §7.
