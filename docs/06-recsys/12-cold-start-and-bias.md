...
Возьмите код из §9 и доведите его до эксперимента: постройте графики Джини по дням, числа
уникальных показанных айтемов и накопленного регрета (разницы между CTR показанного и потолком)
для `eps ∈ {0, 0.01, 0.05, 0.1, 0.3}` и трёх разных сидов. Затем поменяйте априор в скоре
с `+1.0 / +50.0` на `+0.1 / +5.0` (более «доверчивая» модель) и повторите.
*Критерий: вы воспроизвели схлопывание каталога примерно в 9 раз при eps = 0; нашли значение eps,
минимизирующее регрет, и можете объяснить, почему кривая регрета U-образная; и обнаружили, что
сила априора влияет на скорость вырождения не слабее, чем eps, — оптимистичный априор
(«непоказанное считается хорошим») сам работает как эксплорация.*
