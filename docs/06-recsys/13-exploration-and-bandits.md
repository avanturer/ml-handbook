...
**Время на проработку:** ~4 часа (чтение + реализация трёх алгоритмов + сравнение регрета)
...
> Thompson sampling, у которого этой проблемы нет по построению. Числа — в §6.
...
**UCB1 «из учебника» здесь худший.** Ровно по причине из §4: граница Хёфдинга не знает, что
...
**хотя бы одна** из двух моделей (propensity или модель награды), — вывод тот же, что в §10
...
с самого начала знала лучшую руку: $R(T) = T\mu^* - \mathbb{E}[\sum_t r_t] = \sum_a \Delta_a
\mathbb{E}[n_a(T)]$. Он переводит «баланс разведки и эксплуатации» в измеримую величину.
...
**Развёрнуто.** Апостериор $\propto \theta^{\alpha-1}(1-\theta)^{\beta-1}\cdot\theta^s(1-\theta)^{n-s}
= \theta^{(\alpha+s)-1}(1-\theta)^{(\beta+n-s)-1}$ — то же бета-семейство с параметрами
...
Реализуйте ε-greedy, UCB1 и Thompson sampling и воспроизведите таблицу из §6: $K = 10$,
...
- [Slivkins A. «Introduction to Multi-Armed Bandits»](https://arxiv.org/abs/1904.07272) —
  ...
  на уровне, который реально читается инженером. Главы 1–2 закрывают §2–§6 этой главы,
  глава про контекстные бандиты — §7.
...
- **Auer P., Cesa-Bianchi N., Fischer P. «Finite-time Analysis of the Multiarmed Bandit Problem»
  (Machine Learning, 2002)** — первоисточник UCB1 и его анализа. Вывод из §4 — оттуда.
- [Li L., Chu W., Langford J., Schapire R. «A Contextual-Bandit Approach to Personalized News
  ...
  индустриальная проверка контекстных бандитов (Yahoo! News). Читать вместе с §7.
- **Chapelle O., Li L. «An Empirical Evaluation of Thompson Sampling» (NIPS 2011)** — работа,
  ...
  обратной связи. Прямая опора для §6 и §10.
...
- [Dudík M., Langford J., Li L. «Doubly Robust Policy Evaluation and Learning» (ICML 2011)](https://arxiv.org/abs/1103.4601) —
  DR-оценка для контекстных бандитов, разбор компромисса смещение/дисперсия. Основа §8.
