...
Соберём это в один пример. Пусть у запроса пять документов с релевантностями $(4,3,0,0,0)$
и модель поставила их в порядке $(0,0,4,3,0)$ — то есть два хороших документа оказались на 3-й и 4-й
позиции. NDCG@5 такой выдачи — примерно $0{,}66$. Теперь переставим только первый и третий:
получим $(4,0,0,3,0)$ и NDCG $\approx 0{,}88$. Один обмен из десяти возможных дал +0.22, остальные
девять — почти ничего. Обучение должно знать, какие обмены дорогие, а какие нет. **Learning to rank
...
- **Вероятностные**: ListNet вводит распределение «вероятность быть первым»
  $P(i) = e^{s_i}/\sum_k e^{s_k}$ и минимизирует кросс-энтропию между распределением по меткам
  и по скорам; ListMLE максимизирует правдоподобие Плакетта — Льюса для правильной перестановки.
  Фактически это softmax cross-entropy по списку — сегодня самый популярный listwise-лосс в
  нейронных ранжировщиках, потому что дёшев и хорошо себя ведёт.
- **Прямые аппроксимации метрики**: SoftRank, ApproxNDCG, NeuralNDCG — заменяют жёсткую сортировку
  гладкой (сигмоида вместо индикатора «выше», softmax-перестановки) и дифференцируют результат.
