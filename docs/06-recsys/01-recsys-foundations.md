...
**Время на проработку:** ~4 часа (чтение + эксперименты с матрицей взаимодействий)
...
| Датасет / система | $\|U\|$ | $\|I\|$ | Взаимодействий | Плотность | Разреженность |
|---|---|---|---|---|---|
...
```mermaid
flowchart TD
    A["Каталог<br/>10⁶ – 10⁸ айтемов"] --> B

    subgraph B["1. Отбор кандидатов (retrieval)"]
        B1["ANN-поиск<br/>по эмбеддингам"]
        B2["Item2item:<br/>похожие на просмотренные"]
        B3["Популярное<br/>в сегменте / регионе"]
        B4["Правила: новинки,<br/>подписки, брошенная корзина"]
    end

    B --> C["Объединение + дедупликация<br/>10³ – 10⁴ кандидатов<br/>бюджет 10–30 мс"]
    C --> D["Обогащение признаками<br/>feature store, онлайн-счётчики<br/>бюджет 10–30 мс"]
    D --> E["2. Ранжирование<br/>тяжёлая модель, сотни признаков<br/>10² – 10³ айтемов, бюджет 20–50 мс"]
    E --> F["3. Переранжирование и правила<br/>разнообразие, бизнес-логика, фильтры<br/>10 – 50 позиций, бюджет 1–5 мс"]
...
дисперсией. Практический ответ: обучать на «глубоких» сигналах (досмотр, повторный визит,
покупка без возврата), а не на клике; ставить **guardrail-метрики** в A/B; держать
долгосрочный холдаут (см. [Онлайн-оценка рекомендаций](15-recsys-online-evaluation.md)).
