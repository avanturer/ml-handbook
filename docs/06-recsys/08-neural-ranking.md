...
> 🌱 **База.** Минимальный маршрут: §2 (эмбеддинги и hashing trick), §3 (DLRM — базовая
> архитектура CTR), §7 (калибровка). Этого хватает, чтобы разговаривать про нейронный
...
товаров). Вернёмся к примеру из §1: история — дрель, шуруповёрт, кроссовки, детская книжка,
...
Соберите модель: bottom MLP, 5 эмбеддинг-таблиц, interaction из §3, top MLP. Обучите на
...
Примените поправку из §8 и постройте калибровочные кривые всех трёх вариантов.
...
- **Ma J. et al. «Modeling Task Relationships in Multi-task Learning with Multi-gate
  ...
  синтетические эксперименты с регулируемой корреляцией задач; задача 5 из §13 воспроизводит
  именно его.
