...
**Предварительно нужно:** [каркас ответа](01-framework.md), [дисбаланс и калибровка](../02-classic-ml/13-imbalance-and-calibration.md), [метрики качества](../02-classic-ml/04-metrics.md), [нейронное ранжирование](../10-recsys/08-neural-ranking.md), [работа с признаками](../02-classic-ml/14-feature-engineering.md)
...
- **Новые креативы не имеют статистики**, а без показов её и не получат. Отсюда обязательная
  эксплорация — доля показов, выделенная новым кампаниям (см. [§10.4](#104-новый-рекламодатель-залил-кампанию-что-показывает-ваша-модель)).
...
конверсий, см. [§10.3](#103-конверсии-приходят-через-несколько-дней-как-обучать-cvr-модель).
...
(1) коррекция прореживания негативов по формуле из [§4](#4-шаг-3-данные); (2) **изотоническая регрессия
...
баланса) и обязательно применяем аналитическую коррекцию из [§4](#4-шаг-3-данные). Разница между двумя подходами
...
1. **Наследование статистики по иерархии.** У креатива нет данных, но есть у кампании; у кампании
   ...
   данных. Та же формула, что в [§5](#5-шаг-4-постановка-ml-задачи-и-бейзлайн), но с многоуровневым $\alpha$.
...
до и после коррекции по формуле из [§4](#4-шаг-3-данные).
...
- **McMahan et al. «Ad Click Prediction: a View from the Trenches» (KDD 2013)** — обязательное
  ...
  экономия памяти, метрики. Почти всё, что описано в [§5](#5-шаг-4-постановка-ml-задачи-и-бейзлайн)–[§6](#6-шаг-5-признаки-и-модель) этой главы, взято оттуда.
...
- **Chapelle. «Modeling Delayed Feedback in Display Advertising» (KDD 2014)** — формальная
  модель отложенной конверсии из [§10.3](#103-конверсии-приходят-через-несколько-дней-как-обучать-cvr-модель), включая вывод правдоподобия.
...
- Смежные главы: [дисбаланс и калибровка](../02-classic-ml/13-imbalance-and-calibration.md),
