...
**Предварительно нужно:** [каркас ответа](01-framework.md), [основы рекомендаций](../10-recsys/01-recsys-foundations.md), [двухбашенные модели и ANN](../10-recsys/06-two-tower-and-ann.md), [обучение ранжированию](../10-recsys/07-learning-to-rank.md), [холодный старт и смещения](../10-recsys/12-cold-start-and-bias.md)
**Как проработать:** самостоятельный прогон + сверка с разбором
...
> 🌱 **База.** Кейс — не эталон для заучивания, а каркас из [предыдущей главы](01-framework.md)
> в работе: числа здесь входные данные этой конкретной задачи, на другом собеседовании они будут
> другими. Переносимого четыре. Двухстадийная воронка «кандидаты на полноту → ранжирование
> на точность» ([§4](#4-шаг-4-постановка-ml-задачи-и-бейзлайн), [§5](#5-шаг-5-признаки-и-модель)):
> 25 млн карточек дорогой моделью не перебрать, и вся остальная архитектура следует из этого
> одного ограничения. Объединение источников кандидатов **множеством, а не взвешенной суммой
> скоров** ([§5](#5-шаг-5-признаки-и-модель)) — скоры источников несравнимы по шкале, и на этом
> вопросе проваливаются чаще всего. Смещение обратной связи ([§3](#3-шаг-3-данные)) — тема,
> которую поднимаете вы, а не интервьюер. И способ выводить архитектуру из чисел: смотрите
> не на сами 250 мс в [§6](#6-шаг-6-архитектура), а на то, как бюджет раскладывается по стадиям
> и как из «6000 RPS × 1000 кандидатов» получается число машин — считать так вы будете в любом
> кейсе, а миллисекунды не повторятся. [§9](#9-куда-обычно-копает-интервьюер) — шесть углублений,
> куда вас утянут после основного рассказа; это материал второго прохода, перед самим
> собеседованием, и при первом чтении достаточно знать, что такие вопросы существуют.
> Дальше идите, когда сможете своими словами сказать, зачем нужна стадия кандидатов и почему
> источники нельзя смешивать по весам.
...
- **Position bias:** карточка на позиции 1 получает в 4–6 раз больше кликов, чем та же карточка
  ...
  (см. [§9](#9-куда-обычно-копает-интервьюер), вопрос 3).
...
Временной сплит из [§3](#3-шаг-3-данные). Сравниваем **с текущей прод-моделью**, а не со случайной выдачей.
...
инициализируются средним по её категории и продавцу через сглаживание из [§5](#5-шаг-5-признаки-и-модель); (3) явная квота
...
где $C$ — множество кандидатов, $S$ — уже отобранные карточки, $U(u,i)$ — полезность из [§4](#4-шаг-4-постановка-ml-задачи-и-бейзлайн),
...
Что делать: байесовское сглаживание к среднему по категории (формула в [§5](#5-шаг-5-признаки-и-модель)) плюс обязательный
...
Возьмите формулу $U(u,i)$ из [§4](#4-шаг-4-постановка-ml-задачи-и-бейзлайн) и подберите веса так, чтобы: (а) карточка с $\hat p_{\text{click}}=0.05$
...
- **Zhao et al. «Recommending What Video to Watch Next: A Multitask Ranking System» (RecSys 2019)** —
  ...
  в [§5](#5-шаг-5-признаки-и-модель) и [§9.3](#93-у-вас-бесконечная-лента-что-с-позиционным-биасом) этой главы.
- [Zhou et al. «Deep Interest Network for Click-Through Rate Prediction» (2017)](https://arxiv.org/abs/1706.06978) —
  ...
  на вариант B из [§5](#5-шаг-5-признаки-и-модель).
...
- [Google. «Rules of Machine Learning»](https://developers.google.com/machine-learning/guides/rules-of-ml) —
  правила 1–16 почти дословно описывают лестницу усложнения из [§4](#4-шаг-4-постановка-ml-задачи-и-бейзлайн).
- Смежные главы хендбука: [холодный старт и смещения](../10-recsys/12-cold-start-and-bias.md),
