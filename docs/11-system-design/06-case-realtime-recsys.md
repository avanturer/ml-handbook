...
**Предварительно нужно:** [каркас ответа](01-framework.md), [рекомендации в продакшене](../10-recsys/14-recsys-in-production.md), [последовательные рекомендации](../10-recsys/09-sequential-recsys.md), [потоковая обработка](../08-big-data/05-streaming.md), [feature store](../07-mlops/03-data-and-feature-store.md)
...
> товаров, потом дорогое ранжирование сотен (см. [основы рекомендаций](../10-recsys/01-recsys-foundations.md)).
...
> против трёх месяцев на полноценный потоковый контур, и оно даёт большую часть эффекта ([§5](#5-шаг-5-признаки-три-уровня-свежести-и-их-цена)).
...
| Разнообразие выдачи (доля уникальных категорий в топ-10) | не ниже текущего | реалтайм схлопывает выдачу — см. [§9](#9-шаг-8-три-честные-слабости-и-развитие) |
...
так: «мой ожидаемый порядок, который я проверю экспериментом со ступенями задержки из [§2](#2-шаг-2-метрики-чем-свежесть-отличается-от-качества)».
...
в названный бюджет, но съедает его почти целиком. Именно поэтому лестница из [§4](#4-шаг-4-постановка-ml-задачи-бейзлайн-и-лестница) идёт снизу
...
| ANN по сессионному состоянию $h_t$ | HNSW по эмбеддингам товаров, запрос — $h_t$ из [§5](#5-шаг-5-признаки-три-уровня-свежести-и-их-цена) | 12 мс | обобщение: находит то, что не встречалось вместе явно |
...
обучение консистентным продовому поведению ([§3](#3-шаг-3-данные-логи-событий-и-point-in-time-корректность)).

**Четвёртое: контур обратной связи** замкнут через показы и клики, уходящие обратно в Kafka.
Отсюда же берётся смещение и петля, о которой в [§9](#9-шаг-8-три-честные-слабости-и-развитие).
...
| Слой | Реакция |
|---|---|
| Сервис рекомендаций | читает timestamp признака; если возраст > 120 с — подставляет дефолты (те же, что использовались при обучении, см. feature dropout в [§6](#6-шаг-5б-модель-сессионный-энкодер-и-ранжирование)) и инкрементирует метрику деградации |
...
За две недели этого достаточно, чтобы поймать эффекты уровня L1–L3 из [§4](#4-шаг-4-постановка-ml-задачи-бейзлайн-и-лестница) (ожидаемые 0,3–2,5%),
...
свежести. Результат такого теста — та самая кривая из [§4](#4-шаг-4-постановка-ml-задачи-бейзлайн-и-лестница), но измеренная, а не предположенная,
...
- **Снижение дисперсии**: CUPED по предэкспериментальной активности пользователя даёт заметный
  выигрыш на таких метриках — см. [снижение дисперсии](../06-ab-testing/03-variance-reduction.md).
...
а по нашей же лестнице ([§4](#4-шаг-4-постановка-ml-задачи-бейзлайн-и-лестница)) даёт +0,9% и +0,3% против +2,5% и +1,4% у почти бесплатных
...
*Что делать:* кумулятивные признаки со сдвигом ([§3](#3-шаг-3-данные-логи-событий-и-point-in-time-корректность)), `merge_asof` с `direction="backward"`,
...
Возьмите код `merge_asof` из [§3](#3-шаг-3-данные-логи-событий-и-point-in-time-корректность) и постройте два датасета для одной синтетической сессии:
...
Доработайте `SlidingCounter` из [§5](#5-шаг-5-признаки-три-уровня-свежести-и-их-цена): добавьте поддержку нескольких окон (1/10/60 минут)
...
- [Zhou et al. «Deep Interest Network for Click-Through Rate Prediction» (KDD 2018)](https://arxiv.org/abs/1706.06978) —
  внимание кандидата к истории пользователя; вариант B из [§6](#6-шаг-5б-модель-сессионный-энкодер-и-ранжирование), с честным обсуждением
  вычислительной цены.
...
- Внутри хендбука: [рекомендации в продакшене](../10-recsys/14-recsys-in-production.md) —
  полная архитектура сервиса и бюджет по стадиям; [последовательные рекомендации](../10-recsys/09-sequential-recsys.md) —
  ...
  point-in-time correctness и материализация; [холодный старт и смещения](../10-recsys/12-cold-start-and-bias.md) —
