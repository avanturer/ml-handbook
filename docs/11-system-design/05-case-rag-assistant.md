...
**Предварительно нужно:** [каркас ответа](01-framework.md), [RAG](../05-llm/07-rag.md), [эмбеддинги](../04-nlp/02-embeddings.md), [инференс LLM](../05-llm/05-inference-and-serving.md), [оценка LLM](../05-llm/09-llm-evaluation.md), [ANN-поиск](../06-recsys/06-two-tower-and-ann.md)
...
контента; при цене входных токенов это укладывается в бюджет (расчёт в §8). Меньше 200 токенов —
...
в бюджет (§8). Кросс-энкодер размером с LLM в этот бюджет не влезает, и это правильный ответ
...
**Разбор.** Честно посчитать MDE (см. §9: около 3 п.п. за две недели) и признать, что мелкие
...
раздувают ошибку первого рода. См. [ловушки A/B](../10-ab-testing/04-pitfalls.md).
...
*Что делать:* стратегия по типу источника (§5), таблицы с повторением шапки, код по блокам,
...
Возьмите код `split_markdown` из §5 и доработайте: (а) таблицы Markdown не режутся, а если
...
Возьмите код из §5, доведите его до эксперимента: 50 тысяч векторов, доля разрешённых
...
- [Es et al. «RAGAS: Automated Evaluation of Retrieval Augmented Generation» (2023)](https://arxiv.org/abs/2309.15217) —
  ...
  метрик, но помните про валидацию судьи из §9.
- [Asai et al. «Self-RAG: Learning to Retrieve, Generate, and Critique through Self-Reflection» (2023)](https://arxiv.org/abs/2310.11511) —
  про обучение модели решать, когда искать и когда отказываться; прямо про §7.
...
- Внутри хендбука: [RAG](../05-llm/07-rag.md) — полная механика пайплайна;
  ...
  роутинг и стоимость; [ANN-поиск](../06-recsys/06-two-tower-and-ann.md) — про HNSW и его параметры.
