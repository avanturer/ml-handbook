...
**Предварительно нужно:** [каркас ответа](01-framework.md), [uplift и причинность](../02-classic-ml/17-uplift-and-causal.md), [дизайн эксперимента](../06-ab-testing/01-experiment-design.md), [снижение дисперсии](../06-ab-testing/03-variance-reduction.md), [метрики качества](../02-classic-ml/04-metrics.md)
...
> ни разу не видели. Отсюда всё своеобразие метрик ([§3](#3-шаг-2-метрики-от-инкрементальной-маржи-к-qini)): их приходится считать на группах,
> а не на объектах.
...
| Оптимизация бюджета (60 итераций бисекции) | 2 мин | векторизовано, см. код в [§6](#6-шаг-5-признаки-и-модель) |
...
- **MDE.** Маржа за квартал тяжелохвостая, $\sigma \approx 2\,000$ ₽. Для детекции
  ...
  на группу. С [CUPED](../06-ab-testing/03-variance-reduction.md) на предпериодной марже
...
Отвечаем матрицей четырёх типов и арифметикой из [§2](#2-почему-модель-оттока--неправильная-постановка): топ по риску состоит преимущественно
...
Возьмите код из [§3](#3-шаг-2-метрики-от-инкрементальной-маржи-к-qini), обучите T-learner и модель на преобразованном классе на своих
...
Возьмите `allocate()` из [§6](#6-шаг-5-признаки-и-модель), задайте три оффера с разной стоимостью и разными профилями
...
- Для внутренней теории: [uplift и причинность](../02-classic-ml/17-uplift-and-causal.md),
  ...
  [сложные схемы экспериментов](../06-ab-testing/05-complex-designs.md).
