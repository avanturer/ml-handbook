...
def qini_coefficient(uplift_score: np.ndarray, y: np.ndarray, w: np.ndarray) -> float:
    ...
    return area / n                              # нормируем на размер выборки
...
print("Qini у идеального скора:", round(qini_coefficient(tau_true, y, w), 5))
print("Qini у случайного скора:", round(qini_coefficient(rng.normal(size=n), y, w), 5))
```
...
n = 1_000_000
tau = np.clip(rng.normal(0.01, 0.03, size=(n, 2)), -0.05, 0.20)   # два оффера
