...
COVERAGE: list[tuple[str, str, list[str]]] = [
    ...
    ("Ordered boosting и target statistics в CatBoost", "02-classic-ml/09-boosting-in-practice.md",
     [r"ordered (?:boosting|target)", r"prediction shift"]),
    ("GOSS и EFB в LightGBM", "02-classic-ml/09-boosting-in-practice.md",
     [r"GOSS", r"EFB"]),
    ("Каталог утечек данных", "02-classic-ml/05-validation-and-leakage.md",
     [r"утечк", r"leakage"]),
    ("Target encoding и out-of-fold", "02-classic-ml/14-feature-engineering.md",
     [r"out[-\s]?of[-\s]?fold", r"target encoding"]),
    ("SMOTE и почему он часто вредит", "02-classic-ml/13-imbalance-and-calibration.md",
     [r"SMOTE"]),
    ("Калибровка: Платт и изотоническая", "02-classic-ml/13-imbalance-and-calibration.md",
    ...
    # --- recsys ---
    ("Почему «просто SVD» не работает на пропусках", "10-recsys/03-collaborative-filtering.md",
     [r"пропуск", r"заполнени\w+ нул"]),
    ("Вывод шага ALS для implicit", "10-recsys/04-implicit-feedback.md",
     [r"ALS", r"confidence"]),
    ("Вывод функции потерь BPR", "10-recsys/04-implicit-feedback.md",
     [r"BPR"]),
    ("LambdaRank: градиент через ΔNDCG", "10-recsys/07-learning-to-rank.md",
     [r"Lambda(?:Rank|MART)", r"NDCG"]),
    ("Позиционный биас и IPS", "10-recsys/07-learning-to-rank.md",
     [r"позиционн\w+ биас", r"IPS", r"propensity"]),
    ("Почему офлайн-метрики расходятся с онлайном", "10-recsys/02-metrics-offline.md",
     [r"офлайн", r"онлайн"]),
    ("Semantic IDs и генеративный ретривал", "10-recsys/11-llm-recsys.md",
     [r"semantic ID", r"TIGER", r"генеративн\w+ ретривал"]),
    ("Петля обратной связи и вырождение выдачи", "10-recsys/12-cold-start-and-bias.md",
    ...
    # --- A/B ---
    ("MDE и вывод размера выборки", "06-ab-testing/01-experiment-design.md",
     [r"MDE", r"размер выборки"]),
    ("Ratio-метрики и дельта-метод", "06-ab-testing/02-statistical-criteria.md",
     [r"дельта[-\s]?метод", r"ratio[-\s]?метрик"]),
    ("Чем p-value НЕ является", "01-math/03-statistics.md",
     [r"НЕ является|не является"]),
    ("CUPED: вывод и требование к ковариате", "06-ab-testing/03-variance-reduction.md",
     [r"CUPED"]),
    ("Подглядывание и инфляция FPR", "06-ab-testing/04-pitfalls.md",
     [r"подглядыван", r"peeking"]),
    ("SRM и проверка хи-квадрат", "06-ab-testing/04-pitfalls.md",
     [r"SRM"]),
    ("Множественные сравнения: Bonferroni и FDR", "06-ab-testing/04-pitfalls.md",
     [r"Bonferroni", r"FDR|Benjamini"]),
    ("Парадокс Симпсона с механизмом", "06-ab-testing/04-pitfalls.md",
     [r"Симпсон"]),
    ("Switchback и сетевые эффекты", "06-ab-testing/05-complex-designs.md",
     [r"switchback|свитчбэк", r"сетев\w+ эффект"]),
    ("Что на самом деле проверяет Манн — Уитни", "06-ab-testing/02-statistical-criteria.md",
     [r"Манн\w*\s*[—-]\s*Уитни", r"Mann\s*[—-]?\s*Whitney"]),
    ("Тест Уэлча как разумный дефолт", "06-ab-testing/02-statistical-criteria.md",
     [r"Уэлч"]),
    ("Тяжёлые хвосты: винзоризация и бутстрап", "06-ab-testing/02-statistical-criteria.md",
     [r"винзориз", r"тяж\w+ хвост"]),
    ("Novelty и primacy: как детектировать", "06-ab-testing/04-pitfalls.md",
     [r"новизн", r"novelty"]),
    ("Guardrail-метрики и решение при их просадке", "06-ab-testing/01-experiment-design.md",
     [r"guardrail|гардрейл"]),
    ("Выбросы фильтруются по предэкспериментальному периоду", "06-ab-testing/04-pitfalls.md",
     [r"выброс"]),
    ...
    ("Платформа экспериментов: слои и ортогональность",
     "06-ab-testing/07-experiment-platform.md",
     [r"сло[йя]|layer"]),
    ("Платформа экспериментов: counterfactual logging",
     "06-ab-testing/07-experiment-platform.md",
     [r"counterfactual|контрфакт"]),
    ("Триггерный анализ", "06-ab-testing/03-variance-reduction.md",
     [r"триггерн\w+ (?:популяц|анализ|метрик)"]),
    ("Байесовский A/B", "06-ab-testing/02-statistical-criteria.md",
     [r"байесовск"]),
