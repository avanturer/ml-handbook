...
# Названия разделов, которые говорят о РОЛИ куска в шаблоне, а не о его содержании.
# Стандарт (AUTHORING §2.2) требует обратного: по заголовку должно быть понятно, о чём раздел,
# иначе оглавление главы превращается в бланк, одинаковый у всех глав, и читатель по нему
# не может выбрать, что читать. Исключение — пять блоков с фиксированными именами.
TEMPLATE_HEADINGS = {
    "формализация", "вывод", "введение", "интуиция", "на практике", "теория",
    "определение", "основные понятия", "обзор", "заключение", "постановка задачи",
    "математика", "детали", "разбор",
}
...
def check_structure(path: Path, text: str, report: Report) -> None:
    ...
    for heading in re.findall(r"^##\s*(.+?)\s*$", text, re.MULTILINE):
        bare = re.sub(r"^\d+\.?\s*", "", re.sub(r"[`*]", "", heading)).strip().lower().rstrip(":")
        if bare in TEMPLATE_HEADINGS:
            report.warn(path, f"заголовок называет роль в шаблоне, а не содержание: «{heading}»")
